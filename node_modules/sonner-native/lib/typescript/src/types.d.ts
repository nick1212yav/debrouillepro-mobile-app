...
import type { BaseAnimationBuilder, EntryExitAnimationFunction, ReanimatedKeyframe as Keyframe } from 'react-native-reanimated';
...
export type ToastEntryExitAnimation = EntryExitAnimationFunction | BaseAnimationBuilder | typeof BaseAnimationBuilder | Keyframe | 'default';
...
type ExternalToast = Omit<ToastProps, 'id' | 'type' | 'title' | 'jsx' | 'promise' | 'variant' | 'index' | 'numberOfToasts' | 'orderedToastIds'> & {
    id?: string | number;
};
...
export type AddToastContextHandler = (data: Omit<ToastProps, 'id' | 'index' | 'numberOfToasts' | 'orderedToastIds'> & {
    id?: string | number;
}) => string | number;
...
export type ToasterContextType = StableToastContextType & DynamicToastContextType;
export declare const toast: ((message: string, data?: ExternalToast) => string | number) & {
    ...
    promise: <T>(promise: Promise<T>, options: Omit<PromiseOptions<T>, 'promise'> & ExternalToast) => string | number;
