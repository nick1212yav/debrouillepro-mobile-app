  ...
  return (
    <View>
      <Text className="text-white/50 text-xs font-bold uppercase tracking-wider mb-3 flex items-center gap-1.5">
        <Flame size={11} className="text-orange-400" /> Activité (14 derniers
        jours)
      </Text>
      <View className="flex gap-1.5 items-end">
        {data.map((d) => {
          ...
          return (
            ...
            >
              <View
                ...
                style={{ height: `${Math.max(6, intensity * 40 + 6)}px`, backgroundColor: color, borderWidth: 0, borderColor: "rgba(99,102,241,0.4)", borderStyle: "solid" }}
                title={`${d.date}: ${d.count} pub${d.count > 1 ? "s" : ""}`}
              />
