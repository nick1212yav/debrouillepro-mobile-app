  ...
  return (
    ...
    >
      {challenge.thumbnailUrl && (
        <View className="relative h-32 overflow-hidden">
          <Image
           ...
           source={{ uri: challenge.thumbnailUrl }} accessibilityLabel={challenge.title}/>
          <View
          ...
          <View className="absolute bottom-2 left-3 flex items-center gap-1.5">
            ...
            {daysLeft > 0 ? (
              <View
                className="flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] text-white"
                style={{ backgroundColor: "rgba(0,0,0,0.6)" }}
              >
                <Clock size={8} /> {daysLeft}<Text>j restants</Text></View>
            ) : (
              <View
                className="px-2 py-0.5 rounded-full text-[10px] text-red-300"
                style={{ backgroundColor: "rgba(239,68,68,0.3)" }}
              >
                <Text>Terminé</Text></View>
            )}
          </View>
        </View>
      )}
      <View className="p-4">
        ...
        <Text className="text-white/50 text-xs leading-relaxed mb-3">
        ...
        <View
          className="flex items-center gap-1.5 mb-4 p-2 rounded-xl"
          style={{ backgroundColor: `${color}14`, borderStyle: "solid" }}
        >
        ...
        <Pressable
          onPress={onJoin}
          disabled={joined || daysLeft <= 0}
          className="w-full py-2.5 rounded-xl font-bold text-sm disabled:opacity-60 text-white"
          style={
            joined
        ...
        >
          {joined ? (
            <Text className="flex items-center justify-center gap-2">
              <Check size={14} /> <Text>Inscrit</Text></Text>
          ) : (
            <Text className="flex items-center justify-center gap-2">
              <Play size={14} fill="white" /> <Text>Participer</Text></Text>
          )}
        </Pressable>
      </View>
    </View>
