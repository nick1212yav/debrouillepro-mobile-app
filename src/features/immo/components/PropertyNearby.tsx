...
export function PropertyNearby({ city }: Props) {
  ...
  return (
    <View className="bg-white/5 rounded-2xl p-4">
      <Text className="text-sm font-medium text-white/70 mb-3">À proximité</Text>
      <View className="gap-2">
        {places.map((place) => (
          <View key={place.label} className="flex items-center gap-2">
            <place.icon size={14} className="text-white/30" />
            <View>
              <Text className="text-xs text-white/60">{place.label}</Text>
              <Text className="text-[10px] text-white/30">
                {place.count} <Text>·</Text>{place.distance}
              </Text>
            </View>
          </View>
        ))}
