        ...
        <View className="flex-1 overflow-y-auto px-5 pb-8 space-y-4">
          <View className="bg-white/5 rounded-2xl p-4 space-y-2">
            <View className="flex justify-between">
              <Text className="text-white/60 text-sm">Prix du bien</Text>
              <Text className="text-white font-bold">
                {price.toLocaleString()} {currency}
              </Text>
            </View>
            <View className="flex justify-between">
              <Text className="text-white/60 text-sm">Apport</Text>
              <Text className="text-white font-bold">
                {downPayment.toLocaleString()} {currency}
              </Text>
            </View>
            <View className="flex justify-between">
              <Text className="text-white/60 text-sm">Montant du prêt</Text>
              <Text className="text-white font-bold">
                {loanAmount.toLocaleString()} {currency}
              </Text>
            </View>
            <View className="flex justify-between border-t border-white/10 pt-2">
              <Text className="text-white/60 text-sm">Mensualité estimée</Text>
          ...
          <View>
            <Text className="text-xs text-white/40">Apport ({currency})</Text>
            <TextInput
             
              min={0}
              max={price}
              value={downPayment}
              onChangeText={(text) => setDownPayment(parseFloat(text))}
              className="w-full mt-1"
            />
            <View className="flex justify-between text-xs text-white/30">
              <Text>0</Text>
          ...
          <View>
            <Text className="text-xs text-white/40">Taux d'intérêt (%)</Text>
            <TextInput
          ...
          <View>
            <Text className="text-xs text-white/40">Durée (années)</Text>
            <TextInput
              ...
              step={5}
              value={years}
              onChangeText={(text) => setYears(parseInt(text))}
              className="w-full mt-1"
            />
            <View className="flex justify-between text-xs text-white/30">
