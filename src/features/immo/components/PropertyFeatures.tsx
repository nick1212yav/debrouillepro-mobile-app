...
export function PropertyFeatures({
  ...
  return (
    <View><Text className="text-sm font-medium text-white/70 mb-2">Caractéristiques
      </Text><View className="gap-2">{features.map((feature, idx) => (
          <View key={idx} className="flex items-center gap-2 bg-white/5 rounded-xl px-3 py-2">
          ...
          <View className="flex flex-wrap gap-1.5 mt-1">
            {amenities.slice(0, 6).map((amenity) => (
              <Text key={amenity} className="px-2.5 py-1 rounded-full text-[10px] font-medium text-white/50 bg-white/5 border border-white/5">
                {amenity}
              </Text>
            ))}
            {amenities.length > 6 && (
