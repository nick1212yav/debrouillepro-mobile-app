...
export function PropertyFavorite({ propertyId, isFavorited, onToggle }: Props) {
  ...
  const handleToggle = async () => {
    if (!propertyId) {
      toast.error("Erreur: identifiant du bien manquant");
      return;
    }
  ...
  return (
    <Pressable onPress={handleToggle} disabled={loading} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium transition-colors cursor-pointer active:scale-95 ${
        isFavorited
