...
export function VoyageTripCard({
  ...
  return (
    <Pressable whileTap={{ scale: 0.98 }} onPress={onClick} className="w-full text-left bg-white/[0.02] border border-white/5 rounded-3xl overflow-hidden transition-all duration-300 flex flex-col">
      {/* Bannière de trajet */}
      <View className="relative h-28 w-full overflow-hidden bg-white/[0.01]">{trip.imageUrl ? (
          <Image className="w-full h-full object-cover opacity-60" source={{ uri: trip.imageUrl }} accessibilityLabel={`${trip.from} → ${trip.to}`} />
        ) : (
