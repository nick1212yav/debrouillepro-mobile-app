    ...
    <View className="space-y-4">
      <View className="space-y-1.5">
        ...
        <View className="relative">
          ...
          <Input
            ...
            type="text"
            value={form.name}
            onChange={(text) => setForm((f) => ({ ...f, name: text }))}
            placeholder="Votre nom"
            className="pl-9 bg-white/5 border-white/10 text-white placeholder:text-white/25"
            disabled={isLoading}
          />
        </View>
        {errors.name && <Text className="text-red-400 text-xs">{errors.name}</Text>}
      </View>
      ...
      <View className="space-y-1.5">
        ...
        <View className="relative">
          ...
          <Input
            ...
            type="email"
            value={form.email}
            onChange={(text) => setForm((f) => ({ ...f, email: text }))}
            placeholder="votre@email.com"
            className="pl-9 bg-white/5 border-white/10 text-white placeholder:text-white/25"
            disabled={isLoading}
          />
        </View>
        {errors.email && <Text className="text-red-400 text-xs">{errors.email}</Text>}
      </View>
      ...
      <View className="space-y-1.5">
        ...
        <View className="relative">
          ...
          <Input
            ...
            type="tel"
            value={form.phone}
            onChange={(text) => setForm((f) => ({ ...f, phone: text }))}
            placeholder="+243 99 123 4567"
            className="pl-9 bg-white/5 border-white/10 text-white placeholder:text-white/25"
            disabled={isLoading}
          />
        </View>
        {errors.phone && <Text className="text-red-400 text-xs">{errors.phone}</Text>}
      </View>
      ...
      <View className="space-y-1.5">
        <Label
          ...
          className="text-white/70 text-xs uppercase tracking-wider"
        >
          <Text>Ville de départ (optionnel)</Text></Label>
        <View className="relative">
          <MapPin
            size={16}
            className="absolute left-3 top-1/2 -translate-y-1/2 text-white/30"
