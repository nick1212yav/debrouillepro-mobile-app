...
export function QRCardModal({
  ...
  const handleCopy = () => {
    void undefined.writeText(profileUrl);
    setCopied(true);
        ...
        >
          ...
          <View className="flex items-center gap-4 mb-4">
            <View>
              ...
              <View className="text-white/50 text-xs mb-1"><Text>Pro Vérifié</Text></View>
          ...
          <View className="flex gap-3 mb-2">
            {MODULE_STATS.map((s) => (
              <View
                ...
                style={{ backgroundColor: `${s.color}15`, borderStyle: "solid" }}
              >
                <View className="font-black text-sm text-white">{s.value}</View>
                <View className="text-[10px] text-white/40">{s.label}</View>
              </View>
            ))}
          </View>
        </View>

        <View className="px-6 py-4 flex flex-col items-center">
          <View
          ...
          <Text className="text-white/40 text-xs text-center mb-1">
            <Text>Scannez pour voir mon profil</Text></Text>
          <Text className="text-white/25 text-[10px] text-center font-mono truncate max-w-full px-2">
            {profileUrl}
          </Text>
        </View>
