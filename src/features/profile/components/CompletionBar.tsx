  ...
  return (
    ...
    >
      <View className="flex items-center justify-between mb-2">
        <Text className="text-sm font-bold text-white">Profil complété</Text>
        <View className="flex items-center gap-2">
          <Text className="font-black text-sm" style={{ color: accentHex }}>
            {pct}%
          </Text>
          <ChevronRight
            size={14}
      ...
      {pct < 100 && (
        <Text className="text-xs text-white/40">
          {steps.filter((s) => !s.done).length} étape
      ...
      <>
        {expanded && (
          ...
          >
            <View className="flex flex-col gap-2 pt-1 border-t border-white/5">
              {steps.map((step) => (
                <View key={step.id} className="flex items-center gap-3">
                  <View
                    className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0"
                    style={{ backgroundColor: step.done
                                            ? `${accentHex}25`
                                            : "rgba(255,255,255,0.06)", borderColor: "rgba(255,255,255,0.1)", borderStyle: "solid" }}
                  >
                    {step.done ? (
                  ...
                  {!step.done && (
                    <Pressable
                      ...
                      onPress={(e) => {
                        UIService.openToast("Modifiable via Modifier le profil", "info");
                      }}
                      style={{ backgroundColor: `${accentHex}20` }}
                    >
                      <Text>Compléter</Text></Pressable>
                  )}
                </View>
              ))}
            </View>
