...
export function EventReviews({ reviews, onLikeReview, onAddReview }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setRating(0);
      setComment("");
      UIService.openToast("Avis ajouté", "success");
    } catch {
    ...
    <View className="space-y-4">
      <View className="flex items-center gap-4">
        <View>
          ...
          <Text className="text-white/30 text-xs">{reviews.length} avis</Text>
        </View>
      </View>

      {/* Ajouter un avis */}
      <View className="rounded-2xl p-4 bg-white/5 border border-white/5">
        <Text className="text-white/60 text-xs mb-2">Votre avis</Text>
        ...
        <Pressable
          ...
          className="mt-2 px-4 py-1.5 rounded-xl text-xs font-bold text-white disabled:opacity-40"
          style={{  }}
        >
      ...
      >
        {reviews.map((review) => (
          <View
            key={review.id}
            className="rounded-2xl p-3 bg-white/5 border border-white/5"
          >
            <View className="flex items-center gap-2 mb-1">
              {review.authorAvatar ? (
                <Image
                 ...
                 source={{ uri: review.authorAvatar }} accessibilityLabel=""/>
              ) : (
              ...
              <View className="flex gap-0.5 ml-auto">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star
                    key={i}
                    size={10}
            ...
            <Pressable
              ...
              className="flex items-center gap-1 text-xs text-white/30 mt-1"
            >
              <ThumbsUp
                size={12}
                className={
