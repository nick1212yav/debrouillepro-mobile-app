...
export function TicketScanner({ onScan }: Props) {
  ...
  return (
    <View className="rounded-2xl p-5 text-center bg-white/5 border border-white/5">
      <View className="flex items-center justify-between mb-4">
        <Text className="text-white font-bold text-sm"><Text>Scanner un billet</Text></Text>
      ...
      <Pressable
        onPress={handleScan}
        disabled={scanning}
        className="w-full py-3 rounded-xl flex items-center justify-center gap-2 text-white font-medium disabled:opacity-50"
        style={{  }}
      >
        <Camera size={16} />
      ...
      {result && (
        <View
          ...
          style={{ backgroundColor: result.valid
                        ? "rgba(16,185,129,0.1)"
                        : "rgba(239,68,68,0.1)" }}
        >
          {result.valid ? (
