...
export function RewardPoints({ points, level, nextLevelPoints }: Props) {
  ...
  return (
    <View
      ...
      style={{ backgroundColor: "rgba(255,215,0,0.08)", borderWidth: 1, borderColor: "rgba(255,215,0,0.12)", borderStyle: "solid" }}
    >
      <View className="flex items-center justify-between mb-3">
        ...
        <View className="flex items-center gap-1">
          ...
          <Text className="text-white font-bold text-lg">{points}</Text>
          <Text className="text-white/30 text-xs">pts</Text>
        </View>
      </View>
      {nextLevelPoints && (
        <View className="space-y-1">
          <View className="flex justify-between text-[10px] text-white/40">
            <Text>{points} pts</Text>
            <Text>{nextLevelPoints} pts</Text>
          </View>
          ...
          <Text className="text-[10px] text-white/30 text-right">
            {Math.round(progress)}<Text>% vers le niveau supérieur</Text></Text>
        </View>
      )}
    </View>
  );
