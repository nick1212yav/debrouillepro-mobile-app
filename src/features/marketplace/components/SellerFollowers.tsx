  ...
  const handleToggle = async () => {
    ...
    try {
      ...
      UIService.openToast(following
          ...
          : "Vous suivez maintenant ce vendeur", "success");
    } catch {
    ...
    <View className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/5">
      <View className="flex items-center gap-2">
        ...
        <Text className="text-white font-medium text-sm">
          {formatCompactNumber(count)} <Text>abonnés</Text></Text>
      </View>
      <Pressable
        ...
        style={{ backgroundColor: following
                    ? "rgba(239,68,68,0.1)"
                    : "rgba(99,102,241,0.15)", borderColor: "rgba(239,68,68,0.2)", borderStyle: "solid" }}
      >
