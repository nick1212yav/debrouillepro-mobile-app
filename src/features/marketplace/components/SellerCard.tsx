      ...
      <View className="flex items-center gap-3">
        {seller.avatar ? (
          <Image
           ...
           source={{ uri: seller.avatar }} accessibilityLabel={seller.name}/>
        ) : (
          <View className="w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold text-white bg-orange-500">
            {seller.name[0]}
          </View>
        )}
        <View className="flex-1 min-w-0">
          ...
          <View className="flex items-center gap-3 text-xs text-white/40">
            ...
            <Text>{formatCompactNumber(seller.totalSales)} <Text>ventes</Text></Text>
            <Text><Text>Membre depuis</Text>{formatDate(seller.joinDate)}</Text>
          </View>
        </View>
        {onContact && (
          <Pressable
            ...
            style={{  }}
          >
            <MessageCircle size={12} /> <Text>Contacter</Text></Pressable>
        )}
      </View>
    </View>
  );
