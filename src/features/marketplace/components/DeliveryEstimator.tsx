...
export function DeliveryEstimator({ productId, location }: Props) {
  ...
  const handleEstimate = async () => {
    if (!address.trim()) {
      UIService.openToast("Veuillez saisir une adresse", "error");
      return;
    }
    setEstimating(true);
    try {
      ...
      UIService.openToast("Erreur lors de l'estimation", "error");
    } finally {
      setEstimating(false);
    }
  };
    ...
    <View className="space-y-3">
      ...
      <View className="flex items-center gap-2">
        ...
        <TextInput
          ...
          className="flex-1 px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-white text-sm outline-none placeholder-white/25"
        />
        <Pressable
          ...
          <View className="flex items-center justify-between">
            <View>
              ...
              <Text className="text-white/40 text-xs">
                Livraison sous {estimate.days} jours
              </Text>
            </View>
            <View className="text-right">
              ...
              <Text className="text-[10px] text-white/30">
                <Text>Frais de livraison</Text></Text>
            </View>
          </View>
        </View>
      )}
