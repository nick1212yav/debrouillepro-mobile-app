            ...
            >
              {customer.avatar ? (
                <Image
                 ...
                 source={{ uri: customer.avatar }} accessibilityLabel={customer.name}/>
              ) : (
                <View className="w-10 h-10 rounded-full flex items-center justify-center bg-purple-500/20 text-purple-400">
                  <User size={16} />
                </View>
              )}
              <View className="flex-1 min-w-0">
                ...
                <View className="flex items-center gap-3 text-xs text-white/40">
                  ...
                  <Text>{customer.orders} commandes</Text>
                  <Text className="flex items-center gap-0.5">
                    <Calendar size={10} /> {formatDate(customer.joinedAt)}
                  </Text>
                </View>
              </View>
              <View className="text-right">
                ...
                <Text className="text-white/30 text-[10px]"><Text>dépensé</Text></Text>
              </View>
            </View>
          ))
        )}
