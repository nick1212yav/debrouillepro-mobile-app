...
/**
 * Données réellement consommées par ProductCard.
 *
 * ProductCard n'utilise pas l'identifiant Convex `_id`.
 * Le rendre optionnel permet au composant d'être utilisé aussi bien
 * avec un Product complet qu'avec les données adaptées depuis une
 * publication marketplace référencée par `meta.productId`.
 */
export type ProductCardData = Omit<Product, "_id"> & {
  _id?: string;
};
...
export function ProductCard({
  ...
  const handleLike = (e: GestureResponderEvent) => {
    ...
    setLikeCount((previous) => (isLiked ? previous - 1 : previous + 1));
  ...
  const renderStockBadge = () => {
    if (!isAvailable) {
      return (
        <Text className="px-2.5 py-1 rounded-lg text-[10px] font-bold bg-red-500/90 text-white backdrop-blur-sm border border-red-400/30">
          Épuisé
        </Text>
      );
    }
    ...
    if (stockStatus === "low_stock") {
      return (
        <Text className="px-2.5 py-1 rounded-lg text-[10px] font-bold bg-amber-500/90 text-white backdrop-blur-sm border border-amber-400/30 animate-pulse">
          Stock limité
        </Text>
      );
    }
    ...
    return (
      <Text className="px-2.5 py-1 rounded-lg text-[10px] font-bold bg-emerald-500/90 text-white backdrop-blur-sm border border-emerald-400/30">
        En stock
      </Text>
    );
  };
  ...
  return (
    ...
    >
      ...
      <View className="relative h-52 overflow-hidden bg-black/20">
        ...
        <Pressable
          onPress={handleLike}
          className="absolute top-3 right-3 w-9 h-9 rounded-full flex items-center justify-center backdrop-blur-md transition-all active:scale-95"
          style={{
          ...
          >
            {product.sellerAvatar ? (
              <Image
                ...
                accessibilityLabel={product.sellerName}
              />
            ) : (
              <View
          ...
          <View className="flex items-center gap-2">
            ...
            {onAddToCart && isAvailable && (
              <Pressable
                ...
                className="p-2 rounded-xl bg-orange-500/20 transition-all"
                style={{
                  borderWidth: 1,
