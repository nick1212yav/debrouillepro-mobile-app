...
export function MarketplaceCategories({ categories, active, onSelect }: Props) {
  return (
    <View className="flex gap-2 overflow-x-auto pb-1" style={{  }}>
      {categories.map((cat) => (
        <Pressable key={cat.id} onPress={() => onSelect(cat.id)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold flex-shrink-0 transition-all" style={{ backgroundColor: active === cat.id
