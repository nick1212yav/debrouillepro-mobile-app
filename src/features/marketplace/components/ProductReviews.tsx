    ...
    <View className="space-y-4">
      ...
      {averageRating !== undefined && (
        <View className="flex items-center gap-2">
          ...
          <Text className="text-white/40 text-sm">({reviews.length} avis)</Text>
        </View>
      )}
      ...
      {reviews.length === 0 ? (
        ...
        reviews.map((review) => (
          <View key={review._id} className="bg-white/5 p-3 rounded-xl space-y-1">
            <View className="flex items-center justify-between">
              <View className="flex items-center gap-2">
                <View className="w-6 h-6 rounded-full bg-white/10 flex items-center justify-center">
              ...
              {onLikeReview && (
                <Pressable
                  onPress={() => onLikeReview(review._id)} // review._id est une chaîne
      ...
      {onAddReview && (
        <View className="space-y-3">
          ...
          <TextInput
            ...
            className="w-full p-2 rounded-lg bg-white/5 border border-white/10 text-white placeholder-white/40"
