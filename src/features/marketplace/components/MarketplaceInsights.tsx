  ...
  return (
    <View className="space-y-2">
      ...
      {insights.map((insight) => (
        ...
        >
          <Text className="text-white/70 text-sm">{insight.label}</Text>
          <View className="flex items-center gap-2">
            ...
            {insight.change > 0 ? (
              <Text className="flex items-center gap-0.5 text-emerald-400 text-xs">
                <TrendingUp size={12} /> <Text>+</Text>{insight.change}<Text>%</Text></Text>
            ) : insight.change < 0 ? (
              <Text className="flex items-center gap-0.5 text-red-400 text-xs">
                <TrendingDown size={12} /> {insight.change}<Text>%</Text></Text>
            ) : (
              <Text className="text-white/30 text-xs">
                <Minus size={12} /> <Text>0%</Text></Text>
            )}
          </View>
        </View>
      ))}
