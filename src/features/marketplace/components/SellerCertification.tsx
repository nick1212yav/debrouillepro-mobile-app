      ...
      <View className="flex flex-wrap gap-2">
        {verified && (
          <Text className="flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-medium bg-green-500/20 text-green-400 border border-green-500/20">
            <CheckCircle size={12} /> <Text>Vendeur vérifié</Text></Text>
        )}
        {certifications.map((cert) => (
          <Text
            key={cert}
            className="flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-medium bg-purple-500/20 text-purple-400 border border-purple-500/20"
          >
            <Award size={12} /> {cert}
          </Text>
        ))}
