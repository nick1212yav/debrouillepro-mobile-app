...
export function MobileMoneyPayment({ amount, currency, onConfirm }: Props) {
  ...
  const handleConfirm = async () => {
    if (!provider || !phone) {
      UIService.openToast("Veuillez choisir un opérateur et saisir un numéro", "error");
      return;
    }
    setLoading(true);
    try {
  ...
  if (done) {
    return (
      <View className="text-center py-6">
        ...
        <Text className="text-white font-bold text-lg">Paiement initié</Text>
  ...
  return (
    <View className="space-y-4">
      <View className="flex gap-2 flex-wrap">
        {PROVIDERS.map((p) => (
          <Pressable
            ...
            className="flex-1 min-w-[80px] p-3 rounded-xl text-center"
            style={{ backgroundColor: provider === p.id ? `${p.color}22` : "rgba(255,255,255,0.05)", borderColor: "rgba(255,255,255,0.08)", borderStyle: "solid" }}
          >
            <Smartphone
              size={16}
              style={{ color: p.color }}
      ...
      <Pressable
        ...
        disabled={loading || !provider || !phone}
