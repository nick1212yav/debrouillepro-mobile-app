    ...
    <View className="p-4 rounded-2xl bg-white/5 border border-white/10">
      <View className="flex items-center justify-between mb-4">
        <Text className="text-xs text-white/40 font-semibold uppercase tracking-wider flex items-center gap-2">
          <MessageCircle size={14} /> Questions ({questions.length})
        </Text>
        {onAsk && (
          <Pressable
      ...
      {showAskForm && onAsk && (
        <View className="mb-4 p-3 rounded-xl bg-white/5 border border-white/10">
          <TextInput
            value={newQuestion}
            onChangeText={(text) => setNewQuestion(text)}
            placeholder="Votre question..."
            className="w-full p-2 rounded-xl bg-white/10 border border-white/10 text-white text-sm placeholder:text-white/30"
           
           multiline textAlignVertical="top"/>
          <Pressable
            onPress={handleAsk}
            disabled={!newQuestion.trim()}
            className="mt-2 px-4 py-1.5 rounded-xl text-xs font-bold text-white bg-red-500 disabled:opacity-50"
          >
            ...
            >
              <View className="flex items-start justify-between">
                <View>
                  ...
                  <Text className="text-white/70 text-sm mt-0.5">{q.question}</Text>
                </View>
                <Text className="text-[10px] text-white/30">
                  {q.date.toLocaleDateString("fr-FR")}
                </Text>
              </View>

              <View className="flex items-center gap-3 mt-1 text-xs">
                {onLikeQuestion && (
                  <Pressable
                    onPress={() => onLikeQuestion(q.id)}
                ...
                >
                  <Text>Répondre (</Text>{q.answers.length}<Text>)</Text></Pressable>
              </View>
                ...
                <View className="mt-2 pl-3 border-l-2 border-white/10 space-y-2">
                  {q.answers.map((a) => (
                    <View key={a.id} className="text-xs">
                      ...
                      <Text className="text-white/60 ml-1"><Text>:</Text>{a.content}</Text>
                    </View>
                  ))}
                </View>
              )}
              ...
              {showAnswerForm[q.id] && onAnswer && (
                <View className="mt-2 flex gap-2">
                  <TextInput
                    ...
                    className="flex-1 p-1.5 rounded-lg bg-white/10 border border-white/10 text-white text-xs placeholder:text-white/30"
                  />
                  <Pressable
