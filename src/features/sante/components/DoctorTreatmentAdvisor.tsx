  ...
  return (
    <View className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20">
      <View className="flex items-center gap-2 mb-3">
        ...
        <Text className="text-xs text-emerald-400 font-semibold uppercase tracking-wider">
          Conseiller traitement
        </Text>
      </View>
      ...
      <View className="flex gap-2">
        ...
        <Pressable
          onPress={handleAsk}
          disabled={!condition.trim() || loading || !onAsk}
      ...
      {loading && (
        <View className="mt-3 text-emerald-400 text-xs flex items-center gap-2">
          <Loader2 size={12} className="animate-spin" />
          <Text>Recherche des recommandations...</Text></View>
      )}

      {result && !loading && (
        <View className="mt-3 p-3 rounded-xl bg-white/5 border border-white/10">
          <Text className="text-white font-bold text-sm">{result.name}</Text>
          <Text className="text-white/60 text-xs mt-1">{result.description}</Text>
          <View className="mt-2 space-y-1">
            <Text className="text-white/40 text-xs font-medium">
              <Text>Recommandations :</Text></Text>
            {result.recommendations.map((rec, idx) => (
              <View
                key={idx}
                className="flex items-start gap-1.5 text-xs text-white/70"
              >
                <Text className="text-emerald-400"><Text>•</Text></Text>
                {rec}
              </View>
            ))}
          </View>
          <Text className="text-white/40 text-xs mt-2"><Text>📅</Text>{result.followUp}</Text>
        </View>
      )}
    </View>
  );
