...
export function DoctorArticles({
  ...
  return (
    <View className="p-4 rounded-2xl bg-white/5 border border-white/10">
      <Text className="text-xs text-white/40 font-semibold uppercase tracking-wider flex items-center gap-2 mb-3">
        <FileText size={14} /> Articles ({articles.length})
      </Text>
      <View
            ...
            <View className="flex gap-3">
              {article.image && (
                <Image
                 ...
                 source={{ uri: article.image }} accessibilityLabel={article.title}/>
              )}
              <View className="flex-1 min-w-0">
                ...
                <View className="flex items-center gap-3 mt-1 text-[10px] text-white/30">
                  ...
                  <Text className="flex items-center gap-1">
                    <Clock size={10} /> {article.readTime} <Text>min</Text></Text>
                </View>
              </View>
            </View>
          </Pressable>
