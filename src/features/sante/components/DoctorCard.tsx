    ...
    >
      <View className="flex items-start gap-3">
        ...
        <View className="flex-1 min-w-0">
          ...
          <Text className="text-xs text-red-400 font-medium capitalize">
            {doctor.specialty}
          </Text>
        ...
        <View className="text-right flex-shrink-0">
          ...
          <Text className="text-[10px] text-green-400 font-medium mt-0.5">
            {slots.length > 0 ? "Disponible" : "Indisponible"}
          </Text>
        </View>
      </View>
      {slots.length > 0 && (
        <View className="flex gap-1.5 mt-3 flex-wrap">
          {slots.slice(0, 3).map((slot) => (
            <Pressable
              ...
              style={{ backgroundColor: "rgba(239,68,68,0.12)", borderWidth: 1, borderColor: "rgba(239,68,68,0.25)", borderStyle: "solid" }}
            >
              <Clock size={9} /> {slot}
            </Pressable>
          ))}
      ...
      <View className="flex gap-2 mt-3">
        <Pressable
          ...
          className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-2xl text-xs font-bold text-white"
          style={{  }}
        >
          <Calendar size={13} /> <Text>Prendre RDV</Text></Pressable>
        <Pressable
          onPress={(e) => e.stopPropagation()}
          className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-2xl text-xs"
          style={{ backgroundColor: "rgba(59,130,246,0.18)", borderWidth: 1, borderColor: "rgba(59,130,246,0.3)", borderStyle: "solid" }}
        >
