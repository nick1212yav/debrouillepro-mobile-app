  ...
  return (
    <View className="p-4 rounded-2xl bg-white/5 border border-white/10">
      ...
      <View className="space-y-2">
        {education.map((item) => (
          <View key={item.id} className="flex justify-between items-start">
            <View>
              <Text className="text-white text-sm font-medium">{item.degree}</Text>
              <Text className="text-white/40 text-xs">{item.institution}</Text>
            </View>
            {item.year && (
