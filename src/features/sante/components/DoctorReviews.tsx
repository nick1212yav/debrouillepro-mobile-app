    ...
    <View className="p-4 rounded-2xl bg-white/5 border border-white/10">
      <View className="flex items-center justify-between mb-4">
        <View className="flex items-center gap-2">
          ...
          <View className="flex items-center gap-0.5 text-yellow-400 text-sm">
        ...
        {reviews.length === 0 ? (
          ...
          reviews.map((review) => (
            ...
            >
              <View className="flex items-start justify-between">
                <View>
                  <Text className="text-white text-sm font-medium">
                    {review.patientName}
                    {review.verified && (
                      <Text className="ml-1 text-[10px] text-blue-400">
                        ✓ vérifié
                      </Text>
                    )}
                  </Text>
                  <View className="flex items-center gap-1 text-yellow-400 text-xs">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        size={12}
              ...
              <Text className="text-white/70 text-sm mt-1">{review.comment}</Text>
              {onLikeReview && (
                <Pressable
                  onPress={() => onLikeReview(review.id)}
                  className="mt-1 text-xs text-white/30 flex items-center gap-1"
                >
