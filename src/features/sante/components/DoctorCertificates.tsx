    ...
    <View className="p-4 rounded-2xl bg-white/5 border border-white/10">
      ...
      <View className="space-y-2">
        {certificates.map((cert) => (
          <View key={cert.id} className="flex justify-between items-start">
            <View>
              <Text className="text-white text-sm font-medium">{cert.name}</Text>
              <Text className="text-white/40 text-xs">{cert.issuer}</Text>
            </View>
            {cert.year && (
