...
export const EditAccommodationSheet: React.FC<EditAccommodationSheetProps> = ({
  ...
  const handleSubmit = async (e: unknown) => {
    ...
    try {
      ...
      UIService.openToast("Mise à jour effectuée !", "success");
      onClose();
    } catch {
  ...
  return (
    <View className="fixed inset-0 z-50 flex items-end justify-center bg-black/70">
      ...
      >
        ...
        <View className="flex flex-col gap-4 text-xs">
          <View>
            <Text className="text-white/40 font-bold block mb-1 uppercase tracking-wider text-[10px]">
              Titre de l'annonce
            </Text>
            <TextInput
              required
          ...
          <View>
            ...
            <TextInput
              ...
              className="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-white" keyboardType="numeric"
            />
          </View>
