    ...
    >
      <View className="flex items-center justify-between pb-2 border-b border-white/5">
        <Text className="text-xs font-bold text-white/40 uppercase tracking-wider flex items-center gap-1.5">
          ...
          <Text>Vidéo de Présentation</Text>
        </Text>
        <Pressable
         
          onPress={() => Alert.alert("Plein écran vidéo")}
      ...
      <View className="relative aspect-video w-full rounded-xl overflow-hidden border border-white/5 bg-slate-950 flex items-center justify-center group">
        <Image
         
         
          className="w-full h-full object-cover opacity-80"
         source={{ uri: videoUrl }} accessibilityLabel="Video Thumbnail Poster"/>

        <View className="absolute inset-0 flex items-center justify-center bg-black/30">
          <Pressable
           
            onPress={() => setPlaying(!playing)}
            className="w-12 h-12 rounded-full bg-indigo-500 border border-white/10 flex items-center justify-center text-white shadow-xl shadow-indigo-500/20"
          >
            {playing ? (
        ...
        <View className="absolute bottom-2 left-2 right-2 p-1.5 bg-black/60 rounded-lg border border-white/5 flex items-center justify-between opacity-0">
          <View className="flex items-center gap-2">
            ...
            <Text className="text-[9px] text-white/60 font-semibold uppercase">
              {playing ? "Simulation lecture" : "En pause"}
            </Text>
          </View>
