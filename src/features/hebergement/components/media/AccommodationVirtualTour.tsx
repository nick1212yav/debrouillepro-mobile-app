...
> = ({ className = "" }) => {
  ...
  return (
    ...
    >
      <View className="flex items-center justify-between pb-2 border-b border-white/5">
        <Text className="text-xs font-bold text-white/40 uppercase tracking-wider flex items-center gap-1.5">
          ...
          <Text>Visite Virtuelle Interactive</Text>
        </Text>
        <Text className="text-[9px] font-bold text-indigo-400 bg-indigo-500/10 px-1.5 py-0.5 rounded-lg border border-indigo-500/20">
          {activeStep + 1} / {steps.length}
        </Text>
      </View>

      <View className="relative aspect-video w-full rounded-xl overflow-hidden border border-white/5 bg-slate-950 flex items-center justify-center">
        <Image
         
         
          className="w-full h-full object-cover"
         source={{ uri: steps[activeStep].img }} accessibilityLabel={steps[activeStep].name}/>
        <View className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent" />

        <Pressable
         
          onPress={handlePrev}
          className="absolute left-2.5 w-8 h-8 rounded-full bg-black/60 border border-white/10 flex items-center justify-center text-white"
        >
          <ChevronLeft size={16} />
        </Pressable>
        ...
        <Pressable
          ...
          onPress={handleNext}
          className="absolute right-2.5 w-8 h-8 rounded-full bg-black/60 border border-white/10 flex items-center justify-center text-white"
        >
          <ChevronRight size={16} />
        </Pressable>

        <View className="absolute bottom-3 left-3 right-3 flex flex-col gap-0.5">
          <Text className="text-xs font-bold text-white">
            {steps[activeStep].name}
          </Text>
          <Text className="text-[10px] text-white/60 leading-relaxed">
            {steps[activeStep].desc}
          </Text>
        </View>
      </View>

      <View className="flex justify-center gap-1.5 py-1">
        {steps.map((_, i) => (
          <Pressable
            key={i}
