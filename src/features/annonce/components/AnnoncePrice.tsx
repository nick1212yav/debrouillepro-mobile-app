...
export function AnnoncePrice({ annonce, showOriginalPrice }: Props) {
  ...
  return (
    <View className="space-y-1"><View className="flex items-baseline gap-3"><Text className="text-2xl font-black text-white">{formatPrice(price, annonce.currency)}</Text>{annonce.negotiable && (
          <Text className="text-xs text-white/40 font-medium">Négociable</Text>
        )}</View>{isDiscounted && minPrice && (
        <View className="flex items-center gap-2">
          ...
          <Text className="text-xs text-emerald-400 font-medium">
            -{Math.round(((price - minPrice) / price) * 100)}%
          </Text>
        </View>
      )}{annonce.isSold && (
        <View className="text-xs text-red-400 font-medium">⚠️ Déjà vendu</View>
