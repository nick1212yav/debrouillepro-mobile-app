...
export function AnnonceAIDetector({ images, description, onAnalyze }: Props) {
      ...
      >
        ...
        <View className="flex-1">
          <Text className="text-sm font-medium text-white">
            {result.isFraud ? "⚠️ Alerte de sécurité" : "✅ Annonce sécurisée"}
          </Text>
          <Text className="text-xs text-white/50 mt-1">
            <Text>Score de confiance:</Text>{Math.round(result.score * 100)}<Text>%</Text></Text>
  ...
  return (
    <Pressable
      ...
      disabled={loading}
      className="w-full flex items-center justify-center gap-2 py-2 rounded-xl text-xs font-medium text-white/60 bg-white/5 disabled:opacity-50"
    >
      {loading ? (
