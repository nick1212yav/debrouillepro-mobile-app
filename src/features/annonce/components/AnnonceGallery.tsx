...
export function AnnonceGallery({ images, title, video }: Props) {
    ...
    return (
      <>
        ...
        <Image
          ...
          className={`${className} ${isLoadingImg ? "opacity-0" : "opacity-100"} transition-opacity duration-300`}
          loading="lazy"
          onLoad={() => handleImageLoad(imgIndex)}
          onError={() => handleImageError(imgIndex)}
          onPress={() => setIsFullscreen(true)}
         source={{ uri: src }} accessibilityLabel={title}/>
      </>
    );
  };
  ...
  return (
    <>
      <View className="relative rounded-2xl overflow-hidden aspect-[4/3] bg-black/20">
        <View className="relative w-full h-full">
          {renderMedia("w-full h-full object-cover cursor-pointer")}
        </View>
        ...
        {totalMedia > 1 && (
          <>
            ...
            <View className="absolute top-2 left-2 bg-black/50 text-white text-xs px-2 py-1 rounded-full z-10">
              {currentIndex + 1} <Text>/</Text>{totalMedia}
            </View>
