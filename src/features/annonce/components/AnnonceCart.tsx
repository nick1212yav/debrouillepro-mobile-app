...
export function AnnonceCart({
  ...
  if (items.length === 0) {
    return (
      <View className="p-6 text-center">
        ...
        <Text className="text-white/40 text-sm">Votre panier est vide</Text>
      </View>
    );
  }
  ...
  return (
    <View className="space-y-4">
      ...
      {/* Liste des articles */}
      <View className="space-y-3 max-h-64 overflow-y-auto pr-1">
        {items.map((item) => (
          <View
            key={item.id}
          ...
          >
            {item.image ? (
              ...
              <View className="w-14 h-14 rounded-lg bg-white/5 flex items-center justify-center flex-shrink-0">
                <ShoppingCart size={20} className="text-white/20" />
              </View>
            )}
            <View className="flex-1 min-w-0">
              <Text className="text-white text-sm truncate">{item.title}</Text>
            ...
            <View className="flex items-center gap-1">
              ...
              <Text className="text-white text-sm w-6 text-center">
                {item.quantity}
              </Text>
              <Pressable
              ...
              <Pressable
                ...
                className="w-6 h-6 rounded-lg text-red-400/50"
              >
                <Trash2 size={12} />
              </Pressable>
            </View>
      ...
      <View className="pt-3 border-t border-white/10">
        <View className="flex items-center justify-between mb-3">
          <Text className="text-white/50 text-sm"><Text>Total</Text></Text>
        ...
        <Pressable
          ...
          style={{  }}
        >
          <ArrowRight size={16} /> <Text>Procéder au paiement</Text></Pressable>
      </View>
    </View>
  );
}
