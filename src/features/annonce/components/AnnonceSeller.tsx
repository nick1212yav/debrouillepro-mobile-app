...
export function AnnonceSeller({
  ...
  const handleCall = () => {
    if (ownerPhone) undefined.href = `tel:${ownerPhone}`;
  };
    ...
    <View className="bg-white/5 rounded-2xl p-4 space-y-3">
      ...
      <View className="flex items-center gap-3">
        {ownerAvatar ? (
          <Image
           ...
           source={{ uri: ownerAvatar }} accessibilityLabel={ownerName}/>
        ) : (
        ...
        <View className="flex-1 min-w-0">
          <View className="flex items-center gap-2">
            <Text className="text-white font-medium truncate">{ownerName}</Text>
            {isVerified && (
      ...
      <View className="flex flex-wrap gap-2 pt-1">
        {ownerId && (
          ...
          >
            <Mail size={14} /> <Text>Contacter</Text></Pressable>
        )}
        ...
        {ownerPhone && (
          <>
            ...
            >
              <MessageCircle size={14} /> <Text>WhatsApp</Text></Pressable>
          </>
        )}
      </View>
    </View>
