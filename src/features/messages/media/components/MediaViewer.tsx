  ...
  return (
    ...
    >
      <Pressable
        ...
        onPress={onClose}
        className="absolute right-5 top-5 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-xl text-white"
      ...
      <View className="max-h-full max-w-full">
        {isImage ? (
          <ImagePreview
            src={item.url}
            alt={item.name ?? ""}
          ...
          <Pressable
            className="text-white underline" accessibilityHint={item.url}
          >
            <Text>Ouvrir le fichier</Text></Pressable>
        )}
      </View>
