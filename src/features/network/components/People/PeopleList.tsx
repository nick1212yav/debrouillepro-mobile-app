...
export function PeopleList({
  ...
  if (isLoading) {
    return (
      <View className={cn("space-y-3", className)}>{Array.from({ length: 4 }).map((_, i) => (
          <View key={i} className="flex items-center gap-3 p-4 rounded-2xl bg-white/5 border border-white/8"><Skeleton className="w-14 h-14 rounded-full flex-shrink-0" /><View className="flex-1 space-y-2"><Skeleton className="h-4 w-32 rounded-lg" /><Skeleton className="h-3 w-24 rounded-lg" /><Skeleton className="h-3 w-40 rounded-lg" /></View><Skeleton className="h-8 w-20 rounded-xl flex-shrink-0" /></View>
        ))}</View>
    );
  }
...
<View>
      ...
      {maxDisplay && people.length > maxDisplay && (
        <Pressable className="w-full py-2 text-xs text-indigo-400 transition-colors" onPress={() => {}}>
