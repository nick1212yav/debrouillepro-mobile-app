...
export function ProfileCover({
  ...
  if (isLoading) {
    return (
      <View className={cn("relative rounded-[28px] overflow-hidden", className)} style={{ height }}>
  ...
  return (
    <View className={cn(
      ...
      {/* Image de couverture */}
      {cover && (
        <Image className="w-full h-full object-cover" source={{ uri: cover }} accessibilityLabel="Couverture de profil" />
      )}

      {/* Bouton d'édition */}
      {editable && (
        <Pressable onPress={onEdit} className="absolute bottom-3 right-3 px-3 py-1.5 rounded-xl flex items-center gap-1.5 text-xs font-medium text-white bg-black/50 backdrop-blur-sm border border-white/10 transition-colors">
          <Camera size={14} />
          Changer
        </Pressable>
      )}
