...
function EducationItem({
  ...
  return (
    <View className="relative pl-5 pb-5 last:pb-0 border-l-2 border-emerald-500/20 last:border-0">
      <View className="absolute -left-[7px] top-1.5 w-3 h-3 rounded-full bg-emerald-500/30 border-2 border-emerald-500/50" />
        ...
        <View className="flex items-start justify-between gap-2">
          ...
          {editable && (
            <View className="flex items-center gap-1 flex-shrink-0">
              <Pressable
                onPress={() => onEdit?.(education._id)}
                className="p-1 rounded-lg"
              >
                <Edit3 size={12} className="text-white/40" />
              </Pressable>
              <Pressable
                onPress={() => onDelete?.(education._id)}
                className="p-1 rounded-lg"
              >
    ...
    >
      <View className="flex items-center justify-between mb-4">
        <View className="flex items-center gap-2">
          ...
          <Text className="text-white/30 text-xs"><Text>(</Text>{educations.length}<Text>)</Text></Text>
        </View>
        {editable && (
          <Pressable
            onPress={onAdd}
