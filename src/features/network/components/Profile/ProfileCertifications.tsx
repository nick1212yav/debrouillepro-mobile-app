...
function CertificationItem({
  ...
  return (
    ...
    >
      <View className="w-10 h-10 rounded-xl flex items-center justify-center bg-amber-500/10 text-amber-400 flex-shrink-0">
        <Award size={18} />
      </View>
      <View className="flex-1 min-w-0">
        ...
        <Text className="text-white/40 text-xs">{certification.issuer}</Text>
        <View className="flex items-center gap-2 mt-0.5 text-white/30 text-[10px]">
          <Text>Délivrée le {issueDate}</Text>
          ...
          {certification.credentialUrl && (
            <Pressable
              ...
              className="text-indigo-400 flex items-center gap-0.5"
              onPress={(e) => e.stopPropagation()}
             data-href={certification.credentialUrl}>
              <ExternalLink size={10} />
              <Text>Vérifier</Text></Pressable>
          )}
        </View>
      </View>
      {editable && (
        <View className="flex items-center gap-1 flex-shrink-0">
          <Pressable
            onPress={() => onEdit?.(certification._id)}
            className="p-1.5 rounded-lg"
          >
            <Edit3 size={13} className="text-white/40" />
          </Pressable>
          <Pressable
            onPress={() => onDelete?.(certification._id)}
            className="p-1.5 rounded-lg"
          >
  ...
  return (
    ...
    >
      <View className="flex items-center justify-between mb-4">
        <View className="flex items-center gap-2">
          <View className="w-8 h-8 rounded-xl flex items-center justify-center bg-amber-500/10 text-amber-400">
            <Award size={15} />
          </View>
          <Text className="text-white font-bold text-sm">Certifications</Text>
          <Text className="text-white/30 text-xs">
            <Text>(</Text>{certifications.length}<Text>)</Text></Text>
        </View>
        {editable && (
          <Pressable
            onPress={onAdd}
