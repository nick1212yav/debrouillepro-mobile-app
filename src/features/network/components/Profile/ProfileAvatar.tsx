...
export function ProfileAvatar({
  ...
  return (
    <View className={cn("relative flex-shrink-0", className)}>
      <View
        className="relative rounded-full overflow-hidden"
        style={{ width: pixelSize, height: pixelSize, borderColor: "rgba(255,255,255,0.08)", borderStyle: "solid" }}
      >
        {avatar ? (
          <Image
           ...
           source={{ uri: avatar }} accessibilityLabel={name ?? "Avatar"}/>
        ) : (
          <View
      ...
      {verified && (
        <View
          className="absolute -bottom-0.5 -right-0.5 rounded-full bg-emerald-500 flex items-center justify-center border-2 border-[#020617]"
          style={{
            width: badgeSize,
            height: badgeSize,
          }}
