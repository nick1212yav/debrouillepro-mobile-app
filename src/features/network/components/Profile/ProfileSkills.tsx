  ...
  return (
    ...
    >
      <View className="flex items-center justify-between mb-4">
        <View className="flex items-center gap-2">
          <View className="w-8 h-8 rounded-xl flex items-center justify-center bg-purple-500/10 text-purple-400">
            <Wrench size={15} />
          </View>
          <Text className="text-white font-bold text-sm">Compétences</Text>
          <Text className="text-white/30 text-xs"><Text>(</Text>{skills.length}<Text>)</Text></Text>
        </View>
        {editable && (
          <Pressable
            onPress={onAdd}
      ...
      {hasSkills ? (
        <View className="flex flex-wrap gap-2">
          {skills.map((skill) => (
            <View
              key={skill._id}
              className="group flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium bg-purple-500/10 border border-purple-500/15 text-purple-300"
            >
              ...
              {!editable && onEndorse && (
                <Pressable
                  onPress={() => onEndorse(skill._id)}
                  className="ml-1 p-0.5 rounded-full opacity-0"
                >
              ...
              {editable && (
                <View className="flex items-center gap-0.5">
                  <Pressable
                    onPress={() => onEdit?.(skill._id)}
                    className="p-0.5 rounded"
                  >
                    <Edit3 size={10} className="text-white/40" />
                  </Pressable>
                  <Pressable
                    onPress={() => onDelete?.(skill._id)}
                    className="p-0.5 rounded"
                  >
