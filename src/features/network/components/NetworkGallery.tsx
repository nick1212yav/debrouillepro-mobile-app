...
export function NetworkGallery({ images, alt = "" }: NetworkGalleryProps) {
  ...
  const renderImage = (src: string, idx: number, className = "") => {
    return (
      <Image key={`${src}-${idx}`} className={`w-full h-full object-contain bg-black/10 ${className}`} onPress={(e) => {
  ...
  if (count === 1) {
    return (
      <>
        <View className="overflow-hidden rounded-2xl bg-white/5 border border-white/10"><View className="max-h-[560px] min-h-[240px] overflow-hidden">{renderImage(display[0], 0, "object-cover")}</View></View>
        {selectedIndex !== null && (
          <ImageModal
            images={images}
    ...
    <View className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-4 backdrop-blur-sm" onPress={onClose}>
      ...
      {images.length > 1 && (
        <>
          <Pressable onPress={(e) => {
              goPrev();
            }} className="absolute left-5 top-1/2 -translate-y-1/2 z-10 rounded-full bg-white/10 p-3 text-white transition-colors">
            <ChevronLeft size={28} />
          </Pressable>
          <Pressable onPress={(e) => {
              goNext();
            }} className="absolute right-5 top-1/2 -translate-y-1/2 z-10 rounded-full bg-white/10 p-3 text-white transition-colors">
            <ChevronRight size={28} />
          </Pressable>
          <View className="absolute bottom-5 left-1/2 -translate-x-1/2 z-10 flex gap-2">
            {images.map((_, i) => (
              <Pressable key={i} className={`w-2 h-2 rounded-full transition-colors ${
      ...
      <Image className="max-h-[90vh] max-w-[95vw] rounded-2xl object-contain shadow-2xl" onPress={(e) => e.stopPropagation()} source={{ uri: images[currentIndex] }} accessibilityLabel={`Image ${currentIndex + 1}`} />
    </View>
  );
}
