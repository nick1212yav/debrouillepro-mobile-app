...
export function CompanyHeader({
  ...
  if (isLoading) {
    return (
      <View className={cn("space-y-4", className)}>
        ...
        <View className="flex items-start gap-4 -mt-12 px-4">
          <Skeleton className="w-24 h-24 rounded-2xl border-4 border-[#020617]" />
          <View className="flex-1 space-y-2 pt-4">
  ...
  return (
    ...
    >
      {/* Cover */}
      <View className="relative h-40 rounded-[28px] overflow-hidden bg-gradient-to-r from-indigo-900/50 via-purple-900/30 to-black border border-white/10">
      ...
      <View className="px-4 -mt-12">
        <View className="flex items-start gap-4">
          <View className="w-24 h-24 rounded-2xl overflow-hidden border-4 border-[#020617] bg-white/5 flex items-center justify-center flex-shrink-0">
            {company.logo ? (
              <Image
          ...
          <View className="flex-1 min-w-0 pt-4">
            <View className="flex items-center gap-2 flex-wrap">
              <Text className="text-white text-2xl font-black truncate">
                {company.name}
              </Text>
              {company.isVerified && (
                <ShieldCheck
            ...
            <View className="flex items-center gap-3 flex-wrap mt-1 text-xs text-white/40">
              {company.city && (
        ...
        <View className="flex items-center gap-2 mt-4">
          ...
          {onMessage && (
            <Pressable
              onPress={onMessage}
              className="flex items-center gap-1.5 px-4 py-2.5 rounded-2xl bg-white/8 border border-white/10 text-white text-sm font-bold"
            >
          ...
          {onShare && (
            <Pressable
              onPress={onShare}
              className="w-10 h-10 rounded-2xl flex items-center justify-center bg-white/8 border border-white/10"
            >
        ...
        {/* Stats */}
        <View className="flex gap-6 mt-4 pt-4 border-t border-white/5">
          <View>
            <Text className="text-white font-black">{company.followers}</Text>
            <Text className="text-white/35 text-[11px]"><Text>Abonnés</Text></Text>
          </View>
          {company.jobsCount !== undefined && (
            <View>
              <Text className="text-white font-black">{company.jobsCount}</Text>
              <Text className="text-white/35 text-[11px]"><Text>Offres d'emploi</Text></Text>
            </View>
          )}
          {company.servicesCount !== undefined && (
            <View>
              <Text className="text-white font-black">{company.servicesCount}</Text>
              <Text className="text-white/35 text-[11px]"><Text>Services</Text></Text>
            </View>
          )}
        </View>
      </View>
