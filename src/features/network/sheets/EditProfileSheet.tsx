...
export function EditProfileSheet({
  ...
  const handleSubmit = async () => {
    if (!firebaseUser) {
      UIService.openToast("Vous devez être connecté", "error");
      return;
    }
          ...
          >
                ...
                <View>
                  ...
                  <View className="flex flex-wrap gap-2 mb-2">
                    {form.roles.map((role) => (
                      <Text
                        key={role}
                        className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-medium bg-indigo-500/20 text-indigo-300 border border-indigo-500/20"
                      >
                    ...
                    {roleOptions
                      ...
                      .map((role) => (
                        <Pressable
                          ...
                          onPress={() => handleAddRole(role.value)}
                ...
                <View>
                  ...
                  <View className="flex flex-wrap gap-2 mb-2">
                    {form.interests.map((interest) => (
                      <Text
                        key={interest}
                        className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-medium bg-emerald-500/20 text-emerald-300 border border-emerald-500/20"
                      >
                        {interest}
                        <Pressable
            ...
            <View className="px-5 py-4 border-t border-white/5 flex gap-3">
              ...
              <Pressable
                ...
                disabled={isSaving}
                className="flex-1 py-3 rounded-2xl text-sm font-semibold text-white flex items-center justify-center gap-2 bg-gradient-to-r from-indigo-500 to-purple-500 disabled:opacity-50"
              >
                {isSaving ? (
