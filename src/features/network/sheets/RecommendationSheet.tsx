...
export function RecommendationSheet({
  ...
  const handleSubmit = async () => {
    if (rating === 0) {
      UIService.openToast("Veuillez choisir une note", "error");
      return;
    }
    if (!comment.trim()) {
      UIService.openToast("Veuillez rédiger un commentaire", "error");
      return;
    }
          ...
          >
            ...
            <View className="px-5 py-6">
              <View className="space-y-6">
                {/* Note */}
                <View>
                  <Text className="text-white/50 text-xs font-semibold uppercase tracking-wider block mb-3">
                    Note *
                  </Text>
                  <View className="flex items-center gap-2">
                    ...
                    <Text className="ml-2 text-white/40 text-sm">
                      {rating > 0 ? `${rating}/5` : ""}
                    </Text>
                  </View>
                </View>
                ...
                <Text className="text-white/25 text-xs">
                  <Text>Votre recommandation sera visible publiquement sur le profil de cette personne.</Text></Text>
              </View>
            </View>
            ...
            <View className="px-5 py-4 border-t border-white/5 flex gap-3">
              ...
              <Pressable
                ...
                className="flex-1 py-3 rounded-2xl text-sm font-semibold text-white flex items-center justify-center gap-2 bg-gradient-to-r from-yellow-500 to-orange-500 disabled:opacity-50"
              >
                {isSubmitting ? (
