...
export function AgriCategoryFilter({
  ...
  return (
    <View className="flex items-center gap-2 overflow-x-auto pb-1">
      ...
      <Pressable whileTap={{ scale: 0.95 }} onPress={() => onSelectCategory(null)} className={`flex items-center gap-1.5 h-9 px-3.5 rounded-full text-xs font-semibold flex-shrink-0 transition-all border ${
      ...
      {categories.map((cat) => {
        ...
        return (
          <Pressable key={cat} whileTap={{ scale: 0.95 }} onPress={() => onSelectCategory(isSelected ? null : cat)} className={`flex items-center gap-1.5 h-9 px-3.5 rounded-full text-xs font-semibold flex-shrink-0 transition-all border`} style={{ backgroundColor: isSelected
                          ? `${color}15`
