    ...
    return (
      <View className="flex flex-col gap-3">
        {Array.from({ length: 4 }).map((_, i) => (
          <View
            key={i}
            className="flex gap-4 p-3 rounded-[24px] bg-white/[0.01] border border-white/5 animate-pulse"
          >
            <View className="w-24 h-24 rounded-[16px] bg-white/[0.03] flex-shrink-0" />
            <View className="flex-1 space-y-3 py-1">
              <View className="h-3.5 bg-white/[0.04] rounded-lg w-2/3" />
              <View className="h-2.5 bg-white/[0.03] rounded-lg w-full" />
              <View className="flex justify-between items-center pt-2">
                <View className="h-3.5 bg-white/[0.04] rounded-lg w-1/4" />
                <View className="h-3 bg-white/[0.03] rounded-lg w-12" />
              </View>
            </View>
          </View>
        ))}
    ...
    <View className="flex flex-col gap-3">
      {products.map((product) => (
        <Pressable
          ...
          className="group relative flex gap-4 p-3 rounded-[24px] bg-white/[0.02] border border-white/5"
        >
          {/* Section gauche : Image ou Icône */}
          <View className="relative w-24 h-24 rounded-[18px] overflow-hidden bg-white/[0.01] flex-shrink-0">
            {product.media.images && product.media.images.length > 0 ? (
              ...
              <View className="w-full h-full flex items-center justify-center text-3xl bg-gradient-to-br from-white/[0.02] to-transparent">
          ...
          <View className="flex-1 min-w-0 flex flex-col justify-between py-1">
            <View>
              <View className="flex items-start justify-between gap-2">
                <Text className="text-white font-semibold text-xs leading-snug truncate">
                  {product.title}
                </Text>
                {onToggleFavorite && (
                  <Pressable
              ...
              <Text className="text-white/40 text-[10px] leading-relaxed mt-0.5">
                {product.description}
              </Text>
            </View>
            ...
            <View className="flex items-center gap-3 text-[10px] text-white/50">
              ...
              <View className="flex items-center gap-0.5 truncate">
                <MapPin size={10} className="text-white/30 flex-shrink-0" />
                <Text className="truncate">{product.location.city}</Text>
              </View>
            </View>

            {/* Pied du composant ligne */}
            <View className="flex items-center justify-between gap-2 pt-1">
              <Text className="text-green-400 font-bold text-xs">
                {formatPrice(product.pricing.price, product.pricing.currency)}
                <Text className="text-white/40 font-normal text-[9px]">
                  {" "}
              ...
              <View className="flex items-center gap-1.5">
                <View
                  className="px-2 py-0.5 rounded-full text-[8px] font-bold tracking-wider"
                  style={{ backgroundColor: `${CATEGORY_COLORS[product.category]}1a` }}
                >
