...
export function AgriCard({
  ...
  return (
    <Pressable
      onPress={onClick}
      className="group relative flex flex-col w-full rounded-[24px] overflow-hidden bg-white/[0.02] border border-white/5"
    >
      {/* Container de l'image */}
      <View className="relative aspect-[4/3] w-full overflow-hidden bg-white/[0.01]">
        {product.media.images && product.media.images.length > 0 ? (
          ...
          <View className="w-full h-full flex items-center justify-center text-4xl bg-gradient-to-br from-white/[0.02] to-transparent">
        ...
        {onToggleFavorite && (
          <Pressable
            ...
            className="absolute top-3 right-3 z-10 w-8 h-8 rounded-full flex items-center justify-center bg-black/40 border border-white/5"
          >
            <Heart
              size={15}
              className={
        ...
        {/* Badge de catégorie */}
        <View
          className="absolute top-3 left-3 px-2.5 py-1 rounded-full text-[10px] font-bold tracking-wider border border-white/5"
      ...
      <View className="p-3.5 flex flex-col flex-1 justify-between gap-2.5">
        <View className="space-y-1">
          <Text className="text-white font-semibold text-xs leading-snug">
            {product.title}
          </Text>
          <Text className="text-white/40 text-[10px] leading-relaxed">
            {product.description}
          </Text>
        </View>

        {/* Métadonnées (Avis, Ville) */}
        ...
        {/* Pied de carte : Prix et vérification */}
        <View className="pt-2 border-t border-white/5 flex items-center justify-between gap-2">
