    ...
    >
      <View className="flex items-center gap-3 border-b border-slate-800 pb-3">
        ...
        <View>
          <Text className="text-base font-black text-white">
            Organiser un Événement Live
          </Text>
          <Text className="text-[10px] text-slate-400">
            Rassemblez votre communauté autour de concerts et buffets
          </Text>
        </View>
      </View>
      ...
      <View className="space-y-1.5">
        ...
        <TextInput
          ...
          placeholder="Ex: Soirée Rumba & Grillades de Mouton"
          value={title}
      ...
      <View className="space-y-1.5">
        ...
        <TextInput
          ...
          placeholder="Décrivez le programme, les artistes invités, et la formule de buffet incluse..."
          value={description}
      ...
      <View className="gap-3">
        <View className="space-y-1.5">
          <Text className="block text-[10px] text-slate-400 uppercase font-bold flex items-center gap-1">
            <DollarSign size={11} /> Coût d'entrée (FCFA)
          </Text>
          <TextInput
           
            value={ticketCost}
            onChangeText={(text) => setTicketCost(Number(text))}
        ...
        <View className="space-y-1.5">
          <Text className="block text-[10px] text-slate-400 uppercase font-bold flex items-center gap-1">
            <Users size={11} /> Capacité d'accueil
          </Text>
          <TextInput
           
            value={maxSeating}
            onChangeText={(text) => setMaxSeating(Number(text))}
      ...
      <View className="space-y-1.5">
        <Text className="block text-[10px] text-slate-400 uppercase font-bold flex items-center gap-1">
          <Image size={11} /> URL de l'affiche de l'événement
        </Text>
        <TextInput
          ...
          className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-xs text-white outline-none"
          keyboardType="url" autoCapitalize="none" autoCorrect={false} editable={!(isSubmitting)}/>
        {errors.coverImage && (
          <Text className="text-[10px] text-rose-500 flex items-center gap-1">
            <AlertCircle size={10} />
            {errors.coverImage}
      ...
      >
        <ShieldCheck size={14} />
        {isSubmitting ? "Création de l'événement..." : "Publier l'Événement"}
      </Pressable>
    </View>
  );
}
