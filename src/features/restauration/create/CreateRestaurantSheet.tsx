...
export function CreateRestaurantSheet({
  ...
  const handleSubmit = async (data: any): Promise<boolean> => {
    ...
    try {
      ...
      UIService.openToast("Restaurant créé avec succès !", "success");
      onOpenChange(false);
      onSuccess?.();
      return true;
    } catch (error) {
    ...
    <>
      <View className="fixed inset-0 z-50 flex items-end">
        ...
        >
          ...
          <View className="px-5 pt-2 pb-4 flex items-center gap-3 border-b border-white/5">
            ...
            <View className="flex-1">
              ...
              <Text className="text-white/40 text-xs font-medium">
                <Text>Remplissez les informations de votre établissement</Text></Text>
            </View>
            <Pressable
              onPress={() => onOpenChange(false)}
              className="w-10 h-10 rounded-2xl flex items-center justify-center bg-white/5"
          ...
          >
            <CreateRestaurantForm onSubmit={handleSubmit} isLoading={loading} />
          </View>
        </View>
      </View>
    </>
