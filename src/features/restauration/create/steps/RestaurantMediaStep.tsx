  ...
  const handleFileUpload = useCallback(
    (files: FileList | null) => {
      ...
      Array.from(files)
        ...
        .forEach((file) => {
          if (!file.type.startsWith("image/")) {
            UIService.openToast(`${file.name} n'est pas une image`, "error");
            return;
          }
          if (file.size > 5 * 1024 * 1024) {
            UIService.openToast(`${file.name} dépasse 5 Mo`, "error");
            return;
          }
      ...
      if (files.length > remaining) {
        UIService.openToast(`${files.length - remaining} photos ignorées (max 10)`, "info");
      }
    },
    [images, onChange],
  );
  ...
  return (
    <View className="space-y-6">
      <View>
        <View className="flex items-center gap-3 mb-2">
          ...
          <Text className="text-white font-bold text-lg">
            Donnez faim avant même la première bouchée
          </Text>
        </View>
        <Text className="text-white/40 text-sm">
          Ajoutez jusqu'à 10 photos (5 Mo max chacune). Choisissez votre photo
          de couverture.
        </Text>
      </View>
      ...
      >
        {images.length === 0 ? (
          <>
            ...
            <TextInput
              ...
              onChangeText={(e) => handleFileUpload(e.target.files)}
            />
          </>
        ) : (
          <View className="gap-2">
            {images.map((src, i) => (
              <View
                key={i}
                className="relative aspect-square rounded-xl overflow-hidden group bg-black/30"
              >
                <Image
                ...
                <View className="absolute inset-0 bg-black/50 opacity-0 flex items-center justify-center gap-2">
                  <Pressable
                    onPress={() => setCover(i)}
                ...
                {i === coverIndex && (
                  <View className="absolute top-1 left-1 px-1.5 py-0.5 rounded bg-yellow-400 text-black text-[10px] font-bold">
                    <Text>COUV.</Text></View>
                )}
              </View>
            ))}
        ...
        <Text className="text-white/20 text-xs mt-2">
