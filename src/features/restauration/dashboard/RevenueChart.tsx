...
export function RevenueChart({ monthlyRevenues }: RevenueChartProps) {
  ...
  return (
    <View className="p-5 rounded-3xl bg-white/[0.01] border border-white/[0.06] text-left space-y-4">
      ...
      <View className="h-44 flex items-end gap-3 pt-6 relative">
        {monthlyRevenues.map((entry) => {
          ...
          return (
            ...
            >
              ...
              <Text className="absolute -top-1 text-[9px] font-black text-orange-400 opacity-0">
                {(entry.amount / 1000).toFixed(0)}<Text>k</Text></Text>

              <View
                className="w-full bg-gradient-to-t from-orange-600 to-orange-400 rounded-lg"
