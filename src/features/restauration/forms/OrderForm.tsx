    ...
    <View className="space-y-4 text-left">
      <View className="space-y-1.5">
        ...
        <View className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-white/5 border border-white/10">
          ...
          <TextInput
            ...
            placeholder="Ex: Riviera 3, Rue de la Pharmacie des Allées, Villa 45"
      ...
      <View className="space-y-1.5">
        ...
        <Picker
         ...
         selectedValue={paymentMethod} enabled={!(isSubmitting)}>
          <Picker.Item label="Mobile Money / Wave" value="mobile_money" />
          <Picker.Item label="Carte de Crédit (Visa / Mastercard)" value="card" />
          <Picker.Item label="Stablecoin Crypto (USDT)" value="crypto" />
          <Picker.Item label="Paiement Cash à la livraison" value="cash" />
        </Picker>
      ...
      <View className="space-y-1.5">
        <Text className="block text-[10px] text-white/40 uppercase font-black tracking-wider">
          Code Coupon de réduction (Optionnel)
        </Text>
        <View className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-white/5 border border-white/10">
          <Tag size={15} className="text-white/40 shrink-0" />
          <TextInput
           
            placeholder="Ex: MAMA225"
            value={couponCode}
      ...
      >
        <ShieldCheck size={14} />
        {isSubmitting ? "Traitement de l'achat..." : "Enregistrer la Commande"}
      </Pressable>
    </View>
  );
}
