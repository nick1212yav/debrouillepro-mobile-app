...
export default function RestaurationChefPage({
  ...
  const handleBookingSubmit = async (e: unknown) => {
    if (!chef || !bookingDate || !selectedMenu) {
      UIService.openToast("Veuillez renseigner la date de l'événement et la formule de menu.", "error");
      return;
    }
    ...
    if (response.success && response.bookingId && response.escrowId) {
      ...
      UIService.openToast("Honoraires du Chef sécurisés en séquestre DébrouillePay !", "success");
    } else {
      UIService.openToast("Échec lors du blocage de votre réservation.", "error");
    }
  };
  ...
  if (loadingRestaurant) {
    return (
      <View className="h-full flex items-center justify-center text-white/50 text-xs gap-2">
        ...
        <Text>Chargement du profil professionnel du Chef...</Text>
      </View>
    );
  }
  ...
  if (!restaurant || !chef) {
    return (
      <View className="h-full flex flex-col items-center justify-center gap-4 text-white/50 text-xs px-4">
        <ChefHat size={24} className="text-white/20 animate-bounce" />
        <Text>
          Cet établissement ne propose pas de prestations de Chef à domicile
          pour le moment.
        </Text>
        <Pressable
          onPress={onBack}
  ...
  return (
    <View className="h-full w-full flex flex-col relative text-white bg-[#020617]">
      ...
      <View className="flex-1 overflow-y-auto px-4 py-4 space-y-4 no-scrollbar">
        ...
        {/* 2. OPTIMISEUR DE RECETTES IA (DÉBROUILLEAI CHEF ASSISTANT) */}
        <View className="p-4 rounded-2xl bg-gradient-to-tr from-orange-500/[0.04] to-amber-500/[0.02] border border-orange-500/15 text-left space-y-3">
          <View className="flex justify-between items-center">
            <View className="flex items-center gap-1.5 text-orange-400">
              <Sparkles size={14} className="animate-pulse" />
              <Text className="text-[10px] font-black uppercase tracking-wider">
                Chef Assistant DébrouilleAI
              </Text>
            </View>
            {!showAIOptimizer && (
              <Pressable
                onPress={handleTriggerAIOptimizer}
                className="text-[9px] font-black uppercase tracking-wider text-white border border-white/10 px-2 py-1 rounded bg-white/5"
              >
                <Text>Analyser la carte</Text></Pressable>
            )}
          </View>

          {showAIOptimizer && aiRecipeFeedback && (
            <View className="text-xs text-white/60 leading-relaxed space-y-2 pt-2 border-t border-white/[0.04] animate-fade-in font-normal">
              <Text className="block text-[9px] text-orange-400 font-bold uppercase tracking-wider">
                Ajustement éco-responsable suggéré :
              </Text>
              <Text className="italic font-normal">
                "{aiRecipeFeedback.localAlternatives[0]?.reason}"
              </Text>
              <View className="p-2.5 rounded-lg bg-[#020617] border border-white/5 text-[10px] text-white/40">
                <Text>
                  Remplacement conseillé : ➔{" "}
                  {aiRecipeFeedback.localAlternatives[0]?.replacement}
                </Text>
              </View>
            </View>
          )}
        ...
        {/* 3. PLATS SIGNATURES DU CHEF */}
        <ChefRecipes
          recipes={signatureRecipes}
          onAddRecipe={(recipe) => {
            setSelectedMenu(recipe.name);
            setShowBookingModal(true);
            UIService.openToast(`Formule '${recipe.name}' présélectionnée pour votre événement !`, "success");
          }}
        />
      </View>

      {/* MODAL : FORMULAIRE ET SÉQUESTRE DE RÉSERVATION CHEF */}
      <>
        {showBookingModal && (
          <View className="fixed inset-0 z-50 flex items-end justify-center bg-black/70">
            ...
            >
              {confirmedBooking ? (
                ...
                <View className="space-y-4 text-center py-4">
                  <View className="w-14 h-14 rounded-full bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 mx-auto">
                    <CheckCircle size={28} />
                  </View>

                  <View className="space-y-1 text-center">
                    <Text className="text-base font-black text-white">
                      Honoraires Sécurisés !
                    </Text>
                    <Text className="text-xs text-white/50 leading-relaxed font-normal px-2">
                      Les fonds d'honoraires du Chef ont été placés avec succès
                      sous séquestre DébrouillePay. Le virement ne sera débloqué
                      qu'à l'achèvement de l'événement gastronomique.
                    </Text>
                  </View>

                  <View className="text-left bg-[#020617] p-4 rounded-xl border border-white/5 text-xs space-y-2.5">
                    <View className="flex justify-between">
                      <Text className="text-white/40">ID Réservation</Text>
                      <Text className="font-mono font-bold text-white/80">
                        {confirmedBooking.bookingId}
                      </Text>
                    </View>
                    <View className="flex justify-between">
                      <Text className="text-white/40">ID Compte Séquestre</Text>
                      <Text className="font-mono font-bold text-orange-400">
                        {confirmedBooking.escrowId}
                      </Text>
                    </View>
                    <View className="flex justify-between border-t border-white/5 pt-2">
                      <Text className="text-white/40 font-bold">
                        Montant Garanti
                      </Text>
                      <Text className="font-black text-white">
                        {confirmedBooking.total.toLocaleString()} FCFA
                      </Text>
                    </View>
                  </View>
                  ...
                  <Pressable
                    ...
                    className="w-full py-4 rounded-xl bg-emerald-500 text-slate-950 font-black text-xs uppercase tracking-wider"
                  >
                ...
                >
                  ...
                  <View className="gap-4">
                    <View className="space-y-1.5">
                      ...
                      <TextInput
                        ...
                        value={guestsCount}
                        onChangeText={(text) => setGuestsCount(Number(text))}
                        className="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-3 text-xs text-white outline-none"
                        min={1}
                    ...
                    <View className="space-y-1.5">
                      ...
                      <Picker
                        ...
                        <Picker.Item label="Sélectionner une formule" value="" />
                  ...
                  <View className="p-3.5 rounded-xl bg-orange-500/5 border border-orange-500/15 flex items-start gap-3 mt-1.5">
                    <FileText
                      size={15}
                      className="text-orange-400 shrink-0 mt-0.5"
                    />
                    <View>
                      <Text className="block text-[9px] text-orange-400 uppercase font-black">
                        <Text>Sécurisation DébrouillePay</Text></Text>
                      <Text className="text-[11px] text-white/50 leading-relaxed font-normal mt-0.5">
                        <Text>Pour garantir le déplacement du Chef, les honoraires d'intervention de</Text>{" "}
                        <b className="text-white">
                          {chef.dailyRate.toLocaleString()} <Text>FCFA</Text></b>{" "}
                        <Text>seront prélevés et retenus de manière neutre sous séquestre d'arbitrage.</Text></Text>
                    </View>
                  </View>
                  ...
                  >
                    ...
                    {isBooking
                      ? "Blocage de sécurité..."
                      : "Confirmer et Bloquer les honoraires"}
                  </Pressable>
                </View>
              )}
            </View>
