...
export default function RestaurationSearchPage({
  ...
  return (
    <View
      className="h-screen w-full flex flex-col relative text-white"
      style={{  }}
    >
      <View className="flex-shrink-0 px-4 pt-12 pb-3 bg-slate-950/60 border-b border-white/[0.04] flex items-center gap-3">
        ...
        <View className="flex-1 text-left">
          ...
          <Text className="text-[10px] text-white/40 mt-1 uppercase tracking-wider font-bold">
            Filtres multicritères & Géo-tri
          </Text>
        </View>
      </View>

      <View className="flex-1 flex relative overflow-hidden">
        <>
          {showFiltersPanel && (
            <View
              className="absolute inset-y-0 left-0 w-72 bg-[#060a22] border-r border-white/10 p-5 z-20 flex flex-col justify-between text-left shadow-2xl"
            >
              <View className="space-y-5 overflow-y-auto no-scrollbar pb-6">
                <View className="flex justify-between items-center border-b border-white/5 pb-3">
                  <Text className="text-xs font-black uppercase text-white tracking-wider">
                    Ajuster mes critères
                  </Text>
                  <Pressable
                    onPress={handleResetFilters}
                    className="text-[9px] font-black uppercase tracking-wider text-orange-400 flex items-center gap-1"
                  >
                    <RotateCcw size={10} /> <Text>Reset</Text></Pressable>
                </View>

                <View className="space-y-2">
                  <Text className="block text-[9px] text-white/40 uppercase font-black">
                    Spécialité culinaire
                  </Text>
                  <Picker
                   
                    onValueChange={(val) => setSelectedCuisine(val)}
                    className="w-full bg-[#020617] border border-white/10 rounded-xl px-3 py-2.5 text-xs text-white outline-none"
                   selectedValue={selectedCuisine}>
                    <Picker.Item label="Tous les types de cuisines" value="Tout" />
                    {CUISINES.map((c) => (
                ...
                <View className="space-y-2">
                  <View className="flex justify-between items-center text-[9px] text-white/40 uppercase font-black">
                    <Text>Rayon de livraison</Text>
                    <Text className="text-orange-400 font-black">
                      {maxDistance} km
                    </Text>
                  </View>
                  <TextInput
                    ...
                    {[
                      ...
                      return (
                        <Pressable
                          ...
                          className="w-full p-3 rounded-xl border flex items-center gap-2.5 text-left"
                        ...
                        >
                          ...
                          <Text
                            className={`text-xs ${isSelected ? "text-orange-400 font-extrabold" : "text-white/60 font-medium"}`}
                          >
                            {option.label}
                          </Text>
                        </Pressable>
                ...
                <View className="space-y-2 pt-2 border-t border-white/5">
                  ...
                  >
                    <View className="flex items-center gap-2.5">
                      ...
                      />
                      <Text
                        className={`text-xs ${openOnly ? "text-emerald-400 font-bold" : "text-white/60"}`}
                      >
                        Ouvert uniquement
                      </Text>
                    </View>
              ...
              <Pressable
                ...
                className="w-full py-3.5 rounded-xl bg-orange-500 text-slate-950 font-black text-xs uppercase tracking-wider"
              >
                <Text>Appliquer les Filtres</Text></Pressable>
            </View>
          )}
        </>
        ...
        <View className="flex-1 overflow-y-auto px-4 py-4 space-y-4 no-scrollbar">
          <View className="flex justify-between items-center px-1 text-white/40 text-[10px] font-black uppercase tracking-wider">
            <Text><Text>Résultats de recherche</Text></Text>
            <Text>{processedResults.length} <Text>maquis & restos</Text></Text>
          </View>
          ...
          {processedResults.length > 0 ? (
            ...
            <View className="text-center py-20 text-white/30 text-xs flex flex-col items-center gap-2">
              ...
              <Text className="font-bold">
                <Text>Aucun établissement ne correspond aux filtres.</Text></Text>
            </View>
          )}
        </View>
      </View>
