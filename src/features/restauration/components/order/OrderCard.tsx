...
export function OrderCard({ order, onSelect }: OrderCardProps) {
  ...
  return (
    <Pressable
      ...
      className={`p-4 rounded-2xl bg-white/[0.03] border border-white/[0.06] flex flex-col gap-3 text-left ${
        onSelect ? "cursor-pointer hover:bg-white/[0.05] transition-all" : ""
      }`}
    >
      <View className="flex justify-between items-start gap-2">
        <View>
          <Text className="block text-[9px] text-white/40 uppercase font-bold">
            Numéro de Commande
          </Text>
      ...
      <View className="border-t border-b border-white/[0.04] py-2.5 my-0.5 space-y-1">
        <Text className="font-extrabold text-sm text-white">
          {order.restaurantName}
        </Text>
        <Text className="text-xs text-white/50 truncate">
      ...
      <View className="flex justify-between items-center text-xs">
        ...
        <View className="flex items-center gap-1">
          ...
          <Text className="font-black text-orange-400">
            {order.total.toLocaleString()} <Text>FCFA</Text></Text>
        </View>
      </View>
    </Pressable>
  );
