...
export function OrderTimeline({ currentStatus }: OrderTimelineProps) {
  ...
  if (currentStatus === StatusEnum.CANCELLED) {
    return (
      <View className="p-4 rounded-xl bg-rose-500/5 border border-rose-500/10 text-center">
  ...
  return (
    <View className="py-4">
      <View className="flex items-center justify-between relative">
        {/* Ligne d'arrière-plan de progression */}
        <View className="absolute left-4 right-4 h-[2px] bg-white/10 top-[15px] z-0" />

        {/* Remplissage de progression active */}
        <View
          className="absolute left-4 h-[2px] bg-orange-500 top-[15px] z-0"
          style={{
        ...
        {steps.map((step, index) => {
          ...
          return (
            ...
            >
              <View
                className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-black border transition-all duration-300 ${
                  isDone
