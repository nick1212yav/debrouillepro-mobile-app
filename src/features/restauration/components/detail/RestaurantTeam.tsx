...
export function RestaurantTeam() {
  ...
  return (
    <View className="px-4 py-4 border-t border-white/[0.04]">
      ...
      <View className="flex gap-4 overflow-x-auto no-scrollbar">
        {team.map((member) => (
          <View
            key={member.name}
            className="flex items-center gap-3 p-2 rounded-xl bg-white/[0.02] border border-white/[0.04] min-w-44 text-left"
          >
            <Image
             
             
              className="w-10 h-10 rounded-lg object-cover"
             source={{ uri: member.photo }} accessibilityLabel={member.name}/>
            <View>
