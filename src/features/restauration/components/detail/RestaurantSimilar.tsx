  ...
  return (
    <View className="px-4 py-4 border-t border-white/[0.04]">
      ...
      {similars.map((sim) => (
        <View
          key={sim.name}
          className="flex items-center gap-3 p-2.5 rounded-xl bg-white/[0.02] border border-white/[0.04] text-left"
        >
          <Image
           
           
            className="w-12 h-12 rounded-lg object-cover"
           source={{ uri: sim.thumb }} accessibilityLabel={sim.name}/>
          <View className="flex-1 min-w-0">
            <Text className="block text-xs font-bold text-white truncate">
              {sim.name}
            </Text>
          ...
          <View className="flex items-center gap-1 bg-amber-400/10 px-2 py-0.5 rounded">
