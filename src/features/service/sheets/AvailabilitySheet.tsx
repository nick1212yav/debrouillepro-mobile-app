  ...
  return (
    <View className="space-y-2">
      {slots.map((slot) => (
        ...
        >
          <Text className="text-white/80 text-sm font-medium">{slot.date}</Text>
          <View className="flex flex-wrap gap-1 mt-1">
            {slot.slots.map((s, i) => (
              <Text
                key={i}
                className="px-2 py-0.5 rounded-lg text-xs bg-green-500/10 text-green-400"
              >
                {s.start} <Text>-</Text>{s.end}
              </Text>
            ))}
          </View>
        </View>
