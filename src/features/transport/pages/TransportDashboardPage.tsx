  ...
  return (
    <View className="h-full flex flex-col bg-gradient-to-b from-[#020412] to-[#040618] text-white">
      ...
      <View className="flex-shrink-0 px-4 pt-12 pb-3 flex items-center gap-3 border-b border-white/5 bg-[#070914]/40">
        ...
        <View>
          <Text className="text-base font-black">Console de Gestion [2]</Text>
          <Text className="text-[9px] text-violet-400 font-bold uppercase tracking-wider">
            Supervision d'activité de flotte [2]
          </Text>
        </View>
      </View>

      {/* Menu à onglets horizontaux tactiles */}
      <View
        className="flex-shrink-0 px-4 py-3 flex gap-2 overflow-x-auto border-b border-white/5"
        style={{  }}
      >
        {[
          {
          ...
          <Pressable
            key={tab.id}
            onPress={() => setActiveSection(tab.id as DashboardSection)}
      ...
      <View
        ...
        style={{  }}
      >
        {activeSection === "analytics" && <FleetDashboard />}
