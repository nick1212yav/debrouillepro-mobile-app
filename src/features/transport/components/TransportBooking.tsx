...
export function TransportBooking({
  ...
  const handleCancelClick = async () => {
    ...
    try {
      await onCancel();
      UIService.openToast("Réservation annulée avec succès [2].", "success");
    } catch {
      UIService.openToast("Une erreur s'est produite lors de l'annulation.", "error");
    } finally {
      setCancelling(false);
    }
  };
    ...
    <View className="p-5 rounded-3xl border border-white/5 bg-white/[0.02] space-y-4">
      <View className="flex items-center justify-between">
        ...
        <Text className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[9px] font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
          <CheckCircle2 size={10} /> Réservation {status} [2]
        </Text>
      </View>

      <View className="space-y-2 border-b border-white/5 pb-4">
        ...
        <View className="flex items-center justify-between text-xs">
          <Text className="text-white/40">Départ :</Text>
          <Text className="text-white font-bold">{departureTime}</Text>
        </View>
        <View className="flex items-center justify-between text-xs">
          <Text className="text-white/40">Places réservées :</Text>
          <Text className="text-white font-bold">
      ...
      <View className="flex items-center justify-between text-sm">
        <Text className="text-white/40"><Text>Montant acquitté :</Text></Text>
        <Text className="text-white font-black text-base">
          {totalAmount.toLocaleString()} {currency} <Text>[2]</Text></Text>
      </View>
