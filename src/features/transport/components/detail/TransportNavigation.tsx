...
export function TransportNavigation({
  ...
  const handleLaunchNavigation = () => {
    UIService.openToast(`Ouverture de la navigation vers : ${destinationName} [2]...`, "info");
    ...
    if (/iPad|iPhone|iPod/.test(undefined)) {
  ...
  return (
    <View className="p-5 rounded-3xl border border-white/5 bg-white/[0.02] space-y-4 text-white">
      <View className="flex items-center gap-2">
        <Compass size={16} className="text-violet-400 animate-spin-slow" />
      ...
      <View className="space-y-3">
        <Text className="text-xs text-white/50 leading-relaxed">
          <Text>Lancez le guidage vocal vers</Text>{" "}
          <strong className="text-white">{destinationName}</strong> <Text>dans l'application GPS de votre smartphone [2].</Text></Text>

        <Button
          onPress={handleLaunchNavigation}
          className="w-full h-11 rounded-xl text-xs font-bold gap-2 bg-gradient-to-r from-violet-600 to-indigo-600 text-white"
        >
          <Navigation size={14} className="" />
          <Text>Lancer le guidage routier [2]</Text></Button>
      </View>
    </View>
  );
}
