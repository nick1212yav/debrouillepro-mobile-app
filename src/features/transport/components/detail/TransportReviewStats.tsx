  ...
  return (
    <View className="p-5 rounded-3xl border border-white/5 bg-white/[0.02] space-y-4">
      <Text className="text-[10px] font-black text-violet-400 uppercase tracking-widest">
        Détails des évaluations [2]
      </Text>

      <View className="flex items-center gap-6">
        {/* Score global */}
        <View className="text-center space-y-1">
          <Text className="text-3xl font-black text-white">{averageRating}</Text>
          <View className="flex items-center justify-center gap-0.5 text-amber-400">
        ...
        <View className="flex-1 space-y-1.5">
          {distribution.map((dist) => (
            ...
            >
              <Text className="w-2.5 text-right font-bold">{dist.stars}</Text>
              ...
              <Text className="w-7 text-right font-bold">
                {dist.percentage}<Text>%</Text></Text>
            </View>
          ))}
        </View>
      </View>
