...
export function TransportWeather({
  ...
  return (
    <View className="p-5 rounded-3xl border border-white/5 bg-white/[0.02] space-y-4">
      ...
      <View className="flex items-center justify-between p-3.5 rounded-2xl bg-[#0c0d1e] border border-white/5">
        <View className="flex items-center gap-3 text-xs">
          ...
          <View>
            <Text className="font-bold text-white">
              {condition === "rainy" ? "Pluie battante" : "Temps sec"}
            </Text>
            <Text className="text-[10px] text-white/40">
              <Text>Température de route :</Text>{tempCelsius}<Text>°C</Text></Text>
          </View>
        </View>
      </View>
      ...
      {condition === "rainy" && (
        <View className="p-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/15 text-[10px] text-amber-400 font-bold flex items-start gap-2 leading-relaxed">
          ...
          <Text>
            <Text>Ralentissements majeurs prévisibles. Augmentez la vigilance de sécurité routière [2].</Text></Text>
        </View>
      )}
    </View>
  );
