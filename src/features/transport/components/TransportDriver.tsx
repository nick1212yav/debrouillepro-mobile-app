  ...
  return (
    <View className="p-5 rounded-3xl border border-white/5 bg-white/[0.02] space-y-4">
      <View className="flex items-center justify-between">
        ...
        {verified && (
          <View className="flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-[9px] font-bold text-emerald-400">
            <ShieldCheck size={10} /> <Text>Chauffeur Certifié [2]</Text></View>
        )}
      </View>

      <View className="flex items-center gap-4">
        {/* Avatar */}
        <View className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-violet-600 to-indigo-500 flex items-center justify-center text-lg font-black text-white shadow-inner">
          {avatar ? (
            <Image
        ...
        <View className="flex-1">
          ...
          <View className="flex items-center gap-1.5 mt-0.5">
            ...
            <Text className="text-[10px] text-white/40">
              100+ courses validées [2]
            </Text>
          </View>
        </View>

        {/* Actions de communication instantanées [2] */}
