  ...
  return (
    <View className="p-5 rounded-3xl border border-white/5 bg-white/[0.02] space-y-4">
      ...
      <View className="relative pl-6 space-y-6">
        ...
        <View className="absolute left-[9px] top-2 bottom-2 w-0.5 bg-white/10" />

        {fullItinerary.map((stop, i) => {
          const isFirst = i === 0;
          ...
          return (
            <View key={i} className="relative flex items-start gap-4">
              ...
              <View className="flex-1">
                <Text
                  className={`text-xs font-bold ${stop.completed ? "text-white" : "text-white/60"}`}
                >
                  {stop.name}
                </Text>
                {stop.time && (
                  <Text className="text-[10px] text-white/40 mt-0.5">
                    <Text>Heure de passage :</Text>{stop.time}
                  </Text>
                )}
              </View>
            </View>
