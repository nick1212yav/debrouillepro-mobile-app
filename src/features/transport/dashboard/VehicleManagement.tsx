      ...
      <View className="flex items-center justify-between">
        <View className="space-y-0.5">
          ...
          <Text className="text-base font-black">
            Véhicules de la Flotte ({vehicles.length}) [2]
          </Text>
        </View>
        <Button
          size="sm"
          className="rounded-xl h-9 bg-white/5 border border-white/10 text-xs font-bold gap-1"
        >
          <Plus size={14} /> <Text>Ajouter [2]</Text></Button>
      </View>
        ...
        {vehicles.map((v) => (
          ...
          >
            <View className="flex items-center gap-3">
              ...
              <View>
                <Text className="text-sm font-bold text-white">{v.model}</Text>
                <Text className="text-[10px] text-white/40 font-mono mt-0.5">
                  Plaque : {v.plate} · Chauffeur : {v.driverName}
                </Text>
              </View>
            </View>
            ...
            <View className="text-right">
              {v.status === "active" ? (
                <Text className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[9px] font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                  <CheckCircle2 size={10} /> <Text>En Service [2]</Text></Text>
                ...
                <Text className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[9px] font-bold bg-amber-500/10 text-amber-400 border border-amber-500/20">
                  <AlertCircle size={10} /> <Text>Au Garage [2]</Text></Text>
              ) : (
