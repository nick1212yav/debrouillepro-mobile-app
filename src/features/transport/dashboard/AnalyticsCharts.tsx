...
export function AnalyticsCharts() {
  ...
  return (
    <View className="p-5 rounded-3xl border border-white/5 bg-white/[0.02] space-y-4 text-white"><View className="flex items-center gap-2"><Cpu size={16} className="text-violet-400 animate-pulse" /><Text className="text-[10px] font-black text-violet-400 uppercase tracking-widest flex items-center gap-1">Prédiction Affluence IA (Débrouille AI) [2]
          <Sparkles size={11} className="text-violet-400" /></Text></View><View className="space-y-3"><Text className="text-xs text-white/50 leading-relaxed">Modèle d'analyse prédictive estimant le taux de congestion moyen de la
          voirie urbaine par tranche horaire pour l'optimisation des courses
          [2].
        </Text>{}<View className="flex items-end justify-between h-36 pt-4 px-2 border-b border-white/5 relative">{peakForecastHours.map((h, i) => (
            <View key={i} className="flex flex-col items-center gap-2 flex-1">
              <View className={`w-8 rounded-t-lg ${h.color} relative overflow-hidden`} style={{ height: `${h.value}px` }} initial={{ scaleY: 0 }} animate={{ scaleY: 1 }} transition={{ duration: 1, delay: i * 0.1, ease: "easeOut" }}>
