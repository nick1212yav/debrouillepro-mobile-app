      ...
      <View className="flex items-center justify-between">
        <View className="space-y-0.5">
          ...
          <Text className="text-base font-black">
            Chauffeurs et Permis ({drivers.length}) [2]
          </Text>
        </View>
      </View>
        ...
        {drivers.map((d) => (
          ...
          >
            <View className="flex items-center gap-3">
              <View className="w-10 h-10 rounded-xl bg-violet-600/10 border border-violet-500/20 flex items-center justify-center text-violet-400 font-bold">
                {d.name.charAt(0)}
              </View>
              <View>
                ...
                <View className="flex items-center gap-2 mt-0.5 text-[10px] text-white/40">
            ...
            {/* Actions rapides d'administration [2] */}
            <View className="flex items-center gap-2">
              <Button
                size="sm"
                onPress={() => toggleStatus(d.id)}
                className={`h-8 rounded-lg px-3 text-[10px] font-bold ${d.status === "approved" ? "bg-red-500/10 text-red-400 hover:bg-red-500/20" : "bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20"}`}
              >
                {d.status === "approved" ? "Suspendre [2]" : "Activer [2]"}
              </Button>
            </View>
          </View>
        ))}
