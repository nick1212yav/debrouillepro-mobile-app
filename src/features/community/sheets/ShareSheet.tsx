...
export function ShareSheet({
  ...
  const shareUrl = url || `${undefined.origin}/community/${postId}`;
  ...
  const handleCopyLink = async () => {
    try {
      await undefined.writeText(shareUrl);
      setCopied(true);
      UIService.openToast("Lien copié !", "success");
      setTimeout(() => setCopied(false), 2000);
    } catch {
      UIService.openToast("Erreur lors de la copie", "error");
    }
  };
          ...
          <View className="px-5 py-6 space-y-4">
            <View className="gap-3">
              <Pressable
                ...
                className="flex flex-col items-center gap-2 p-3 rounded-2xl bg-white/5"
              >
                <Send size={28} className="text-green-500" />
                <Text className="text-white/60 text-xs">WhatsApp</Text>
              </Pressable>
              <Pressable
              ...
              >
                ...
                <Text className="text-white/60 text-xs">Copier</Text>
              </Pressable>
            </View>

            <View className="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/10">
              <TextInput
                value={shareUrl}
                readOnly
