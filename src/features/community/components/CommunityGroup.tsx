  ...
  return (
    <View className="space-y-3">
      ...
      >
        {/* Cover image */}
        {coverImage && (
          <View className="relative aspect-[2/1] bg-black/20 overflow-hidden">
            <Image
             
             
              className="w-full h-full object-cover"
             source={{ uri: coverImage }} accessibilityLabel={name}/>
            <View
        ...
        <View className="p-4 space-y-3">
          ...
          <View className="flex items-start gap-3">
            {avatar ? (
              <Image
                ...
                className="w-14 h-14 rounded-2xl object-cover flex-shrink-0 border-2 border-white/10"
            ...
            <View className="flex-1 min-w-0">
              ...
              <View className="flex items-center gap-2 text-xs text-white/40">
                ...
                <View className="flex items-center gap-0.5">
                  ...
                  <Text>{isPrivate ? "Privé" : "Public"}</Text>
                </View>
                {city && (
                  <>
                    <Text>·</Text>
            ...
            <View className="space-y-1">
              <View className="flex items-center justify-between">
                ...
                {members.length > 6 && (
                  ...
                  >
                    {expandedMembers ? "Voir moins" : "Voir plus"}
                  </Pressable>
                )}
              </View>
              <View className="flex -space-x-2">
                {displayMembers.map((member) => (
                  <View
                    key={member.id}
                    className="w-7 h-7 rounded-full border-2 border-[#0D1117] overflow-hidden"
                  >
                    {member.avatar ? (
                      <Image
          ...
          >
            {isJoining ? (
              ...
              <>
                <Lock size={14} /> <Text>Demander à rejoindre</Text></>
            ) : (
              <>
                <Plus size={14} /> <Text>Rejoindre le groupe</Text></>
            )}
          </Pressable>
        </View>
      </Pressable>
