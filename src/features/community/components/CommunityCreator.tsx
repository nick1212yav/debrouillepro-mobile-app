...
export function CommunityCreator({
  ...
  return (
    ...
    >
      <View className="flex items-start gap-3">
        {avatar ? (
          <Image
            ...
            className="w-14 h-14 rounded-full object-cover flex-shrink-0 border-2 border-purple-500/30"
           source={{ uri: avatar }} accessibilityLabel={name}/>
        ) : (
          <View className="w-14 h-14 rounded-full flex items-center justify-center flex-shrink-0 bg-gradient-to-br from-purple-500 to-indigo-500">
        ...
        <View className="flex-1 min-w-0">
          <View className="flex items-center gap-1.5">
            ...
            {isVerified && (
              <Text className="text-[10px] bg-green-500/20 text-green-400 px-1.5 py-0.5 rounded-full">
                ✓ Vérifié
              </Text>
            )}
          </View>
          ...
          <View className="flex items-center gap-2 text-xs text-white/30 mt-1">
            <Calendar size={10} />
            <Text>Membre depuis {formatDate(joinedAt)}</Text>
          </View>
        </View>
        <Pressable
          onPress={(e) => {
      ...
      {/* Stats */}
      <View className="flex gap-3 mt-3 pt-3 border-t border-white/5">
        {stats.map((stat) => (
      ...
      {badges.length > 0 && (
        <View className="flex flex-wrap gap-1 mt-3 pt-3 border-t border-white/5">
          {badges.map((badge) => (
            <Text
              key={badge}
              className="flex items-center gap-0.5 px-2 py-0.5 rounded-full text-[10px] font-medium bg-yellow-500/10 text-yellow-400"
            >
              <Award size={10} />
              {badge}
            </Text>
