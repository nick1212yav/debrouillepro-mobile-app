    ...
    return (
      <View className="space-y-4">
        <View className="flex items-center gap-2">
          ...
          <Text className="text-sm font-medium text-white/50">Résultats</Text>
        </View>
        <View className="p-6 rounded-2xl bg-white/5 border border-white/10 text-center">
          <View className="text-4xl font-bold text-white mb-2">
            {score}<Text>/</Text>{totalQuestions}
          </View>
          <View className="text-lg font-medium text-white/80 mb-1">
            {percentage}<Text>%</Text></View>
          ...
          {isSubmitting && (
            <View className="flex items-center justify-center gap-2 mt-3">
              ...
              <Text className="text-white/40 text-sm">Enregistrement...</Text>
            </View>
          )}
          <Pressable
            onPress={handleRestart}
            className="mt-4 px-6 py-2 rounded-xl text-sm font-medium text-white bg-purple-500/20"
          >
            <Text>Recommencer</Text></Pressable>
        </View>
      </View>
    );
  }
    ...
    <View className="space-y-3">
      <View className="flex items-center justify-between">
        <View className="flex items-center gap-2">
          ...
          <Text className="text-sm font-medium text-white/50">{quiz.title}</Text>
        </View>
      ...
      <View className="p-4 rounded-xl bg-white/5 border border-white/10 space-y-3">
            ...
            return (
              ...
              >
                <Text className="text-white/60 text-xs font-mono">
                  {String.fromCharCode(65 + idx)}<Text>.</Text></Text>
                <Text
                  className={
