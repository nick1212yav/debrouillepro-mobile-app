...
import { Play, Image as ImageIcon, X } from "lucide-react-native";
    ...
    <View className="space-y-2">
          ...
          {validImages.slice(0, 6).map((img, idx) => (
            ...
            >
              <Image
                ...
                loading="lazy"
               source={{ uri: img }} accessibilityLabel={`${title} ${idx + 1}`}/>
              {idx === 5 && validImages.length > 6 && (
      ...
      {validVideos.length > 0 && (
        <View className="gap-2">
          {validVideos.slice(0, 4).map((video, idx) => (
            <Pressable
              key={idx}
