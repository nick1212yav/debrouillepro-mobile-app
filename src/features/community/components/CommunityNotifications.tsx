  ...
  return (
    <View className="relative">
      ...
      <>
        {isOpen && (
          <View
            className="absolute right-0 top-full mt-2 w-80 max-h-96 overflow-y-auto rounded-2xl p-3 bg-[#0D1117] border border-white/10 z-50"
          >
            <View className="flex items-center justify-between mb-3">
              ...
              <View className="flex items-center gap-2">
                {unreadCount > 0 && (
                  ...
                  >
                    <Text>Tout marquer comme lu</Text></Pressable>
                )}
                <Pressable
                  onPress={handleClose}
            ...
            {notifications.length === 0 ? (
              <View className="text-center py-6 text-white/30 text-sm">
                <Text>Aucune notification</Text></View>
            ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <View className="flex-1 min-w-0">
                        <Text className="text-white/80 text-sm">{notif.message}</Text>
                        <View className="flex items-center gap-2 text-white/20 text-[10px] mt-0.5">
                          <Clock size={10} />
                          <Text>{formatDate(notif.createdAt)}</Text>
                        </View>
                      </View>
                      {!notif.read && (
                        <Pressable
