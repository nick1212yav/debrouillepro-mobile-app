...
export function CommunityActions({
  ...
  return (
    <View className="flex items-center justify-between p-2 rounded-xl bg-white/5 border border-white/5">{}<View className="relative"><Pressable ref={buttonRef} onPress={() => {
            if (selectedReaction) {
          ...
          }} className="flex items-center gap-1.5 text-xs font-medium transition-all active:scale-95" style={{  }}>{getReactionIcon()}{displayCount > 0 && (
            <Text className="text-xs font-medium">{displayCount}</Text>
          )}</Pressable>{}{showReactionPicker && (
          <View ref={pickerRef} className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-zinc-800 rounded-2xl shadow-2xl border border-white/10 p-1.5 flex gap-1 z-20 animate-fade-in">
            {REACTION_EMOJIS.map((emoji) => {
              const count = reactions[emoji] || 0;
              const isActive = selectedReaction === emoji;
              return (
                <Pressable key={emoji} onPress={() => handleReactionSelect(emoji)} className={`
        ...
        )}</View>{}<Pressable onPress={onComment} className="flex items-center gap-1.5 text-xs font-medium text-white/40 transition-colors active:scale-95"><MessageCircle size={18} />{commentCount > 0 && <Text>{commentCount}</Text>}</Pressable>{}<Pressable onPress={onShare} className="flex items-center gap-1.5 text-xs font-medium text-white/40 transition-colors active:scale-95"><Share2 size={18} />{shareCount > 0 && <Text>{shareCount}</Text>}</Pressable>{}<Pressable onPress={onBookmark} className="flex items-center gap-1.5 text-xs font-medium transition-colors active:scale-95" style={{  }}><Bookmark size={18} className={isBookmarked ? "fill-amber-400" : ""} /></Pressable>{}{onReport && (
        <Pressable onPress={onReport} className="text-white/20 transition-colors active:scale-95">
