...
export function CommunityGallery({ images, title }: Props) {
  ...
  if (validImages.length === 0) {
    return (
      <View className="relative rounded-2xl overflow-hidden aspect-[16/9] bg-black/20 flex items-center justify-center">
        ...
        {imgError[index] ? (
          ...
          <>
            ...
            <Image
             ...
             source={{ uri: image }} accessibilityLabel={`${title} - ${index + 1}`}/>
          </>
        )}
      </View>
    );
    ...
    <>
      ...
      >
        ...
        <View className="relative w-full h-full">
          <>
            <ImageSlide index={currentIndex} />
          </>
        </View>

        {/* Contrôles - visibles au survol (desktop) ou toujours sur mobile */}
                    ...
                    >
                      {imgError[currentIndex] ? (
                        <View className="w-full h-full flex items-center justify-center">
                          <Text className="text-white/20 text-6xl"><Text>🖼️</Text></Text>
                        </View>
                      ) : (
                        <Image
