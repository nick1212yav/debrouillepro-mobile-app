  ...
  if (!isAdmin || reports.length === 0) {
    return (
      <View className="p-4 rounded-2xl bg-white/5 border border-white/5 text-center">
        ...
        <Text className="text-white/40 text-sm">Aucun signalement en attente</Text>
      </View>
    );
  }
    ...
    <View className="space-y-3">
      <View className="flex items-center justify-between">
        <View className="flex items-center gap-2">
          ...
          <Text className="text-sm font-medium text-white/50">Modération</Text>
          <Text className="text-xs text-white/30 bg-white/10 px-2 py-0.5 rounded-full">
              ...
              >
                <View className="flex-1 min-w-0">
                  <View className="flex items-center gap-2">
                    ...
                    <Text className="text-xs text-white/30">{report.type}</Text>
                  </View>
                  <Text className="text-white/60 text-xs truncate">
                    {report.reason}
                  </Text>
                  <View className="flex items-center gap-2 text-white/20 text-[10px] mt-0.5">
                    <Text><Text>Par</Text>{report.reportedBy}</Text>
                    <Text><Text>·</Text></Text>
                    <Text>{formatDate(report.reportedAt)}</Text>
                  </View>
                </View>
                  ...
                  report.status === "reviewing" ? (
                    <View className="flex flex-wrap gap-2">
                      <Pressable
                        ...
                        className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium text-white/60 bg-white/5"
                      >
                        <XCircle size={12} /> <Text>Ignorer</Text></Pressable>
                      <Pressable
                        onPress={() => handleAction(report.id, "warn")}
                        disabled={isLoading === report.id}
                        className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium text-amber-400 bg-amber-500/10"
                      >
                        <AlertTriangle size={12} /> <Text>Avertir</Text></Pressable>
                      <Pressable
                        onPress={() => handleAction(report.id, "remove")}
                        disabled={isLoading === report.id}
                        className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium text-red-400 bg-red-500/10"
                      >
                        <Ban size={12} /> <Text>Supprimer</Text></Pressable>
                      {report.status === "pending" && (
                        <Pressable
                          onPress={() => onReview(report.id)}
                          disabled={isLoading === report.id}
                          className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium text-blue-400 bg-blue-500/10"
                        >
                          <Eye size={12} /> <Text>Examiner</Text></Pressable>
                      )}
                    </View>
                  ) : (
