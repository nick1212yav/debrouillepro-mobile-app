      ...
      >
        ...
        <View className="space-y-3">
          <View className="flex items-start justify-between">
            <View>
              <Text className="text-white text-xl font-bold">{hospital.name}</Text>
              <View className="flex items-center gap-2 mt-1">
                <Text className="text-white/40 text-sm">{hospital.type}</Text>
                {hospital.rating && (
            ...
            <View className="text-right">
              <Text className="text-sm font-black text-orange-400">
                {hospital.priceRange}
              </Text>
              <Text
                className={`text-xs ${hospital.open ? "text-green-400" : "text-red-400"}`}
              >
                {hospital.open ? "Ouvert" : "Fermé"}
              </Text>
            </View>
          </View>
          ...
          <View className="flex flex-wrap gap-3 text-sm text-white/60">
            ...
            <View className="flex items-center gap-1">
              <Clock size={14} /> {hospital.hours}
            </View>
          </View>

          <View className="gap-2">
            <View className="p-3 rounded-xl bg-white/5 text-center">
              <Bed size={20} className="mx-auto text-white/40" />
              <Text className="text-white font-bold text-sm">{hospital.beds}</Text>
              <Text className="text-white/40 text-[10px]">Lits</Text>
            </View>
            <View className="p-3 rounded-xl bg-white/5 text-center">
              <Users size={20} className="mx-auto text-white/40" />
              <Text className="text-white font-bold text-sm">{hospital.doctors}</Text>
              <Text className="text-white/40 text-[10px]">Médecins</Text>
            </View>
            <View className="p-3 rounded-xl bg-white/5 text-center">
              <Stethoscope size={20} className="mx-auto text-white/40" />
              <Text className="text-white font-bold text-sm">
                {hospital.specialties}
              </Text>
              <Text className="text-white/40 text-[10px]">Spécialités</Text>
            </View>
          </View>
          ...
          <View className="flex flex-wrap gap-2">
            {hospital.services?.map((s: string) => (
              <Text
                key={s}
          ...
          >
            <Ambulance size={18} /> <Text>Urgences</Text></Pressable>

          {/* Correction : hospital n'a pas de champ 'reviews', on passe un tableau vide */}
          <DoctorReviews reviews={[]} averageRating={hospital.rating} />
          <DoctorCommunity
            reviews={hospital.reviewCount || 0}
            questions={0}
            followers={0}
          />
        </View>
