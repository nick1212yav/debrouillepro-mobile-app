...
function PublicProfileInner({ userId, onBack }: PublicProfilePageProps) {
  ...
  const handleShare = () => {
    ...
    if (undefined) {
      ...
      void undefined.writeText(`${name} – Débrouille Pro`);
  ...
  return (
    ...
    >
      {/* Cover */}
      <View className="relative h-40 flex-shrink-0 overflow-hidden">
        <View
      ...
      <View
        ...
        style={{  }}
      >
        <View className="px-5 -mt-14 relative z-10">
          <View className="flex items-end justify-between mb-4">
            ...
            <View className="flex items-center gap-2 pb-1 flex-wrap">
              <Authenticated>
                {!isLoading && profile && (
                  <FollowButton
                    userId={userId}
          ...
          {isLoading ? (
            ...
            <>
              <View className="flex items-center gap-2 mb-1">
                ...
                {role && (
                  <Text
                    ...
                    style={{ backgroundColor: role === "entreprise"
                                              ? "rgba(249,115,22,0.2)"
                                              : role === "professionnel"
                                                ? "rgba(99,102,241,0.2)"
              ...
              {profile?.bio && (
                <Text className="text-white/55 text-sm mb-3 leading-relaxed">
                  {profile.bio}
                </Text>
              )}

              <View className="flex items-center gap-3 mb-4 flex-wrap text-xs text-white/40">
                {profile?.city && (
              ...
              <View className="flex items-center gap-6 mb-5 px-1">
                <StatChip
          ...
          <View className="mb-2">
            <View className="flex items-center gap-2 mb-3">
              ...
              <Text className="text-xs font-bold text-white/50 uppercase tracking-wider">
                <Text>Publications</Text></Text>
              {publications && (
                <Text className="text-white/25 text-xs">
                  <Text>(</Text>{publications.length}<Text>)</Text></Text>
              )}
            </View>
            ...
            {publications === undefined ? (
              ...
              <View className="text-center py-10">
                ...
                <Text className="text-white/30 text-sm"><Text>Aucune publication</Text></Text>
              </View>
            ) : (
