...
function CartSheet({
  ...
  return (
    ...
    >
      <View
        className="w-full rounded-t-3xl max-h-[80vh] flex flex-col"
        ...
        <View className="flex items-center justify-between px-5 py-4 border-b border-white/8">
          ...
          <View className="flex items-center gap-2">
            {cart && cart.length > 0 && (
              ...
              >
                <Trash2 size={12} /> <Text>Vider</Text></Pressable>
            )}
            <Pressable
              onPress={onClose}
        ...
        {cart && cart.length > 0 && (
          <View className="px-5 pb-8 pt-3 border-t border-white/8">
            ...
            <Pressable
              ...
              style={{  }}
            >
              <Text>Commander</Text><ArrowRight size={16} />
            </Pressable>
          </View>
        )}
      </View>
...
function CheckoutSheet({
  ...
  const handleOrder = async () => {
    if (!address.trim()) {
      UIService.openToast("Adresse requise", "error");
      return;
    }
    setLoading(true);
    try {
    ...
    >
      ...
      >
        {step === "done" ? (
          ...
          <>
            <View className="flex items-center justify-between mb-6">
              <Text className="text-white font-bold text-lg">Récapitulatif</Text>
              <Pressable
            ...
            <View className="space-y-2 mb-4">
              {(cart ?? []).map((item) => (
                ...
                >
                  ...
                  <Text className="text-orange-400 font-bold text-sm ml-3">
                    {(
            ...
            <View className="flex items-center justify-between py-3 border-t border-white/10 mb-4">
            ...
            >
              {loading ? (
                <>
                  <Loader2 size={16} className="animate-spin" /> <Text>Traitement...</Text></>
              ) : (
    ...
    >
      ...
      <View className="relative h-64 flex-shrink-0">
        {product.images[imgIdx] ? (
          <Image
           ...
           source={{ uri: product.images[imgIdx] }} accessibilityLabel={product.title}/>
        ) : (
          <View
          ...
          <View className="absolute bottom-4 left-0 right-0 flex justify-center gap-1.5">
            {product.images.map((_, i) => (
              <Pressable
                ...
                onPress={() => setImgIdx(i)}
                className=""
                style={{ width: i === imgIdx ? 16 : 6, height: 6, borderRadius: 3, backgroundColor: i === imgIdx ? "#8B5CF6" : "rgba(255,255,255,0.4)" }}
              />
            ))}
          </View>
        )}
      ...
      >
        <View className="flex items-start justify-between mb-2">
          <View className="flex-1">
            <Text
              ...
              style={{ backgroundColor: "rgba(249,115,22,0.2)", color: "#fb923c", borderWidth: 1, borderColor: "rgba(249,115,22,0.3)", borderStyle: "solid" }}
            >
              {CATEGORY_ICONS[product.category] ?? "📦"} {product.category}
            </Text>
            <Text className="text-white font-bold text-xl leading-tight mt-1">
              {product.title}
            </Text>
          </View>
          <View className="text-right ml-4">
            <View className="text-2xl font-black" style={{  }}>
              {product.price.toLocaleString()}
            </View>
            <View className="text-white/50 text-xs">{product.currency}</View>
          </View>
        </View>

        <View className="flex items-center gap-4 mb-4">
          {avgRating && (
            <Text className="flex items-center gap-1 text-sm text-yellow-400">
              <Star size={13} fill="currentColor" /> {avgRating}
              <Text className="text-white/40 text-xs">({reviews?.length})</Text>
            </Text>
          )}
          {product.deliveryAvailable && (
            <Text className="flex items-center gap-1 text-xs text-emerald-400">
              <Zap size={11} /> Livraison dispo
            </Text>
          )}
          <Text className="text-xs text-white/50">Stock: {product.stock}</Text>
        </View>
        ...
        {product.tags.length > 0 && (
          <View className="flex gap-2 flex-wrap mb-5">
            {product.tags.map((tag) => (
              <Text
                key={tag}
                className="text-xs px-2.5 py-1 rounded-full"
                style={{ backgroundColor: "rgba(99,102,241,0.15)", borderWidth: 1, borderColor: "rgba(99,102,241,0.25)", borderStyle: "solid", color: "#a5b4fc" }}
              >
                #{tag}
              </Text>
            ))}
          ...
          <View className="flex items-center gap-3">
            ...
            <View className="flex-1">
              ...
              <View className="flex items-center gap-1 mt-0.5">
                ...
                <Text className="text-green-400 text-xs">Vendeur vérifié</Text>
              </View>
            </View>
          </View>
        </View>
      ...
      >
        <Authenticated>
          <View className="flex gap-3">
            <Pressable
              onPress={() => UIService.openToast("Messagerie vendeur — bientôt !", "info")}
              className="flex-1 py-3.5 rounded-2xl font-bold text-white/80 flex items-center justify-center gap-2"
              style={{ backgroundColor: "rgba(255,255,255,0.1)", borderWidth: 1, borderColor: "rgba(255,255,255,0.15)", borderStyle: "solid" }}
            >
              <MessageCircle size={16} /> <Text>Contacter</Text></Pressable>
            <Pressable
...
function SellForm({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = async () => {
    if (!title.trim() || !price || !description.trim()) {
      UIService.openToast("Veuillez remplir tous les champs", "error");
      return;
    }
    setLoading(true);
    try {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <View className="space-y-4">
          <View>
            <Text className="text-xs text-white/50 mb-1 block">Titre *</Text>
            <TextInput
              value={title}
          ...
          <View>
            <Text className="text-xs text-white/50 mb-2 block">
              Catégorie *
            </Text>
          ...
          <View className="flex gap-3">
            ...
            <View className="w-24">
              <Text className="text-xs text-white/50 mb-1 block">Stock</Text>
              <TextInput
                value={stock}
          ...
          <View>
            ...
            <TextInput
              ...
              placeholder="artisanat, fait main, coton"
              className="w-full rounded-xl px-4 py-3 text-sm text-white placeholder-white/30 outline-none"
              style={{ backgroundColor: "rgba(255,255,255,0.07)", borderWidth: 1, borderColor: "rgba(255,255,255,0.1)", borderStyle: "solid" }}
            />
          </View>
          <Pressable
            onPress={() => setDelivery((d) => !d)}
            className="flex items-center gap-2"
          >
            <View
              className="w-5 h-5 rounded-md flex items-center justify-center"
              style={{ backgroundColor: delivery
                                ? "rgba(16,185,129,0.3)"
                                : "rgba(255,255,255,0.1)", borderColor: "#10b981", borderStyle: "solid" }}
            >
              {delivery && <Check size={12} className="text-emerald-400" />}
            </View>
            <Text className="text-white/70 text-sm">Livraison disponible</Text>
          </Pressable>
          <Pressable
...
function OrdersSheet({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        <View className="flex items-center justify-between px-5 py-4 border-b border-white/8">
          <Text className="text-white font-bold text-lg">Mes commandes</Text>
          <Pressable
            onPress={onClose}
            className="w-8 h-8 rounded-full flex items-center justify-center"
            style={{ backgroundColor: "rgba(255,255,255,0.1)" }}
        ...
        <View
          className="flex gap-1 mx-4 mt-3 mb-0 p-1 rounded-xl"
        ...
        >
          {(["buyer", "seller"] as const).map((t) => (
            <Pressable
              ...
              style={{ backgroundColor: tab === t ? "rgba(255,255,255,0.1)" : "transparent" }}
            >
              {t === "buyer" ? "Mes achats" : "Mes ventes"}
            </Pressable>
          ))}
        </View>
        <View
        ...
        >
          {orders === undefined ? (
            ...
            <View className="text-center py-10">
              <ShoppingBag size={32} className="mx-auto mb-3 text-white/15" />
              <Text className="text-white/30 text-sm">Aucune commande</Text>
            </View>
          ) : (
            orders.map((order) => (
              <View
              ...
              >
                <View className="flex items-start justify-between mb-2">
                  <Text className="text-white font-semibold text-sm flex-1 truncate">
                  ...
                  <Pressable
                    onPress={() =>
                      void updateStatus({
                        id: order._id,
                        status: "confirmed",
                      }).then(() => UIService.openToast("Commande confirmée", "success"))
                    }
                    className="mt-2 w-full py-1.5 rounded-lg text-xs font-bold text-white"
                    style={{ backgroundColor: "rgba(99,102,241,0.3)", borderWidth: 1, borderColor: "rgba(99,102,241,0.4)", borderStyle: "solid" }}
                  >
    ...
    >
      ...
      <View className="flex-shrink-0 px-5 pt-12 pb-4">
        <View className="flex items-center gap-3 mb-4">
          ...
          <View className="flex-1">
            <Text className="text-white font-black text-xl">Boutique</Text>
            <Text className="text-white/40 text-xs">
              Achetez et vendez dans votre communauté
            </Text>
          </View>
          <View className="flex items-center gap-2">
            <Authenticated>
              ...
              >
                ...
                {cartCount > 0 && (
                  <Text className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-red-500 flex items-center justify-center text-[10px] font-bold text-white">
                    {cartCount}
                  </Text>
                )}
              </Pressable>
            ...
            <Pressable
              ...
              style={{  }}
            >
              <Plus size={15} /> <Text>Vendre</Text></Pressable>
          </View>
        </View>
        ...
        <View className="flex gap-3 mb-4">
          {[
            ...
            <View
              key={i}
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-medium flex-1 justify-center"
              style={{ backgroundColor: `${s.color}18`, borderStyle: "solid" }}
            >
              {s.icon} {s.label}
            </View>
          ))}
      ...
      {/* Sort bar */}
      <View className="flex-shrink-0 flex items-center justify-between px-5 py-2">
        <Text className="text-white/40 text-xs">
        ...
        <View className="relative">
          <Pressable
            ...
            style={{ backgroundColor: "rgba(255,255,255,0.07)", borderWidth: 1, borderColor: "rgba(255,255,255,0.1)", borderStyle: "solid" }}
          >
            <Filter size={13} /> {SORT_LABELS[sort]} <ChevronDown size={12} />
          </Pressable>
          <>
            {showSortMenu && (
              <View
                className="absolute right-0 top-full mt-1 z-20 rounded-xl overflow-hidden py-1 w-40"
                style={{ backgroundColor: "#0f1729", borderWidth: 1, borderColor: "rgba(255,255,255,0.1)", borderStyle: "solid" }}
              >
      ...
      >
        {status === "LoadingFirstPage" ? (
          ...
          <View className="flex flex-col items-center justify-center h-48 text-center">
            <Package size={40} className="text-white/20 mb-3" />
            <Text className="text-white/50 text-sm">Aucun article trouvé</Text>
          ...
          <>
              ...
              {sorted.map((product: Product, i: number) => (
                ...
                >
                  <View className="relative h-36">
                    ...
                    {product.deliveryAvailable && (
                      <Text
                        className="absolute top-2 left-2 text-[10px] font-bold px-1.5 py-0.5 rounded-lg"
                  ...
                  <View className="p-2.5">
                    ...
                    <View className="flex items-center justify-between">
                      ...
                      >
                        {product.price.toLocaleString()}{" "}
                        <Text className="text-[9px] text-white/40">FCFA</Text>
                      </Text>
                    ...
                    <View className="flex items-center gap-1 mt-1">
                      <SellerAvatar
                        ...
                        size={16}
                      />
                      <Text className="text-white/40 text-[10px] truncate">
                        {product.sellerName ?? "Vendeur"}
                      </Text>
                    </View>
                  </View>
            ...
            {status === "CanLoadMore" && (
              <View className="flex justify-center mt-4">
                <Pressable
                  onPress={() => loadMore(8)}
                  className="px-5 py-2.5 rounded-2xl text-sm font-semibold text-white/60"
                  style={{ backgroundColor: "rgba(255,255,255,0.07)", borderWidth: 1, borderColor: "rgba(255,255,255,0.1)", borderStyle: "solid" }}
                >
            ...
            >
              ...
              >
                <Text className="text-white font-bold mb-4">
                  <Text>Connectez-vous pour vendre</Text></Text>
                <SignInButton />
              </Pressable>
            </Pressable>
          </Unauthenticated>
        ...
        {showCheckout && (
          <CheckoutSheet
            ...
            onClose={(ordered) => {
              ...
              if (ordered) UIService.openToast("Merci pour votre commande !", "success");
            }}
          />
        )}
      </>
