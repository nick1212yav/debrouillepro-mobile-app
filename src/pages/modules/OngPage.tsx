        ...
        <View className="flex items-start justify-between mb-3">
          <View className="flex-1">
            <View className="flex items-center gap-2 mb-1">
              {urgent && (
                <Text className="px-2 py-0.5 rounded-full text-[10px] font-bold"
                  style={{ backgroundColor: "rgba(239,68,68,0.15)", color: "#EF4444" }}>🔥 Urgent</Text>
              )}
            </View>
            <Text className="text-white font-bold text-sm leading-tight">{campaign.title}</Text>
            <Text className="text-white/50 text-xs mt-0.5">{campaign.ngoName}</Text>
          </View>
        </View>

        <Text className="text-white/60 text-xs leading-relaxed mb-3">{campaign.description}</Text>

        <View className="mb-3">
          <View className="flex justify-between text-[10px] text-white/50 mb-1">
            <Text>{formatAmount(campaign.raised)} collectés</Text>
            <Text className="font-bold" style={{ color: urgent ? "#EF4444" : "#10B981" }}>{pct}%</Text>
          </View>
          <View className="h-2 rounded-full" style={{ backgroundColor: "rgba(255,255,255,0.08)" }}>
            <View
              className="h-full rounded-full"
              style={{  }} />
          </View>
          <Text className="text-[10px] text-white/30 mt-1">Objectif : {formatAmount(campaign.goal)} · {campaign.donorCount} donateurs</Text>
        </View>

        <Authenticated>
          <Pressable onPress={() => onDonate(campaign._id)}
            className="w-full py-2.5 rounded-xl text-sm font-bold text-white flex items-center justify-center gap-2"
            style={{  }}>
            <Heart size={14} /> <Text>Faire un don</Text></Pressable>
        </Authenticated>
        <Unauthenticated>
          <Pressable onPress={() => UIService.openToast("Connectez-vous pour faire un don", "info")}
            className="w-full py-2.5 rounded-xl text-sm font-bold text-white flex items-center justify-center gap-2"
            style={{  }}>
            <Heart size={14} /> <Text>Faire un don</Text></Pressable>
        </Unauthenticated>
...
function DonateModal({ campaignId, onClose }: { campaignId: string; onClose: () => void }) {
  ...
  const handleDonate = async () => {
    ...
    if (!num || num < 100) { UIService.openToast("Montant minimum : 100 FCFA", "error"); return; }
    setLoading(true);
    try {
      await donate({
      ...
      UIService.openToast("Merci pour votre don !", "success");
      onClose();
    } catch {
      UIService.openToast("Erreur lors du don", "error");
    } finally {
      setLoading(false);
    }
  };
      ...
      <View
        ...
        <View className="flex items-center justify-between mb-5">
          <View className="flex items-center gap-2">
            ...
            <Text className="text-white font-bold">Faire un don</Text>
          </View>
        ...
        <View className="flex items-center gap-2 px-4 py-3 rounded-2xl mb-3"
          ...
          <Text className="text-white/40 text-xs">FCFA</Text>
        </View>

        <TextInput value={message} onChangeText={(text) => setMessage(text)}
          placeholder="Message (optionnel)"
          className="w-full rounded-2xl px-4 py-3 text-sm text-white/90 outline-none mb-3"
          style={{ backgroundColor: "rgba(255,255,255,0.06)", borderWidth: 1, borderColor: "rgba(255,255,255,0.09)", borderStyle: "solid" }}  multiline textAlignVertical="top"/>

        <Pressable onPress={() => setAnon(!anon)}
          className="flex items-center gap-2 mb-4">
          <View className="w-4 h-4 rounded flex items-center justify-center"
            style={{ backgroundColor: anon ? "#EC4899" : "rgba(255,255,255,0.1)", borderWidth: 1, borderColor: "rgba(255,255,255,0.2)", borderStyle: "solid" }}>
            {anon && <CheckCircle size={10} className="text-white" />}
          </View>
          <Text className="text-white/60 text-xs">Don anonyme</Text>
        </Pressable>

        <Pressable onPress={handleDonate} disabled={loading}
...
export default function OngPage({ onBack }: { onBack: () => void }) {
  ...
  const handleCreateNGO = async () => {
    ...
    try {
      ...
      UIService.openToast("ONG créée avec succès !", "success");
  ...
  return (
    <View className="h-full flex flex-col overflow-hidden" style={{  }}>
      {/* Header */}
      <View className="flex items-center gap-3 px-4 pt-6 pb-4">
        <Pressable onPress={onBack} className="p-2 rounded-full" style={{ backgroundColor: "rgba(255,255,255,0.08)" }}>
          <ArrowLeft size={18} color="white" />
        </Pressable>
        <View className="flex-1">
          <Text className="text-white font-bold text-lg">ONG & Bénévolat</Text>
          <Text className="text-gray-400 text-xs">Dons · Missions · Impact</Text>
        </View>
        <View className="w-8 h-8 rounded-full flex items-center justify-center" style={{ backgroundColor: "rgba(249,115,22,0.15)" }}>
          <HandHeart size={16} color="#F97316" />
        </View>
      </View>
      ...
      <View className="flex-1 overflow-y-auto px-4 pb-6 space-y-4" style={{  }}>
          ...
          <>
            {campaigns === undefined ? (
              ...
              <View className="flex flex-col items-center py-16 gap-3">
                ...
                <Text className="text-white/40 text-sm text-center">Aucune campagne active pour le moment.</Text>
              </View>
            ) : (
            ...
            {myNGO === undefined ? (
              ...
              <View className="rounded-2xl p-4" style={{ backgroundColor: "rgba(255,255,255,0.05)", borderWidth: 1, borderColor: "rgba(255,255,255,0.08)", borderStyle: "solid" }}>
                <View className="flex items-center gap-3 mb-3">
                  <View className="w-12 h-12 rounded-2xl flex items-center justify-center text-2xl" style={{ backgroundColor: "rgba(249,115,22,0.15)" }}><Text>🏛️</Text></View>
                  <View>
                    <Text className="text-white font-bold">{myNGO.name}</Text>
                    <Text className="text-white/50 text-xs">{myNGO.category} · {myNGO.city}</Text>
                  </View>
                  {myNGO.verified && <CheckCircle size={16} className="text-green-400 ml-auto" />}
                </View>
                <Text className="text-white/60 text-sm mb-3">{myNGO.mission}</Text>
                <View className="flex gap-4 text-xs">
                  <Text className="text-white/50"><Text className="text-white font-bold">{formatAmount(myNGO.totalDonations)}</Text> collectés</Text>
                  <Text className="text-white/50"><Text className="text-white font-bold">{myNGO.donorCount}</Text> donateurs</Text>
                </View>
              </View>
            ) : (
              <View className="flex flex-col items-center py-8 gap-4">
                <View className="w-16 h-16 rounded-2xl flex items-center justify-center" style={{ backgroundColor: "rgba(249,115,22,0.1)" }}>
                  <HandHeart size={28} className="text-orange-400" />
                </View>
                <View className="text-center">
                  <Text className="text-white font-bold mb-1"><Text>Créer votre ONG</Text></Text>
                  <Text className="text-white/50 text-xs"><Text>Lancez des campagnes et collectez des fonds pour vos projets</Text></Text>
                </View>
                <Pressable onPress={() => setShowCreateNGO(true)}
                  ...
                  style={{  }}>
                  <Plus size={15} /> <Text>Créer mon ONG</Text></Pressable>
              </View>
            )}
          </Authenticated>
        )}
        ...
        {activeTab === "Mon ONG" && (
          <Unauthenticated>
            <View className="text-center py-8 text-white/40"><Text>Connectez-vous pour gérer votre ONG</Text></View>
          </Unauthenticated>
        )}
      </View>
        ...
        {showCreateNGO && (
          <View
            ...
            <View
              ...
              style={{ borderWidth: 1, borderColor: "rgba(255,255,255,0.08)", borderStyle: "solid" }}>
              <View className="flex items-center justify-between mb-5">
                <Text className="text-white font-bold"><Text>Créer mon ONG</Text></Text>
              ...
              {[
                ...
                <TextInput key={key} value={ngoForm[key as keyof typeof ngoForm]}
                  onChangeText={(text) => setNgoForm((p) => ({ ...p, [key]: text }))}
                  placeholder={placeholder}
                  className="w-full rounded-2xl px-4 py-3 text-sm text-white outline-none mb-3"
                  style={{ backgroundColor: "rgba(255,255,255,0.06)", borderWidth: 1, borderColor: "rgba(255,255,255,0.09)", borderStyle: "solid" }} />
              ))}
              <Pressable onPress={handleCreateNGO} disabled={!ngoForm.name || !ngoForm.mission}
                className="w-full py-3.5 rounded-2xl text-sm font-bold text-white disabled:opacity-40"
                style={{  }}>
                <Text>Créer l'ONG</Text></Pressable>
            </View>
          </View>
        )}
      </>
