...
export default function CommunityPage({ onBack }: Props) {
  ...
  const handleLike = async (publicationId: Id<"publications">) => {
    if (!user) {
      UIService.openToast("Connectez-vous pour réagir", "error");
      return;
    }
    try {
  ...
  const handleVote = async (
    ...
    if (!user) {
      UIService.openToast("Connectez-vous pour voter", "error");
      return;
    }
    try {
  ...
  const handleBookmark = async (publicationId: Id<"publications">) => {
    ...
    UIService.openToast("Fonctionnalité de favori à venir", "info");
  };

  const handleJoinGroup = async (groupId: string) => {
    if (!user) {
      UIService.openToast("Connectez-vous pour rejoindre un groupe", "error");
      return;
    }
    try {
      const res = await joinGroup({ groupId: groupId as Id<"groups"> });
      UIService.openToast(res.joined ? "Groupe rejoint !" : "Groupe quitté", "success");
    } catch {
  ...
  return (
    <>
      ...
      >
        ...
        >
          ...
          >
            {groups.map((g, i) => (
              <Pressable
                key={g._id}
                onPress={() => handleJoinGroup(g._id)}
                className="flex-shrink-0 flex flex-col items-center gap-1"
              >
                <View
                  className="w-12 h-12 rounded-2xl flex items-center justify-center relative"
                  style={{ backgroundColor: g.isMember
                                        ? `${g.color}35`
                                        : "rgba(255,255,255,0.05)", borderColor: "rgba(255,255,255,0.12)", borderStyle: "solid" }}
                >
                  <Users
                    size={18}
                    style={{
                      color: g.isMember ? g.color : "rgba(255,255,255,0.4)",
                    }}
                  />
                  {g.isMember && (
                    <Text className="absolute -top-1 -right-1 text-[10px]">
                      {g.emoji}
                    </Text>
                  )}
                </View>
                <Text className="text-[9px] text-white/40 text-center leading-tight w-14 truncate">
                  {g.name}
                </Text>
              </Pressable>
            ))}
            ...
            >
              <View
                ...
                style={{ backgroundColor: "rgba(255,255,255,0.05)", borderWidth: 2, borderColor: "rgba(255,255,255,0.15)", borderStyle: "dashed" }}
              >
                <Plus size={16} className="text-white/30" />
              </View>
              <Text className="text-[9px] text-white/25">Rejoindre</Text>
            </Pressable>
          </View>
        ...
        {/* Liste des posts */}
        <View
          className="flex-1 overflow-y-auto px-5 pb-24 flex flex-col gap-4"
          style={{  }}
        >
          {posts === undefined ? (
            ...
            <View className="flex flex-col items-center justify-center py-16 gap-3">
              ...
              <Text className="text-white/30 text-sm"><Text>Aucun post trouvé</Text></Text>
              {activeFilter === "mine" && (
                <Pressable
                  onPress={() => setShowCompose(true)}
                  className="text-xs text-purple-400 underline"
                >
                  <Text>Créer votre premier post</Text></Pressable>
              )}
            </View>
          ) : (
            filtered.map((post, i) => (
        ...
        <Pressable
          onPress={() => setShowCompose(true)}
          className="absolute bottom-6 right-5 w-14 h-14 rounded-2xl flex items-center justify-center shadow-2xl"
          style={{  }}
        >
