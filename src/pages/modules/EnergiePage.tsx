    ...
    <>
      {[
        ...
        <View key={label} className="flex items-center gap-3 p-4 rounded-2xl" style={{ backgroundColor: "rgba(255,255,255,0.04)", borderWidth: 1, borderColor: "rgba(255,255,255,0.08)", borderStyle: "solid" }}>
          ...
          <Pressable onPress={() => { void toggle(field); }} className="w-12 h-6 rounded-full relative" style={{ backgroundColor: val ? "#FBBF24" : "rgba(255,255,255,0.1)" }}>
            <View className="absolute top-1 w-4 h-4 rounded-full bg-white" style={{ left: val ? "calc(100% - 20px)" : "4px" }} />
          </Pressable>
        </View>
      ))}
    </>
  ...
  return (
    <View className="h-full flex flex-col overflow-hidden" style={{  }}>
      <View className="flex items-center gap-3 px-4 pt-12 pb-4">
        ...
        <View className="flex-1">
          <Text className="text-white font-bold text-lg">Énergie & Solaire</Text>
          <Text className="text-gray-400 text-xs">Gérez votre consommation</Text>
        </View>
        <Pressable className="p-2 rounded-full" style={{ backgroundColor: "rgba(251,191,36,0.15)" }}>
      ...
      <View className="flex gap-3 px-4 mb-4">
        <View className="flex-1 p-3 rounded-2xl" style={{ borderWidth: 1, borderColor: "rgba(251,191,36,0.3)", borderStyle: "solid" }}>
          <View className="flex items-center gap-1 mb-1"><Sun size={12} color="#FBBF24" /><Text className="text-gray-400 text-xs">Production</Text></View>
          <Text className="text-white font-bold text-base">{solarProduction} kWh</Text>
          <Text className="text-yellow-400 text-xs">{"Aujourd'hui"}</Text>
        </View>
        <View className="flex-1 p-3 rounded-2xl" style={{ borderWidth: 1, borderColor: "rgba(239,68,68,0.3)", borderStyle: "solid" }}>
          <View className="flex items-center gap-1 mb-1"><Zap size={12} color="#EF4444" /><Text className="text-gray-400 text-xs">Consommation</Text></View>
          <Text className="text-white font-bold text-base">{totalConsumption.toFixed(1)} kWh</Text>
          <Text className="text-red-400 text-xs">Ce jour</Text>
        </View>
        <View className="flex-1 p-3 rounded-2xl" style={{ borderWidth: 1, borderColor: "rgba(16,185,129,0.3)", borderStyle: "solid" }}>
          <View className="flex items-center gap-1 mb-1"><Leaf size={12} color="#10B981" /><Text className="text-gray-400 text-xs">Économies</Text></View>
          <Text className="text-white font-bold text-base">{savings.toLocaleString()}</Text>
          <Text className="text-green-400 text-xs">FCFA/mois</Text>
        </View>
      </View>

      <View className="flex gap-1 mx-4 mb-4 p-1 rounded-xl" style={{ backgroundColor: "rgba(255,255,255,0.04)" }}>
        {["Tableau de bord", "Consommation", "Fournisseurs"].map(tab => (
          <Pressable key={tab} onPress={() => setActiveTab(tab)} className="flex-1 py-2 rounded-lg text-xs font-medium" style={{ backgroundColor: activeTab === tab ? "rgba(255,255,255,0.1)" : "transparent" }}>
            {tab}
          </Pressable>
        ))}
      </View>
      ...
      <View className="flex-1 overflow-y-auto px-4 pb-6 space-y-4">
        {activeTab === "Tableau de bord" && (
          <>
            {ALERTS.map((a, i) => (
              <View key={i} className="flex items-center gap-3 p-3 rounded-xl" style={{ backgroundColor: a.type === "warning" ? "rgba(245,158,11,0.08)" : a.type === "success" ? "rgba(16,185,129,0.08)" : "rgba(99,102,241,0.08)", borderColor: "rgba(245,158,11,0.2)", borderStyle: "solid" }}>
                {a.type === "warning" ? <AlertTriangle size={14} color="#F59E0B" /> : a.type === "success" ? <CheckCircle size={14} color="#10B981" /> : <Zap size={14} color="#6366F1" />}
                <View className="flex-1"><Text className="text-white text-xs">{a.msg}</Text><Text className="text-gray-500 text-xs">{a.time}</Text></View>
              </View>
            ))}
              ...
              <View key={i}
                ...
                <View className="flex items-center justify-between mb-2">
                  <View className="flex items-center gap-2">
                    <View className="w-8 h-8 rounded-full flex items-center justify-center" style={{ backgroundColor: `${s.color}20` }}>
                      {s.type === "Solaire" ? <Sun size={14} color={s.color} /> : s.type === "Stockage" ? <Battery size={14} color={s.color} /> : <Zap size={14} color={s.color} />}
                    </View>
                    <View>
                      <Text className="text-white font-semibold text-sm">{s.name}</Text>
                      <Text className="text-gray-400 text-xs">{s.status}</Text>
                    </View>
                  </View>
                  <Text className="text-white font-bold text-sm">{s.production} {s.unit}</Text>
                </View>
                <View className="w-full h-1.5 rounded-full" style={{ backgroundColor: "rgba(255,255,255,0.08)" }}>
                  <View className="h-full rounded-full" style={{ width: `${s.efficiency}%`, backgroundColor: s.color }} />
                </View>
          ...
          <>
            <View className="p-4 rounded-2xl" style={{ backgroundColor: "rgba(255,255,255,0.04)", borderWidth: 1, borderColor: "rgba(255,255,255,0.08)", borderStyle: "solid" }}>
              <View className="flex items-center justify-between mb-4">
                <Text className="text-white font-bold">Répartition mensuelle</Text>
                <Text className="text-gray-400 text-sm">{totalCost.toLocaleString()} FCFA</Text>
              </View>
              {CONSUMPTION_DATA.map((item, i) => (
                <View key={item.label} className="mb-3">
                  <View className="flex justify-between mb-1">
                    ...
                    <Text className="text-white text-xs font-medium">{item.kwh} kWh · {item.cost.toLocaleString()} FCFA</Text>
                  </View>
                  <View className="w-full h-2 rounded-full" style={{ backgroundColor: "rgba(255,255,255,0.06)" }}>
                    <View className="h-full rounded-full" style={{ backgroundColor: item.color }} />
                  </View>
                </View>
              ))}
            </View>
              ...
              <View key={i}
                ...
                <View className="flex items-center justify-between mb-2">
                  ...
                  <View className="flex items-center gap-1">
                    <View className="w-2 h-2 rounded-full" style={{ backgroundColor: p.reliability > 85 ? "#10B981" : "#F59E0B" }} />
                    <Text className="text-xs" style={{ color: p.reliability > 85 ? "#10B981" : "#F59E0B" }}>{p.reliability}% fiabilité</Text>
                  </View>
                </View>
                <Text className="text-gray-400 text-xs mb-2">{p.type}</Text>
                <View className="flex items-center justify-between mb-3">
                  <View className="flex items-center gap-1"><DollarSign size={10} color="#FBBF24" /><Text className="text-yellow-400 text-xs">{p.tariff}</Text></View>
                ...
                <Pressable className="w-full py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-2" style={{ backgroundColor: "rgba(251,191,36,0.15)", borderWidth: 1, borderColor: "rgba(251,191,36,0.3)", borderStyle: "solid" }}>
                  <Text>Contacter:</Text>{p.contact}
                </Pressable>
              </View>
            ))}
            <View className="p-4 rounded-2xl" style={{ backgroundColor: "rgba(255,255,255,0.04)", borderWidth: 1, borderColor: "rgba(255,255,255,0.1)", borderStyle: "dashed" }}>
              <View className="flex items-center gap-2 mb-2"><Sun size={16} color="#FBBF24" /><Text className="text-white font-bold text-sm"><Text>Calculez votre retour sur investissement</Text></Text></View>
              <Text className="text-gray-400 text-xs mb-3"><Text>Un panneau solaire de 3kWh peut vous faire économiser 90 000 FCFA/mois</Text></Text>
              <Pressable className="w-full py-2 rounded-xl text-sm font-bold" style={{ backgroundColor: "#FBBF24" }}>
                <Text>Simuler mon installation</Text></Pressable>
            </View>
          </>
        )}
      </View>
