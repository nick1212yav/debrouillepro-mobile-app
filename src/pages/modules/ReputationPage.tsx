...
function BadgeCard({ badge, onClick }: { badge: Badge; onClick: () => void }) {
  ...
  return (
    <Pressable
      onPress={onClick}
      className="relative rounded-2xl p-3 flex flex-col items-center gap-2 text-center"
      style={{ backgroundColor: locked ? "rgba(255,255,255,0.03)" : rarity.bg, borderColor: "rgba(255,255,255,0.06)", borderStyle: "solid", opacity: locked ? 0.6 : 1 }}
    >
      ...
      <View className="relative w-12 h-12 rounded-xl flex items-center justify-center text-2xl"
        style={{ backgroundColor: locked ? "rgba(255,255,255,0.05)" : `${rarity.color}15` }}>
        <Text className={locked ? "grayscale opacity-50" : ""}>{badge.emoji}</Text>
        {locked && (
          <View className="absolute inset-0 rounded-xl flex items-center justify-center" style={{ backgroundColor: "rgba(0,0,0,0.5)" }}>
      ...
      <Text className={`text-[11px] font-bold leading-tight ${locked ? "text-white/30" : "text-white/90"}`}>
        {badge.nom}
      </Text>

      <Text className="px-1.5 py-0.5 rounded-full text-[8px] font-black"
        style={{ backgroundColor: `${rarity.color}20`, color: rarity.color }}>
        {rarity.label}
      </Text>

      {inProgress && badge.progression !== undefined && (
        <View className="w-full h-1 rounded-full overflow-hidden" style={{ backgroundColor: "rgba(255,255,255,0.1)" }}>
          <View
...
function BadgeModal({ badge, onClose }: { badge: Badge; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        <View className="absolute top-0 left-1/2 -translate-x-1/2 w-48 h-24"
          style={{  }} />
        ...
        <View className="flex flex-col items-center gap-3 mb-5">
          <View
            ...
            style={{ backgroundColor: rarity.bg, borderStyle: "solid" }}>
            {badge.emoji}
            {badge.status === "obtenu" && (
              <View className="absolute -top-2 -right-2 w-7 h-7 rounded-full flex items-center justify-center"
                style={{  }}>
          ...
          <View className="text-center">
            <Text className="text-xl font-black text-white">{badge.nom}</Text>
            <Text className="inline-block px-3 py-1 rounded-full text-xs font-bold mt-1"
              style={{ backgroundColor: rarity.bg, color: rarity.color }}>
              {rarity.label} · +{badge.xpReward} XP
            </Text>
          </View>
        </View>
        ...
        {badge.status === "en_cours" && badge.progression !== undefined && (
          <View className="mb-5">
            <View className="flex justify-between text-xs text-white/50 mb-2">
              <Text>Progression</Text>
              <Text className="font-bold" style={{ color: rarity.color }}>{badge.progression}%</Text>
            </View>
            <View className="h-3 rounded-full overflow-hidden" style={{ backgroundColor: "rgba(255,255,255,0.08)" }}>
              <View
                className="h-full rounded-full"
                style={{  }}
              />
        ...
        {badge.status === "obtenu" && badge.obtenusLe && (
          <View className="flex items-center justify-center gap-2 mb-5 px-4 py-3 rounded-2xl"
            style={{ backgroundColor: "rgba(16,185,129,0.1)", borderWidth: 1, borderColor: "rgba(16,185,129,0.2)", borderStyle: "solid" }}>
            <CheckCircle size={14} className="text-green-400" />
            <Text className="text-sm text-green-400 font-semibold">Obtenu le {badge.obtenusLe}</Text>
          </View>
        )}
        ...
        <Pressable onPress={onClose}
          ...
          style={{  }}>
          {badge.status === "obtenu" ? "Partager ce badge" : badge.status === "en_cours" ? "Continuer la progression" : "Voir comment débloquer"}
        </Pressable>
      </Pressable>
    </Pressable>
  );
...
function ClassementRow({ entry }: { entry: LeaderboardEntry }) {
  ...
  return (
    <View
      className={`flex items-center gap-3 px-4 py-3 rounded-2xl ${entry.isMe ? "border" : ""}`}
      style={{ backgroundColor: entry.isMe ? "rgba(139,92,246,0.1)" : "rgba(255,255,255,0.03)", borderColor: "rgba(139,92,246,0.3)", borderStyle: "solid" }}>
      ...
      <Image className="w-10 h-10 rounded-xl object-cover flex-shrink-0"
        style={{ borderColor: "rgba(139,92,246,0.5)", borderStyle: "solid" }}  source={{ uri: entry.avatar || "https://images.unsplash.com/photo-1646658104783-2eec2433c1d1?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=80" }} accessibilityLabel={entry.nom}/>
      <View className="flex-1 min-w-0">
        <View className="flex items-center gap-1.5">
          <Text className="text-sm font-bold text-white truncate">{entry.nom}</Text>
          {entry.isMe && <Text className="text-[9px] font-black text-purple-400 px-1 py-0.5 rounded-md" style={{ backgroundColor: "rgba(139,92,246,0.2)" }}>Vous</Text>}
        </View>
        <View className="flex items-center gap-2 mt-0.5">
          <Text className="text-[10px]">{niveau.emoji}</Text>
          <Text className="text-[10px] text-white/50">{entry.niveau}</Text>
          <Text className="text-[10px] text-white/30">·</Text>
          <Text className="text-[10px] text-white/40">{entry.badges} badges</Text>
        </View>
      </View>
      <View className="text-right flex-shrink-0">
        <Text className="text-sm font-black" style={{ color: rang <= 3 ? rankColor : "rgba(255,255,255,0.6)" }}>
          {entry.xp.toLocaleString()}
        </Text>
        <Text className="text-[9px] text-white/30">XP</Text>
      </View>
    </View>
  );
}
...
function ReputationSkeleton() {
  return (
    <View className="h-full w-full flex flex-col overflow-hidden p-5 gap-4"
      style={{  }}>
...
function ReputationPageInner({ onBack }: { onBack: () => void }) {
  ...
  return (
    <View className="relative h-full w-full flex flex-col overflow-hidden"
      style={{  }}>

      {/* Ambient glows */}
      <View className="absolute top-0 right-0 w-64 h-64 rounded-full"
        style={{  }} />
      <View className="absolute bottom-16 left-0 w-48 h-48 rounded-full"
        style={{  }} />

      {/* Header */}
      <View className="flex-shrink-0 px-5 pt-6 pb-0">
        <View className="flex items-center gap-3 mb-5">
          ...
          <View>
            <Text className="text-lg font-black text-white">Réputation & Badges</Text>
            <Text className="text-[11px] text-white/40">{myBadgeCount > 0 ? myBadgeCount : obtainedCount} badges obtenus{myRank > 0 ? ` · rang #${myRank} cette semaine` : ""}</Text>
          </View>
        </View>
        ...
        <View
          ...
          <View className="flex items-center gap-4 mb-4">
            <View className="relative">
              <Image
                ...
                style={{ borderStyle: "solid" }}  source={{ uri: myAvatar }} accessibilityLabel={myName}/>
              <View className="absolute -bottom-1 -right-1 w-7 h-7 rounded-xl flex items-center justify-center text-base"
                style={{ backgroundColor: `${niveau.color}20`, borderStyle: "solid" }}>
                {niveau.emoji}
              </View>
            </View>
            <View className="flex-1">
              <Text className="text-base font-black text-white">{myName}</Text>
              <Text className="text-xs font-bold px-2 py-0.5 rounded-full"
                style={{ backgroundColor: `${niveau.color}18`, color: niveau.color }}>
                {niveau.nom}
              </Text>
              <View className="flex items-center gap-3 mt-2">
                <Text className="text-xs text-white/50 flex items-center gap-1"><Star size={10} className="text-yellow-400" /> {myXp.toLocaleString()} XP</Text>
                <Text className="text-xs text-white/50 flex items-center gap-1"><Award size={10} className="text-purple-400" /> {myBadgeCount > 0 ? myBadgeCount : obtainedCount} badges</Text>
                {myRank > 0 && <Text className="text-xs text-white/50 flex items-center gap-1"><Crown size={10} className="text-orange-400" /> Rang #{myRank}</Text>}
              </View>
            </View>
          </View>
          ...
          {nextNiveau && (
            <View>
              <View className="flex justify-between text-xs text-white/40 mb-2">
                <Text>{niveau.nom}</Text>
                <Text className="font-semibold" style={{ color: niveau.color }}>{xpToNext.toLocaleString()} XP pour {nextNiveau.nom}</Text>
              </View>
              <View className="h-2.5 rounded-full overflow-hidden" style={{ backgroundColor: "rgba(255,255,255,0.07)" }}>
                <View
                  className="h-full rounded-full"
                  style={{  }}
                />
          ...
          <View className="gap-2 mt-4">
            {[
              ...
              const Icon = s.icon;
              return (
                <View key={s.label} className="flex flex-col items-center gap-0.5 p-2 rounded-xl"
                  style={{ backgroundColor: "rgba(255,255,255,0.04)" }}>
                  <Icon size={13} style={{ color: s.color }} />
                  <Text className="text-sm font-black text-white">{s.value}</Text>
                  <Text className="text-[9px] text-white/40">{s.label}</Text>
                </View>
              );
            })}
          </View>
          ...
          {TABS.map((t) => {
            ...
            return (
              <Pressable key={t.id} onPress={() => setTab(t.id)}
                ...
                style={{ backgroundColor: active ? "rgba(245,158,11,0.18)" : "rgba(255,255,255,0.05)", borderColor: "rgba(245,158,11,0.35)", borderStyle: "solid" }}>
      ...
      <View className="flex-1 overflow-y-auto px-5 pb-8" style={{  }}>
        <>
            ...
            <View key="badges">
              <View className="gap-2 mb-4">
                {[
                  ...
                  <View key={s.label} className="rounded-2xl py-3 flex flex-col items-center gap-0.5"
                    ...
                    <Text className="text-[10px] text-white/40">{s.label}</Text>
                  </View>
                ))}
              </View>

              <View className="flex gap-2 overflow-x-auto pb-3 mb-3" style={{  }}>
          ...
          {tab === "classement" && (
            <View key="classement">
              <View className="flex gap-2 mb-4">
                {["Cette semaine", "Ce mois", "Tout temps"].map((p, i) => (
                  <Pressable key={p}
                    className="flex-1 py-2 rounded-xl text-xs font-bold"
                    style={{ backgroundColor: i === 0 ? "rgba(245,158,11,0.18)" : "rgba(255,255,255,0.05)" }}>
                    {p}
                  </Pressable>
                ))}
              </View>
              ...
              {classementData.length >= 3 && (
                <View className="flex items-end justify-center gap-3 mb-5 px-2">
                  {/* 2nd */}
                  <View
                    className="flex flex-col items-center gap-1 flex-1">
                    <Image className="w-12 h-12 rounded-2xl object-cover border-2" style={{ borderColor: "#9CA3AF" }}  source={{ uri: classementData[1].avatar }} accessibilityLabel=""/>
                    <Text className="text-2xl">🥈</Text>
                    <Text className="text-[10px] text-white/70 font-bold text-center">{classementData[1].nom.split(" ")[0]}</Text>
                    <Text className="text-[10px] text-white/40">{classementData[1].xp.toLocaleString()} XP</Text>
                  </View>
                  {/* 1st */}
                  <View
                    className="flex flex-col items-center gap-1 flex-1 mb-3">
                    <View className="relative">
                      <Image className="w-16 h-16 rounded-2xl object-cover border-2" style={{ borderColor: "#F59E0B" }}  source={{ uri: classementData[0].avatar }} accessibilityLabel=""/>
                      <Text className="absolute -top-3 left-1/2 -translate-x-1/2 text-lg">👑</Text>
                    </View>
                    <Text className="text-2xl">🥇</Text>
                    <Text className="text-[10px] text-yellow-400 font-black text-center">{classementData[0].nom.split(" ")[0]}</Text>
                    <Text className="text-[10px] text-yellow-400/70">{classementData[0].xp.toLocaleString()} XP</Text>
                  </View>
                  {/* 3rd */}
                  <View
                    className="flex flex-col items-center gap-1 flex-1">
                    <Image className="w-12 h-12 rounded-2xl object-cover border-2" style={{ borderColor: "#F97316" }}  source={{ uri: classementData[2].avatar }} accessibilityLabel=""/>
                    <Text className="text-2xl">🥉</Text>
                    <Text className="text-[10px] text-white/70 font-bold text-center">{classementData[2].nom.split(" ")[0]}</Text>
                    <Text className="text-[10px] text-white/40">{classementData[2].xp.toLocaleString()} XP</Text>
                  </View>
                </View>
              )}
              ...
              <View className="flex flex-col gap-2">
                {classementData.slice(3).map((entry, i) => (
                  <View key={entry.rang}>
              ...
              {classementData.length === 0 && (
                <View className="flex flex-col items-center gap-3 py-10 text-center">
                  <Trophy size={32} className="text-white/20" />
                  <Text className="text-sm text-white/40">Le classement sera disponible prochainement</Text>
                </View>
              )}
              ...
              {myRank > 0 && (
                <View className="mt-4 p-4 rounded-2xl" style={{ backgroundColor: "rgba(139,92,246,0.08)", borderWidth: 1, borderColor: "rgba(139,92,246,0.2)", borderStyle: "solid" }}>
                  <Text className="text-xs text-white/50 mb-1">Votre position cette semaine</Text>
                  <View className="flex items-center gap-3">
                    <Text className="text-2xl font-black text-purple-400">#{myRank}</Text>
                    <View>
                      <Text className="text-sm font-bold text-white">{myXp.toLocaleString()} XP</Text>
                      <Text className="text-[11px] text-white/40">Continuez pour monter dans le classement</Text>
                    </View>
                    <TrendingUp size={20} className="text-green-400 ml-auto" />
                  </View>
                </View>
              )}
            </View>
          ...
          {tab === "historique" && (
            <View key="historique">
              {/* XP earned this week */}
              <View className="rounded-2xl p-4 mb-4 flex items-center gap-4"
                ...
                <View>
                  <Text className="text-xs text-white/40">XP gagnés cette semaine</Text>
                  <Text className="text-2xl font-black text-yellow-400">+{weekXp} XP</Text>
                </View>
              </View>
              ...
              {events && events.length > 0 ? (
                <View className="flex flex-col gap-2">
                  {events.map((event, i) => (
                    <View key={event._id}
                      ...
                      <View className="w-10 h-10 rounded-xl flex items-center justify-center text-xl flex-shrink-0"
                        style={{ backgroundColor: "rgba(255,255,255,0.06)" }}>
                        {eventTypeToIcon(event.type)}
                      </View>
                      <View className="flex-1 min-w-0">
                        <Text className="text-sm text-white/80 font-medium truncate">{event.description}</Text>
                        <Text className="text-[10px] text-white/35 flex items-center gap-1"><Clock size={9} /> {formatEventTime(event._creationTime)}</Text>
                      </View>
                      <Text className="flex-shrink-0 text-xs font-black px-2 py-1 rounded-xl"
                ...
                <View className="flex flex-col items-center gap-3 py-8 text-center">
                  ...
                  <Text className="text-sm text-white/40">Aucune activité récente</Text>
                  <Text className="text-xs text-white/30">Vos contributions apparaîtront ici</Text>
                </View>
              )}
              ...
              <View className="mt-4 rounded-2xl p-4" style={{ backgroundColor: "rgba(139,92,246,0.08)", borderWidth: 1, borderColor: "rgba(139,92,246,0.2)", borderStyle: "solid" }}>
                <View className="flex items-center gap-2 mb-3">
                  ...
                  <Text className="text-xs font-bold text-white">Comment gagner plus de XP</Text>
                </View>
                {[
...
export default function ReputationPage({ onBack }: ReputationPageProps) {
  return (
    <>
      ...
      <Unauthenticated>
        <View className="h-full w-full flex flex-col items-center justify-center gap-4 p-6"
          style={{  }}>
          ...
          <Award size={48} className="text-yellow-400/60" />
          <Text className="text-sm text-white/50 text-center"><Text>Connectez-vous pour voir votre réputation et vos badges</Text></Text>
          <SignInButton />
        </View>
      </Unauthenticated>
      <Authenticated>
