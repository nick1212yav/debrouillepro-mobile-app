...
function ReceiveModal({ onClose }: { onClose: () => void }) {
  ...
  const copy = () => {
    undefined.writeText(phone).catch(() => {});
    setCopied(true);
    setTimeout(() => setCopied(false), 2000);
  };
  return (
    <View
      className="fixed inset-0 z-40 flex items-end justify-center" style={{ backgroundColor: "rgba(0,0,0,0.6)" }}>
      <View
        ...
        <View className="flex items-center justify-between mb-5">
          <Text className="text-base font-bold text-white">Recevoir un paiement</Text>
          <Pressable onPress={onClose} className=""><X size={18} className="text-white/60" /></Pressable>
        </View>
        <View className="flex justify-center mb-4">
          <View className="p-4 rounded-2xl" style={{ backgroundColor: "white" }}>
            <View className="w-40 h-40 gap-0.5">
        ...
        <View className="flex items-center justify-center gap-2 mb-5">
          ...
          <Text className="text-base font-bold text-white">{phone}</Text>
          <Pressable onPress={copy} className="">
            {copied ? <Check size={14} className="text-green-400" /> : <Copy size={14} className="text-white/40" />}
          </Pressable>
        </View>
        <View className="flex gap-2">
          {OPERATORS.map(op => (
            <View key={op.id} className="flex-1 py-2 rounded-xl text-[10px] font-semibold text-center" style={{ backgroundColor: op.bg }}>
...
function PaiementInner({ onBack }: { onBack: () => void }) {
  ...
  async function handleSend() {
    ...
    try {
      ...
      setSendAmt("");
      setSendDesc("");
      UIService.openToast("Transfert envoyé !", "success");
    } catch {
  ...
  async function handleTopup(opId: string) {
    ...
    try {
      ...
      setShowTopup(false);
      UIService.openToast("Compte rechargé !", "success");
    } catch {
  ...
  return (
    <View className="h-full flex flex-col" style={{  }}>
      <View className="px-5 pt-5 pb-4 flex-shrink-0">
        <View className="flex items-center gap-3 mb-5">
          ...
          <View>
            <Text className="text-xl font-bold text-white">Mobile Money Pro</Text>
            <Text className="text-xs" style={{ color: "#10B981" }}>Orange · MTN · Airtel · Wave</Text>
          </View>
          <View className="ml-auto flex gap-2">
            <Pressable onPress={() => setBalanceVisible(v => !v)} className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ backgroundColor: "rgba(255,255,255,0.07)" }}>
              {balanceVisible ? <Eye size={14} className="text-white/60" /> : <EyeOff size={14} className="text-white/60" />}
            </Pressable>
            <Pressable onPress={() => setShowReceive(true)} className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ backgroundColor: "rgba(16,185,129,0.15)", borderWidth: 1, borderColor: "rgba(16,185,129,0.3)", borderStyle: "solid" }}>
        ...
        {/* Balance Card */}
        <View
          className="rounded-3xl p-5 mb-3 overflow-hidden relative"
          ...
          <View className="flex items-start justify-between mb-1">
            <View>
              <Text className="text-xs text-green-300/60 mb-1">Solde Débrouille Pay</Text>
              <Text key={balanceVisible ? "show" : "hide"} className="text-4xl font-black text-white">
                {balanceVisible ? <>{formatFCFA(balance)}</> : <Text className="tracking-widest">••••••</Text>}
              </Text>
            </View>
            <View className="text-right">
              <Text className="text-[10px] text-white/40 mb-1">Activité</Text>
              <View className="h-8 w-20">
                <Sparkline data={sparkData.length > 0 ? sparkData : [0, 0]} />
              </View>
            </View>
          </View>
          <View className="flex gap-2 mt-3 pt-3" style={{ borderTopWidth: 1, borderTopColor: "rgba(255,255,255,0.07)", borderTopStyle: "solid" }}>
            <View className="flex-1 flex items-center gap-1.5">
              ...
              <View>
                <Text className="text-[9px] text-white/40">Reçu</Text>
                <Text className="text-xs font-bold text-green-400">{formatFCFA(monthIn)}</Text>
              </View>
            </View>
            <View className="w-px" style={{ backgroundColor: "rgba(255,255,255,0.08)" }} />
            <View className="flex-1 flex items-center gap-1.5">
              <View className="w-6 h-6 rounded-lg flex items-center justify-center" style={{ backgroundColor: "rgba(239,68,68,0.2)" }}>
                <TrendingDown size={12} style={{ color: "#EF4444" }} />
              </View>
              <View>
                <Text className="text-[9px] text-white/40">Dépensé</Text>
                <Text className="text-xs font-bold text-red-400">{formatFCFA(monthOut)}</Text>
              </View>
            </View>
          </View>
          <View className="flex gap-2 mt-3">
            {[
              ...
              <Pressable key={label} onPress={action} className="flex-1 flex flex-col items-center gap-1.5 py-2.5 rounded-2xl"
                style={{ backgroundColor: "rgba(255,255,255,0.1)" }}>
                <Icon size={16} style={{ color }} />
                <Text className="text-[10px] text-white/70 font-medium">{label}</Text>
              </Pressable>
            ))}
          </View>
        </View>
        ...
        <View className="flex gap-2">
          {OPERATORS.map((op, i) => (
            <Pressable key={op.id}
              ...
              style={{ backgroundColor: op.bg, borderStyle: "solid" }}>
              {op.logo} {op.shortName}
            </Pressable>
          ))}
        </View>
      ...
      {/* Transactions */}
      <View className="flex-1 overflow-y-auto px-5 pb-6" style={{  }}>
        <View className="flex gap-2 mb-4 overflow-x-auto pb-1" style={{  }}>
          {FILTER_LABELS.map(({ key, label }) => (
            <Pressable key={key} onPress={() => setFilter(key)}
              className="flex-shrink-0 px-3 py-1.5 rounded-xl text-xs font-semibold"
              style={{ backgroundColor: filter === key ? "rgba(16,185,129,0.2)" : "rgba(255,255,255,0.06)", borderColor: "rgba(16,185,129,0.4)", borderStyle: "solid" }}>
              {label}
            </Pressable>
          ))}
        </View>
        ...
        <View className="flex items-center justify-between mb-3">
          ...
          <View className="flex items-center gap-1">
            ...
            <Text className="text-[10px] text-green-400">Sécurisé</Text>
          </View>
        </View>
              ...
              return (
                <View key={tx._id}
                  ...
                  <View className="relative">
                    <View className="w-10 h-10 rounded-full flex items-center justify-center text-lg" style={{ backgroundColor: op.bg }}>
                      {op.logo}
                    </View>
                  </View>
                  <View className="flex-1 min-w-0">
                    <Text className="text-sm font-semibold text-white truncate">{tx.description}</Text>
                    <View className="flex items-center gap-1.5 mt-0.5">
                      <Text className="text-[9px] px-1.5 py-0.5 rounded-full font-medium" style={{ backgroundColor: "rgba(255,255,255,0.07)", color: "rgba(255,255,255,0.4)" }}>
                        {tx.type}
                      </Text>
                      <Clock size={9} className="text-white/25" />
                      <Text className="text-[10px] text-white/35 truncate">
                        {tx.completedAt ? new Date(tx.completedAt).toLocaleDateString("fr-FR", { day: "numeric", month: "short" }) : "—"}
                      </Text>
                    </View>
                  </View>
                  <View className="text-right flex-shrink-0">
                    <Text className="text-sm font-black" style={{ color: tx.isIn ? "#10B981" : "#EF4444" }}>
                      {tx.isIn ? "+" : "-"}{formatFCFA(tx.amount)}
                    </Text>
                  </View>
                </View>
              );
              ...
              <View className="space-y-3 mb-4">
                <View className="text-center mb-4">
                  <TextInput value={sendAmt} onChangeText={text => setSendAmt(text)}
                    placeholder="0" className="bg-transparent text-5xl font-black text-white text-center w-40 outline-none" inputMode="numeric"  keyboardType="numeric"/>
                  <Text className="text-white/40 text-sm">FCFA</Text>
                </View>
                <View className="flex gap-2 flex-wrap mb-3">
                  {[1000, 2000, 5000, 10000].map(v => (
                    <Pressable key={v} onPress={() => setSendAmt(String(v))}
                      className="flex-1 py-1.5 rounded-xl text-xs font-semibold"
                      style={{ backgroundColor: sendAmt === String(v) ? "rgba(16,185,129,0.2)" : "rgba(255,255,255,0.07)", borderColor: "rgba(16,185,129,0.4)", borderStyle: "solid" }}>
                      {formatFCFA(v)}
                    </Pressable>
                  ))}
                </View>
                <TextInput value={sendDesc} onChangeText={text => setSendDesc(text)} placeholder="Description (optionnel)…"
                  className="w-full px-4 py-2.5 rounded-xl text-sm text-white placeholder:text-white/30"
                  style={{ backgroundColor: "rgba(255,255,255,0.07)", borderWidth: 1, borderColor: "rgba(255,255,255,0.1)", borderStyle: "solid" }} />
                <Text className="text-xs text-white/40">Opérateur</Text>
                <View className="flex gap-2">
                  {OPERATORS.map(op => (
                    <Pressable key={op.id} onPress={() => setSelectedOp(op.id)}
                      className="flex-1 py-2 rounded-xl text-[10px] font-semibold"
                      style={{ backgroundColor: selectedOp === op.id ? op.bg : "rgba(255,255,255,0.05)", borderColor: "rgba(255,255,255,0.07)", borderStyle: "solid" }}>
                      {op.logo} {op.shortName}
                    </Pressable>
                  ))}
                </View>
              </View>
              <Pressable onPress={handleSend} disabled={!sendAmt || parseFloat(sendAmt) <= 0}
        ...
        {showTopup && (
          <View
            ...
            <View
              ...
              <View className="flex flex-col gap-2">
                {OPERATORS.map(op => (
                  <Pressable key={op.id} onPress={() => handleTopup(op.id)}
                    className="flex items-center gap-3 p-3.5 rounded-2xl"
                    style={{ backgroundColor: op.bg, borderStyle: "solid" }}>
                    <Text className="text-2xl">{op.logo}</Text>
                    <View className="flex-1 text-left">
                      <Text className="text-sm font-bold" style={{ color: op.color }}>{op.name}</Text>
                      <Text className="text-xs text-white/40"><Text>Recharge instantanée (+10,000 FCFA)</Text></Text>
                    </View>
                    <Star size={14} style={{ color: op.color }} />
                  </Pressable>
                ))}
              </View>
            </View>
