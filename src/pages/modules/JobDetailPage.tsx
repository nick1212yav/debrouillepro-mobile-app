...
export default function JobDetailPage() {
  ...
  if (!job) {
    return (
      ...
      >
        ...
        <Briefcase size={48} className="text-white/15" />
        <Text className="text-white/40">Offre d'emploi introuvable</Text>
      </View>
    );
  }
  ...
  return (
    ...
    >
      {/* Header */}
      <View className="flex-shrink-0 px-5 pt-5 pb-3 flex items-center gap-3">
        <Pressable
          onPress={handleBack}
          className="w-10 h-10 rounded-2xl flex items-center justify-center"
          style={{ backgroundColor: "rgba(255,255,255,0.08)" }}
      ...
      {/* Content */}
      <View className="flex-1 overflow-y-auto px-5 pb-8 space-y-6">
        <View
          className="space-y-4"
        >
          {/* Entreprise */}
          <View className="flex items-center gap-4">
            {job.employerAvatar ? (
              <Image
               ...
               source={{ uri: job.employerAvatar }} accessibilityLabel={job.employerName || job.company}/>
            ) : (
              <View
                className="w-16 h-16 rounded-2xl flex items-center justify-center"
                style={{ backgroundColor: `${contractColor}20` }}
              >
                <Building size={24} style={{ color: contractColor }} />
              </View>
            )}
            <View>
              <Text className="text-white font-bold text-lg">{job.company}</Text>
              <Text className="text-xs text-white/40">
                {job.employerName || "Employeur"}
              </Text>
          ...
          <View className="gap-2">
            {job.salaryMin || job.salaryMax ? (
              <View className="bg-white/5 rounded-2xl p-3">
                <Text className="text-[10px] text-white/40">Salaire</Text>
                <Text className="text-white font-bold">
                  {job.salaryMin ? `${job.salaryMin.toLocaleString()}` : ""}
                  {job.salaryMax ? ` – ${job.salaryMax.toLocaleString()}` : ""}
                  {job.currency ? ` ${job.currency}` : ""}
                  {job.salaryMin || job.salaryMax ? "/mois" : ""}
                </Text>
              </View>
            ) : null}
            <View className="bg-white/5 rounded-2xl p-3">
              <Text className="text-[10px] text-white/40">Contrat</Text>
              <Text
            ...
            <View className="bg-white/5 rounded-2xl p-3">
              <Text className="text-[10px] text-white/40">Localisation</Text>
              <Text className="text-white font-bold flex items-center gap-1">
            ...
            <View className="bg-white/5 rounded-2xl p-3">
              <Text className="text-[10px] text-white/40">Statut</Text>
              ...
              <View className="flex flex-wrap gap-2">
                {job.skills.map((skill) => (
                  <Text
                    ...
                    style={{ backgroundColor: `${contractColor}15`, color: contractColor, borderStyle: "solid" }}
                  >
                    {skill}
                  </Text>
                ))}
