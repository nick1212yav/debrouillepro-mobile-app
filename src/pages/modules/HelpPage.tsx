...
function FaqItem({ question, answer, color }: { question: string; answer: string; color: string }) {
  ...
  return (
    <View className="border-b last:border-0" style={{ borderColor: "rgba(255,255,255,0.04)" }}>
      <Pressable
        onPress={() => setOpen(!open)}
        className="w-full flex items-center gap-3 px-4 py-3.5 text-left"
      >
        <Text className="flex-1 text-sm font-medium text-white/80">{question}</Text>
        <View>
...
export default function HelpPage({ onBack }: HelpPageProps) {
  ...
  return (
    <View className="flex flex-col h-full" style={{  }}>
      {/* Header */}
      <View
        className="px-5 pt-12 pb-4 shrink-0 border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
        <View className="flex items-center gap-3 mb-4">
          ...
          <View>
            ...
            <Text className="text-xs text-white/40">Centre d'aide Débrouille Pro</Text>
          </View>
        </View>
        ...
        <View className="relative">
          ...
          <TextInput
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-2xl text-sm text-white/80 placeholder:text-white/25 outline-none"
      ...
      <View className="flex-1 overflow-y-auto px-5 py-5 space-y-5">

        {/* Quick actions */}
        <View className="gap-2">
          {[
            { icon: MessageCircle, label: "Chat avec support", color: hex, action: () => UIService.openToast("Chat ouvert ! Temps d'attente : ~2 min", "success") },
            { icon: Mail,          label: "Envoyer un email",  color: "#3B82F6", action: () => UIService.openToast("support@debrouille.pro", "info") },
            { icon: Phone,         label: "Appeler",           color: "#10B981", action: () => UIService.openToast("+243 997 123 456 · Lun–Ven 8h–18h", "info") },
            { icon: AlertTriangle, label: "Signaler un bug",   color: "#F97316", action: () => UIService.openToast("Rapport de bug envoyé. Merci !", "success") },
        ...
        {/* Resources */}
        <View>
          <Text className="text-[10px] font-bold text-white/30 uppercase tracking-widest mb-2 px-1">Ressources</Text>
          <View className="rounded-3xl overflow-hidden" style={{ backgroundColor: "rgba(255,255,255,0.04)", borderWidth: 1, borderColor: "rgba(255,255,255,0.07)", borderStyle: "solid" }}>
            {[
              ...
              <Pressable key={label} onPress={() => UIService.openToast(`${label} — bientôt disponible`, "info")}
                ...
                <View className="flex-1">
                  ...
                  <Text className="text-[11px] text-white/30">{desc}</Text>
                </View>
        ...
        {/* Category filter */}
        <View>
          <Text className="text-[10px] font-bold text-white/30 uppercase tracking-widest mb-2 px-1">FAQ</Text>
          <View className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
            <Pressable onPress={() => setActiveCategory(null)}
              className="shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold"
              style={{ backgroundColor: !activeCategory ? `${hex}30` : "rgba(255,255,255,0.06)", borderColor: "rgba(255,255,255,0.08)", borderStyle: "solid" }}>
              <Text>Tout</Text></Pressable>
            {FAQ.map(cat => (
              <Pressable key={cat.category} onPress={() => setActiveCategory(activeCategory === cat.category ? null : cat.category)}
                className="shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold"
                style={{ backgroundColor: activeCategory === cat.category ? `${cat.color}30` : "rgba(255,255,255,0.06)", borderColor: "rgba(255,255,255,0.08)", borderStyle: "solid" }}>
                {cat.category}
              </Pressable>
            ))}
          </View>
        ...
        {filtered.length === 0 ? (
          ...
          <View key={cat.category}>
            <View className="flex items-center gap-2 mb-2 px-1">
              <View className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: cat.color }} />
              <Text className="text-[10px] font-bold uppercase tracking-widest" style={{ color: cat.color }}>{cat.category}</Text>
            </View>
            <View className="rounded-3xl overflow-hidden" style={{ backgroundColor: "rgba(255,255,255,0.04)", borderWidth: 1, borderColor: "rgba(255,255,255,0.07)", borderStyle: "solid" }}>
              {cat.items.map(item => (
        ...
        {/* Status */}
        <View className="rounded-2xl p-4 flex items-center gap-3"
          style={{ backgroundColor: "#10B98112", borderWidth: 1, borderColor: "#10B98130", borderStyle: "solid" }}>
          <View className="w-2.5 h-2.5 rounded-full bg-green-400 animate-pulse shrink-0" />
          <View>
            <Text className="text-sm font-semibold text-green-400"><Text>Tous les systèmes opérationnels</Text></Text>
            <Text className="text-[11px] text-white/30"><Text>Dernière vérification il y a 2 min · status.debrouille.pro</Text></Text>
          </View>
        </View>

        <Text className="text-center text-[10px] text-white/20 pb-2"><Text>Support disponible Lun–Sam · 8h–20h (heure de Kinshasa)</Text></Text>
      </View>
    </View>
  );
}
