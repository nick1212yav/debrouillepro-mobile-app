...
function ProfilePageInner({ onBack, onNavigate }: ProfilePageProps) {
  ...
  return (
    ...
    >
      ...
      >
        <View className="relative h-36 overflow-hidden">
          <Image
           ...
           source={{ uri: "https://images.unsplash.com/photo-1771539091406-feccb76ec825?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=600" }} accessibilityLabel="cover"/>
          <View
        ...
        <View className="px-5 -mt-12 relative z-10">
          <View className="flex items-end justify-between mb-3">
            <View className="relative">
              <UserAvatar user={user} size="w-20 h-20" className="border-4" />
              <Text className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full bg-green-400 border-2 border-[#020617]" />
              {isActive && (
                <View
                  className="absolute -top-2 -right-2 px-1.5 py-0.5 rounded-full text-[9px] font-black"
                  style={{ borderWidth: 1, borderColor: "rgba(255,255,255,0.2)", borderStyle: "solid" }}
                >
                  <Text>ACTIF</Text></View>
              )}
              <View
                className="absolute -top-2 -left-2 px-1.5 py-0.5 rounded-full text-[10px] font-black flex items-center gap-0.5"
                style={{ backgroundColor: "rgba(0,0,0,0.8)", borderStyle: "solid" }}
              >
                {LEVEL_ICONS[level]} {level}
              </View>
            </View>
            <View className="flex items-center gap-2 pb-1 flex-wrap justify-end">
              <Pressable
                onPress={() => setShowQR(true)}
                className="flex items-center gap-1.5 px-3 py-2 rounded-2xl text-xs font-bold"
                style={{ backgroundColor: `${accentHex}20`, borderStyle: "solid" }}
              >
                <Text>Carte</Text></Pressable>
              <Pressable
                onPress={() => onNavigate("dashboard")}
                className="flex items-center gap-1.5 px-3 py-2 rounded-2xl text-xs font-bold"
                style={{ backgroundColor: "rgba(139,92,246,0.2)", borderWidth: 1, borderColor: "rgba(139,92,246,0.3)", borderStyle: "solid" }}
              >
                <Text>Stats</Text></Pressable>
              <Pressable
                onPress={() => onNavigate("documents")}
                className="flex items-center gap-1.5 px-3 py-2 rounded-2xl text-xs font-bold"
                style={{ backgroundColor: "rgba(16,185,129,0.2)", borderWidth: 1, borderColor: "rgba(16,185,129,0.3)", borderStyle: "solid" }}
              >
                <Text>Coffre</Text></Pressable>
              <Pressable
              ...
              >
                <Text>Export</Text></Pressable>
              <Pressable
                onPress={() => onNavigate("reels")}
                className="flex items-center gap-1.5 px-3 py-2 rounded-2xl text-xs font-bold"
                style={{ backgroundColor: "rgba(236,72,153,0.2)", borderWidth: 1, borderColor: "rgba(236,72,153,0.3)", borderStyle: "solid" }}
              >
                <Play size={13} /> <Text>Reels</Text></Pressable>
              <Pressable
                onPress={() => onNavigate("creator-dashboard")}
                className="flex items-center gap-1.5 px-3 py-2 rounded-2xl text-xs font-bold"
                style={{ backgroundColor: "rgba(245,158,11,0.15)", borderWidth: 1, borderColor: "rgba(245,158,11,0.3)", borderStyle: "solid" }}
              >
                <Text>Créateur</Text></Pressable>
            </View>
          </View>

          <View className="flex items-center gap-2 mb-0.5">
            <Text className="text-xl font-black text-white">{displayName}</Text>
            <Text className="w-2.5 h-2.5 rounded-full bg-blue-400" />
          </View>
          <Text className="text-white/50 text-sm mb-2">{bio}</Text>
          <View className="flex items-center gap-3 mb-3 flex-wrap">
            ...
            {user?.email && (
              <View className="flex items-center gap-1">
                ...
                <Text className="text-xs text-white/40 truncate max-w-[160px]">
                  {user.email}
                </Text>
              </View>
            )}
            <View className="flex items-center gap-1">
              ...
              <Text className="text-xs text-white/40">Membre depuis 2024</Text>
            </View>
          </View>
          ...
          <View className="flex items-center gap-3 mb-3 flex-wrap">
            ...
            <View
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl"
              style={{ backgroundColor: "rgba(245,158,11,0.12)", borderWidth: 1, borderColor: "rgba(245,158,11,0.25)", borderStyle: "solid" }}
            >
          ...
          <View className="flex items-center gap-5 mb-3 flex-wrap">
            <View className="text-center">
              <View className="text-base font-black text-white">
                {followStats
              ...
              <View className="text-[10px] text-white/40"><Text>Abonnés</Text></View>
            </View>
            ...
            <View className="text-center">
              ...
              <View className="text-[10px] text-white/40"><Text>Abonnements</Text></View>
            </View>
            {myStreak && myStreak.currentStreak > 0 && (
              <>
                ...
                >
                  ...
                  <Text className="text-xs font-black text-orange-400">
          ...
          >
            {(
              ...
              <Pressable
                ...
                style={{ backgroundColor: tab === t ? `${accentHex}30` : "transparent", borderColor: "transparent", borderStyle: "solid" }}
              >
                {label}
              </Pressable>
            ))}
      ...
      <View
        className="flex-1 overflow-y-auto px-5 pb-8 pt-3"
        style={{  }}
      >
        <>
          {tab === "apercu" && (
                ...
                {MODULE_STATS.map((s) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      />
                      <Text className="text-lg font-black text-white">{s.value}</Text>
              ...
              >
                ...
                {prefs.favoriteModules.length > 0 ? (
                  <View className="flex gap-2 flex-wrap">
                    {prefs.favoriteModules.slice(0, 8).map((mod) => (
                      <View
                        key={mod}
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold"
                        style={{ backgroundColor: `${accentHex}15`, borderStyle: "solid" }}
                      >
                        <Text>📱</Text>{mod.charAt(0).toUpperCase() + mod.slice(1)}
                      </View>
                    ))}
                  </View>
                ) : (
                  <Text className="text-white/30 text-xs italic">
                    Aucun module favori — épinglez-en depuis le feed
                  </Text>
                )}
              </View>
                ...
                <View className="gap-2">
                  {BADGES.map((b) => (
                    <View
                      ...
                      style={{ backgroundColor: `${b.color}12`, borderStyle: "solid" }}
                    >
              ...
              >
                <View className="flex items-center justify-between mb-3">
                  ...
                  <Pressable
                    className="text-xs"
                    style={{  }}
                    onPress={() => UIService.openToast("Bientôt disponible !", "info")}
                  >
                    <Text>+ Ajouter</Text></Pressable>
                </View>
                <View className="flex items-center gap-3 opacity-40">
                  <View
                    className="w-8 h-8 rounded-xl flex items-center justify-center"
                    style={{ backgroundColor: "rgba(255,255,255,0.06)", borderWidth: 1, borderColor: "rgba(255,255,255,0.2)", borderStyle: "dashed" }}
                  >
                    <Link2 size={14} className="text-white/40" />
                  </View>
                  <Text className="text-xs text-white/40 italic">
                    Aucun lien ajouté
                  </Text>
                </View>
              </View>
            ...
            >
              ...
              >
                ...
                >
                  <Text>🥉</Text></View>
                <View>
                  <View className="text-white font-black text-xl">
                    {total.toLocaleString()} <Text>XP</Text></View>
                  <View className="text-white/50 text-xs">
                    <Text>Niveau</Text>{level} <Text>· Pro Débrouille</Text></View>
                  <View className="mt-1 flex gap-3 flex-wrap">
                    <Pressable
                      onPress={() => onNavigate("recompenses")}
                      className="text-xs font-bold flex items-center gap-1"
                      style={{  }}
                    >
                      <Text>Mes récompenses</Text><ChevronRight size={11} />
                    </Pressable>
                    <Pressable
                      onPress={() => onNavigate("badges")}
                      className="text-xs font-semibold flex items-center gap-1 text-indigo-400"
                    >
                      <Text>Badges</Text><ChevronRight size={11} />
                    </Pressable>
                    <Pressable
                      onPress={() => onNavigate("analytics")}
                      className="text-xs font-semibold flex items-center gap-1 text-emerald-400"
                    >
                      <Text>Analytics</Text><ChevronRight size={11} />
                    </Pressable>
                  </View>
                </View>
              </View>
            ...
            >
                ...
                <View className="space-y-2">
                  {leaderboard.map((entry, i) => {
                    ...
                    return (
                      ...
                      >
                        <Text className="text-xl w-8 text-center flex-shrink-0">
                          {rankEmoji}
                        </Text>
                        <View
                        ...
                        <View className="flex-1 min-w-0">
                          <Text className="text-sm font-bold text-white/85 truncate">
                            {entry.name}
                            {isMe && (
                              <Text
              ...
              {myStreak && myStreak.currentStreak > 0 && (
                <View
                  ...
                  style={{ backgroundColor: "rgba(249,115,22,0.12)", borderWidth: 1, borderColor: "rgba(249,115,22,0.25)", borderStyle: "solid" }}
                >
                  <Flame size={20} className="text-orange-400 flex-shrink-0" />
                  <View>
                    <Text className="text-sm font-black text-white">
                      Ton streak : {myStreak.currentStreak} jours 🔥
                    </Text>
  ...
  if (!isAuthenticated) {
    return (
      ...
      >
        ...
        <View className="text-center">
          <Text className="text-white font-black text-2xl mb-2">Mon Profil</Text>
          <Text className="text-white/40 text-sm leading-relaxed">
            <Text>Connectez-vous pour voir et gérer votre profil</Text></Text>
        </View>
        <SignInButton />
      </View>
    );
