...
function StatPill({
  ...
  return (
    <View className="flex flex-col items-center gap-0.5">
      ...
      <Text className="text-white/50 text-[10px]">{label}</Text>
    </View>
  );
}
    ...
    >
      {showWinner && entry.winner && (
        ...
        >
          ...
          <Text className="text-black font-bold text-[10px]">Gagnant</Text>
        </View>
      )}
      ...
      <View className="flex items-center gap-2 mb-3">
        ...
        <View>
          <Text className="text-white font-semibold text-sm">{entry.authorName}</Text>
      ...
      <View className="flex items-center gap-3">
        <Pressable
          onPress={() => onVote(entry._id)}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl"
          style={{ borderWidth: 1, borderColor: "rgba(255,255,255,0.1)", borderStyle: "solid" }}
        >
          <ThumbsUp
            size={13}
            className={entry.voted ? "text-white" : "text-white/60"}
          />
          <Text
            className={`text-xs font-bold ${entry.voted ? "text-white" : "text-white/60"}`}
          >
            {entry.votes}
          </Text>
        </Pressable>
        <Pressable
          className="ml-auto p-1.5 rounded-xl"
  ...
  return (
    ...
    >
      ...
      <View className="p-4">
        ...
        <View className="flex items-start justify-between mb-2">
          <View className="flex items-center gap-2">
            ...
            <View>
              ...
              <Text className="text-white/40 text-[11px]">{timeLabel}</Text>
            </View>
          </View>
          <Text
        ...
        {/* Stats row */}
        <View className="flex items-center gap-4 text-[11px] text-white/50">
...
function ChallengesSkeleton() {
  return (
    <View className="flex flex-col gap-3 px-4 pb-6">
...
export default function CoCreationPage({ onBack }: CoCreationPageProps) {
  ...
  const handleVote = async (entryId: Id<"coCreationEntries">) => {
    if (!isAuthenticated) {
      UIService.openToast("Connecte-toi pour voter", "error");
      return;
    }
    try {
  ...
  const handleSubmitContribution = async () => {
    ...
    if (!isAuthenticated) {
      UIService.openToast("Connecte-toi pour participer", "error");
      return;
    }
    try {
      await submitEntry({
      ...
      UIService.openToast("Contribution soumise !", "success");
    } catch {
  ...
  const handleCreateDefi = async () => {
    ...
    if (!isAuthenticated) {
      UIService.openToast("Connecte-toi pour créer un défi", "error");
      return;
    }
    try {
      ...
      setNewDefiDesc("");
      setTab("defis");
      UIService.openToast("Défi créé avec succès !", "success");
    } catch {
    ...
    return (
      ...
      >
        ...
        >
          ...
          <Text className="text-2xl">{selectedChallenge.emoji}</Text>
        </View>
        ...
        >
          ...
          >
            ...
            <View className="gap-2">
              <StatPill
                icon={Users}
                value={selectedChallenge.participantCount}
                label="participants"
                color="#8B5CF6"
              />
              <StatPill
                icon={ThumbsUp}
                value={typedEntries.reduce((sum, e) => sum + e.votes, 0)}
                label="votes"
                color="#10B981"
              />
              <StatPill
                icon={Gift}
                value={`+${selectedChallenge.rewardPoints}`}
                label="pts à gagner"
                color="#F59E0B"
              />
            </View>
          ...
          {/* Contribute button */}
          {selectedChallenge.status === "actif" && (
            <Pressable
              onPress={() => {
                if (!isAuthenticated) {
                  UIService.openToast("Connecte-toi pour participer", "error");
              ...
              className="mx-4 mt-3 w-[calc(100%-2rem)] py-3 rounded-2xl flex items-center justify-center gap-2"
              style={{  }}
            >
          ...
          {/* Entries list */}
          <View className="mx-4 mt-4 mb-6 flex flex-col gap-3">
            <Text className="text-white font-bold text-sm">
              {selectedChallenge.status === "vote"
                ? "🗳️ Vote pour la meilleure contribution"
                : "💬 Contributions"}
              <Text className="ml-2 text-white/40 font-normal text-xs">
                {typedEntries.length}
              </Text>
            </Text>
            {entries === undefined ? (
              ...
              <View className="text-center py-8 text-white/30 text-sm">
                <Text>Sois le premier à contribuer !</Text></View>
            ) : (
              typedEntries.map((e) => (
                <EntryCard
                  key={e._id}
        ...
        <>
          {showContribForm && (
            <>
              <Pressable
                onPress={() => setShowContribForm(false)}
              ...
              >
                ...
                <TextInput
                  value={contribText}
                  onChangeText={(text) => setContribText(text)}
                  placeholder="Décris ta contribution..."
                 
                  className="w-full rounded-xl px-3 py-3 text-sm text-white placeholder-white/30 outline-none"
                ...
                <Pressable
                  ...
                  disabled={!contribText.trim()}
                  className="mt-3 w-full py-3 rounded-2xl flex items-center justify-center gap-2 disabled:opacity-40"
                  style={{  }}
                >
  ...
  return (
    ...
    >
      {/* Ambient glows */}
      <View
        className="absolute top-0 left-1/2 -translate-x-1/2 w-64 h-40"
        style={{  }}
      />
          ...
          <View className="px-4 pb-6">
            ...
            >
              {[
                ...
                <Pressable
                  key={cat.id}
                  onPress={() => setStatusFilter(cat.id)}
              ...
              {challenges === undefined ? (
                ...
                <>
                  ...
                  >
                    ...
                    <View className="flex-1">
                      <Text className="text-white font-bold text-sm">
                        {filteredChallenges.length} défis
                      </Text>
                    ...
                    <View className="flex flex-col items-end">
                      <Text className="text-yellow-400 font-black text-sm">
                        {filteredChallenges
                  ...
                  <View className="flex flex-col gap-3">
                    {filteredChallenges.length === 0 ? (
                      <View className="text-center py-10 text-white/30 text-sm">
                        <Text>Aucun défi trouvé</Text></View>
                    ) : (
          ...
          <View className="px-4 pb-6">
            ...
            >
              <View className="flex items-center gap-3 mb-4">
                ...
                >
                  <Text>⚡</Text></View>
                <View>
                  <Text className="text-white font-bold">Lancer un défi</Text>
                  <Text className="text-white/40 text-xs">
              ...
              <View className="flex flex-col gap-3">
                <View>
                  <Text className="text-white/60 text-xs mb-1 block">
                    Titre du défi *
                  </Text>
                  <TextInput
                    value={newDefiTitle}
                    onChangeText={(text) => setNewDefiTitle(text)}
                ...
                <View>
                  ...
                  <TextInput
                    value={newDefiDesc}
                    onChangeText={(text) => setNewDefiDesc(text)}
                ...
                <Pressable
                  ...
                  disabled={!newDefiTitle.trim() || !newDefiDesc.trim()}
                  className="w-full py-3.5 rounded-2xl flex items-center justify-center gap-2 disabled:opacity-40 mt-1"
                  style={{  }}
                >
                  <Flame size={16} className="text-white" />
                  <Text className="text-white font-black">Lancer le défi</Text>
                </Pressable>
              </View>
            </View>
            ...
            >
              <Text className="text-white/50 text-xs font-semibold mb-2">
                <Text>💡 Conseils pour un bon défi</Text></Text>
              {[
