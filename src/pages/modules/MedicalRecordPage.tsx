...
export default function MedicalRecordPage() {
  ...
  return (
    ...
    >
      ...
      <View className="flex-shrink-0 px-4">
      ...
      <View
        className="flex-1 overflow-y-auto px-4 pb-8 pt-4 space-y-3"
        style={{  }}
      >
        {records === undefined ? (
          ...
          <View className="text-center py-12 text-white/40 text-sm">
            ...
            <Text>Aucun enregistrement médical</Text>
          </View>
        ) : (
          records.map((record: any) => (
            ...
            >
              <View className="w-10 h-10 rounded-xl bg-orange-500/20 flex items-center justify-center">
                <Pill size={18} className="text-orange-400" />
              </View>
              <View className="flex-1">
                <Text className="text-white font-medium text-sm">{record.title}</Text>
                <Text className="text-white/40 text-xs">
                  {record.date} <Text>·</Text>{record.doctor || "Médecin"}
                </Text>
              </View>
