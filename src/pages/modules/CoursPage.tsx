...
function CoursPageInner({ onBack }: Props) {
  ...
  const handleCompleteLesson = (courseId: string, lessonId: string) => {
    ...
    UIService.openToast("Leçon terminée !", "success");
  };
  ...
  if (activeLesson && selectedCourse) {
    ...
    return (
      <View className="h-full flex flex-col bg-gray-950 text-white overflow-hidden">
        <View className="flex items-center gap-3 px-4 pt-12 pb-4 border-b border-white/10">
          <Pressable onPress={() => setActiveLesson(null)} className="p-2 rounded-xl bg-white/10">
            <ArrowLeft size={20} />
          </Pressable>
          <View className="flex-1 min-w-0">
            <Text className="text-xs text-gray-400 truncate">{selectedCourse.title}</Text>
            <Text className="text-sm font-bold truncate">{activeLesson.title}</Text>
          </View>
          <Text className={`flex items-center gap-1 text-xs px-2 py-1 rounded-full border ${activeLesson.type === "video" ? "bg-blue-500/20 text-blue-400 border-blue-500/30" : activeLesson.type === "quiz" ? "bg-purple-500/20 text-purple-400 border-purple-500/30" : "bg-green-500/20 text-green-400 border-green-500/30"}`}>
            <LessonIcon size={11} />{activeLesson.type}
          </Text>
        </View>

        <View className="flex-1 overflow-y-auto p-4 pb-8 space-y-4">
          {/* Video placeholder */}
          {activeLesson.type === "video" && (
            <View className="relative rounded-2xl overflow-hidden bg-gray-900 border border-white/10">
              <Image className="w-full h-48 object-cover opacity-40"  source={{ uri: selectedCourse.cover }} accessibilityLabel=""/>
              <View className="absolute inset-0 flex flex-col items-center justify-center gap-3">
                <View className="w-16 h-16 rounded-full bg-white/20 flex items-center justify-center">
                  <Play size={28} className="text-white ml-1" />
                </View>
                <Text className="text-sm text-white/80">{activeLesson.duration}</Text>
              </View>
            </View>
          )}

          {/* Text lesson */}
          <View className="bg-white/5 rounded-2xl p-5 border border-white/10 space-y-4">
            <Text className="text-lg font-bold">{activeLesson.title}</Text>
            {activeLesson.type === "text" && (
              <View className="space-y-3 text-sm text-gray-300 leading-relaxed">
                <Text>Dans cette leçon, nous allons explorer les concepts fondamentaux liés à <strong className="text-white">{activeLesson.title.toLowerCase()}</strong>.</Text>
                <Text>Les points clés à retenir :</Text>
                <View className="space-y-2 pl-4">
                  {["Comprendre la théorie de base", "Appliquer les concepts à des exemples concrets", "Identifier les erreurs courantes et les éviter", "Mettre en pratique avec des exercices guidés"].map((point, i) => (
                    <View key={i} className="flex items-start gap-2">
                      <View className="w-1.5 h-1.5 rounded-full bg-cyan-400 mt-2 flex-shrink-0" />
                      {point}
                    </View>
                  ))}
                </View>
                <Text>Cette leçon est essentielle pour progresser vers les modules suivants. Prenez le temps de bien assimiler chaque concept avant de continuer.</Text>
              </View>
            )}
            {activeLesson.type === "quiz" && (
              <View className="space-y-3">
                <Text className="text-sm text-gray-400">Quiz de validation — 3 questions pour tester vos connaissances</Text>
                {["Quelle est la principale utilité de cette fonctionnalité ?", "Dans quel cas utilise-t-on cette approche ?", "Quelle est la bonne syntaxe ?"].map((q, i) => (
                  <View key={i} className="bg-white/5 rounded-xl p-3 border border-white/10">
                    <Text className="text-sm font-medium mb-3">Q{i + 1}. {q}</Text>
                    <View className="space-y-2">
                      {["Option A", "Option B", "Option C", "Option D"].map((opt, j) => (
                        <Pressable key={j} className="w-full text-left text-xs py-2 px-3 rounded-lg bg-white/5 border border-white/10">{opt}</Pressable>
                      ))}
                    </View>
                  </View>
                ))}
        ...
        <View className="p-4 border-t border-white/10">
          <Pressable
            onPress={() => handleCompleteLesson(selectedCourse.id, activeLesson.id)}
            className="w-full py-3 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-xl font-semibold flex items-center justify-center gap-2"
          >
            <CheckCircle2 size={18} />
            <Text>Marquer comme terminé</Text></Pressable>
        </View>
      </View>
    );
  }
    ...
    return (
      <View className="h-full flex flex-col bg-gradient-to-b from-gray-950 via-gray-900 to-gray-950 text-white overflow-hidden">
        ...
        <View className="relative h-52 flex-shrink-0">
          <Image className="w-full h-full object-cover"  source={{ uri: selectedCourse.cover }} accessibilityLabel={selectedCourse.title}/>
          <View className="absolute inset-0 bg-gradient-to-b from-black/50 to-black/85" />
          <Pressable onPress={() => setSelectedCourse(null)} className="absolute top-12 left-4 p-2 rounded-xl bg-black/40">
            <ArrowLeft size={20} />
          </Pressable>
          <View className="absolute bottom-4 left-4 right-4">
            <View className="flex items-center gap-2 mb-2">
              <Text className={`text-xs px-2 py-1 rounded-full border font-medium ${LEVEL_COLOR[selectedCourse.level]}`}>{selectedCourse.level}</Text>
              <Text className={`text-xs px-2 py-1 rounded-full bg-gradient-to-r ${cfg.gradient} text-white font-medium`}>{cfg.label}</Text>
            </View>
            <Text className="text-xl font-bold leading-tight">{selectedCourse.title}</Text>
            <View className="flex items-center gap-3 mt-1 text-xs text-white/70">
              <Text className="flex items-center gap-1"><Star size={12} className="text-yellow-400 fill-yellow-400" />{selectedCourse.rating}</Text>
              <Text className="flex items-center gap-1"><Users size={12} />{selectedCourse.students.toLocaleString("fr-FR")}</Text>
              <Text className="flex items-center gap-1"><Clock size={12} />{selectedCourse.duration}</Text>
            </View>
          </View>
        </View>
        ...
        <View className="flex-1 overflow-y-auto pb-24">
          ...
          <View className="px-4 pt-4 space-y-3">
            <View className="flex items-center gap-3 bg-white/5 rounded-xl p-3 border border-white/10">
              <View className={`w-10 h-10 rounded-full bg-gradient-to-br ${cfg.gradient} flex items-center justify-center text-sm font-bold flex-shrink-0`}>
                {selectedCourse.instructorAvatar}
              </View>
              <View>
                <Text className="text-xs text-gray-400">Instructeur</Text>
                <Text className="text-sm font-semibold">{selectedCourse.instructor}</Text>
              </View>
            </View>

            <Text className="text-sm text-gray-300 leading-relaxed">{selectedCourse.description}</Text>
            ...
            {selectedCourse.enrolled && totalLessons > 0 && (
              <View className="bg-white/5 rounded-2xl p-4 border border-white/10">
                <View className="flex items-center justify-between mb-2">
                  <Text className="text-sm font-semibold">Ma progression</Text>
                  <Text className={`text-sm font-bold ${cfg.color}`}>{selectedCourse.progress}%</Text>
                </View>
                ...
                <Text className="text-xs text-gray-500 mt-1">{doneLessons}/{totalLessons} leçons complétées</Text>
              </View>
            )}
          </View>
          ...
          {selectedCourse.modules.length > 0 && (
            <View className="px-4 mt-4 space-y-2">
              <Text className="text-sm font-semibold mb-1">Contenu du cours</Text>
              {selectedCourse.modules.map((mod) => {
                const isExpanded = expandedModules.has(mod.id);
                const modDone = mod.lessons.filter(l => l.done).length;
                return (
                  <View key={mod.id} className="bg-white/5 rounded-2xl overflow-hidden border border-white/10">
                    <Pressable
                      onPress={() => toggleModule(mod.id)}
                      className="w-full flex items-center gap-3 p-4"
                    >
                      <View className={`w-8 h-8 rounded-lg bg-gradient-to-br ${cfg.gradient} flex items-center justify-center flex-shrink-0`}>
                        <BookOpen size={14} className="text-white" />
                      </View>
                      <View className="flex-1 text-left">
                        <Text className="text-sm font-semibold">{mod.title}</Text>
                        <Text className="text-xs text-gray-400">{modDone}/{mod.lessons.length} leçons</Text>
                      </View>
                          ...
                          <View className="p-3 space-y-1">
                            {mod.lessons.map((lesson) => {
                              ...
                              return (
                                <Pressable
                                  key={lesson.id}
                                  onPress={() => !lesson.locked && setActiveLesson(lesson)}
                                  disabled={lesson.locked}
                                  className={`w-full flex items-center gap-3 p-3 rounded-xl transition-colors text-left ${lesson.locked ? "opacity-40 cursor-not-allowed" : "hover:bg-white/10 cursor-pointer"}`}
                                >
                                  <LIcon size={16} className={lesson.type === "video" ? "text-blue-400" : lesson.type === "quiz" ? "text-purple-400" : "text-green-400"} />
                                  <View className="flex-1 min-w-0">
                                    <Text className={`text-sm truncate ${lesson.done ? "line-through text-gray-500" : "text-white"}`}>{lesson.title}</Text>
        ...
        {/* CTA */}
        <View className="absolute bottom-0 left-0 right-0 p-4 bg-gray-950/90 border-t border-white/10">
          {selectedCourse.enrolled ? (
            <Pressable
              ...
              className={`w-full py-3 bg-gradient-to-r ${cfg.gradient} rounded-xl font-semibold cursor-pointer hover:opacity-90 transition-opacity flex items-center justify-center gap-2`}
            >
              <Play size={18} /> <Text>Continuer le cours</Text></Pressable>
          ) : (
            <Pressable
              onPress={() => handleEnroll(selectedCourse.id)}
              className={`w-full py-3 bg-gradient-to-r ${cfg.gradient} rounded-xl font-semibold cursor-pointer hover:opacity-90 transition-opacity flex items-center justify-center gap-2`}
            >
              <BookOpen size={18} /> <Text>S'inscrire gratuitement</Text></Pressable>
          )}
        </View>
      </View>
    );
  ...
  return (
    <View className="h-full flex flex-col bg-gradient-to-b from-gray-950 via-gray-900 to-gray-950 text-white overflow-hidden">
      {/* Header */}
      <View className="px-4 pt-12 pb-3 flex-shrink-0">
        <View className="flex items-center gap-3 mb-4">
          <Pressable onPress={onBack} className="p-2 rounded-xl bg-white/10">
            <ArrowLeft size={20} />
          </Pressable>
          <View className="flex-1">
            <Text className="text-xl font-bold">Cours & Catalogue</Text>
            <Text className="text-xs text-gray-400">{courses.length} cours disponibles</Text>
          </View>
        </View>
        ...
        <View className="relative mb-3">
          ...
          <TextInput
            ...
            className="w-full bg-white/10 rounded-xl pl-9 pr-4 py-3 text-sm text-white placeholder-gray-500 outline-none border border-white/10"
          />
          {searchQuery && (
        ...
        <View className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
          <Pressable
            ...
            className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-all cursor-pointer ${selectedCategory === "all" ? "bg-white text-gray-900" : "bg-white/10 text-gray-400 hover:bg-white/20"}`}
          >
          ...
          {(Object.keys(CATEGORY_CONFIG) as Category[]).map(cat => {
            ...
            return (
              <Pressable
                ...
                className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all cursor-pointer ${sel ? `bg-gradient-to-r ${cfg.gradient} text-white` : "bg-white/10 text-gray-400 hover:bg-white/20"}`}
              >
      ...
      <View className="flex-1 overflow-y-auto px-4 pb-8 space-y-5">
        {/* My courses */}
        {enrolled.length > 0 && !searchQuery && selectedCategory === "all" && (
          <View>
            <Text className="text-sm font-semibold mb-3 flex items-center gap-2">
              <Flame size={16} className="text-orange-400" /> En cours
            </Text>
            <View className="space-y-3">
              {enrolled.map((course, i) => {
                ...
                return (
                  <Pressable
                    ...
                    className="w-full flex items-center gap-3 bg-white/5 rounded-2xl p-3 border border-white/10 text-left"
                  >
                    <Image className="w-16 h-16 rounded-xl object-cover flex-shrink-0"  source={{ uri: course.cover }} accessibilityLabel=""/>
                    <View className="flex-1 min-w-0">
                      <Text className="text-sm font-semibold">{course.title}</Text>
                      <Text className="text-xs text-gray-400 mb-2">{course.instructor}</Text>
                      <View className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                        <View className={`h-full rounded-full bg-gradient-to-r ${cfg.gradient}`} style={{ width: `${course.progress}%` }} />
                      </View>
                      <Text className="text-xs text-gray-500 mt-1">{course.progress}% complété</Text>
                    </View>
        ...
        {!searchQuery && selectedCategory === "all" && (
          <View className="gap-2">
            {[
              ...
              <View key={s.label} className="bg-white/5 rounded-xl p-3 border border-white/10 text-center">
              ...
              return (
                ...
                >
                  <View className="relative h-36">
                    <Image className="w-full h-full object-cover"  source={{ uri: course.cover }} accessibilityLabel={course.title}/>
                    ...
                    <View className="absolute top-2 left-2 flex gap-1">
                      <Text className={`text-xs px-2 py-0.5 rounded-full border font-medium ${LEVEL_COLOR[course.level]}`}>{course.level}</Text>
                    </View>
                    {course.enrolled && (
                      <View className="absolute top-2 right-2 bg-green-500/30 border border-green-500/50 rounded-full px-2 py-0.5 text-[10px] text-green-400 font-medium flex items-center gap-1">
                        <Award size={10} /> <Text>Inscrit</Text></View>
                    )}
                    <View className="absolute bottom-2 left-3 right-3">
                      <Text className="text-sm font-bold">{course.title}</Text>
                    </View>
                  </View>
                  <View className="p-3">
                    <View className="flex items-center justify-between">
                      <View className="flex items-center gap-1 text-xs text-gray-400">
                        <View className={`w-5 h-5 rounded-full bg-gradient-to-br ${cfg.gradient} flex items-center justify-center text-[9px] font-bold`}>
                          {course.instructorAvatar}
                        </View>
                        {course.instructor}
                      </View>
                      <View className="flex items-center gap-3 text-xs text-gray-400">
                        <Text className="flex items-center gap-1"><Star size={11} className="text-yellow-400 fill-yellow-400" />{course.rating}</Text>
                        <Text className="flex items-center gap-1"><Clock size={11} />{course.duration}</Text>
                      </View>
                    </View>
                    {course.enrolled && (
                      <View className="mt-2">
                        ...
                        <Text className="text-xs text-gray-500 mt-1">{course.progress}<Text>% complété</Text></Text>
                      </View>
                    )}
                  </View>
                </Pressable>
