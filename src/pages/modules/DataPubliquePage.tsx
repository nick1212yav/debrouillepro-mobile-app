...
function AlertButton({ dataset }: { dataset: typeof DATASETS[0] }) {
  ...
  return (
    <Pressable onPress={handleToggle} className="w-8 h-8 rounded-full flex items-center justify-center" style={{ backgroundColor: isSet ? "rgba(245,158,11,.3)" : "rgba(255,255,255,.06)" }}><Bell size={14} color={isSet ? "#F59E0B" : "#9CA3AF"} /></Pressable>
  );
}
...
export default function DataPubliquePage({ onBack }: { onBack: () => void }) {
  ...
  return (
    <View className="h-full flex flex-col overflow-hidden" style={{  }}><View className="flex items-center gap-3 px-4 pt-12 pb-3"><Pressable onPress={onBack} className="p-2 rounded-full" style={{ backgroundColor: "rgba(255,255,255,.08)" }}><ArrowLeft size={18} color="white" /></Pressable><View className="flex-1"><Text className="text-white font-bold text-lg">Data Publique</Text><Text className="text-gray-400 text-xs">Open Data · Statistiques · Transparence</Text></View><View className="w-8 h-8 rounded-full flex items-center justify-center" style={{ backgroundColor: "rgba(99,102,241,.15)" }}><Database size={16} color="#6366F1" /></View></View><View className="flex gap-1 mx-4 mb-4 p-1 rounded-xl" style={{ backgroundColor: "rgba(255,255,255,.04)" }}>{["Indicateurs", "Données"].map(tab => (
        ...
        ))}</View><View className="flex-1 overflow-y-auto px-4 pb-6 space-y-4" style={{  }}>{activeTab === "Indicateurs" && (
          <>
            <Text className="text-gray-400 text-xs">Indicateurs macroéconomiques – Côte d'Ivoire 2025</Text>
            {INDICATORS.map((ind, i) => (
              <View key={ind.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * .07 }} className="flex items-center gap-4 p-4 rounded-2xl" style={{ backgroundColor: "rgba(255,255,255,.04)", borderWidth: 1, borderColor: "rgba(255,255,255,.08)", borderStyle: "solid" }}>
                <View className="w-12 h-12 rounded-2xl flex items-center justify-center" style={{ backgroundColor: `${ind.color}15` }}><BarChart2 size={22} color={ind.color} /></View>
                <View className="flex-1"><Text className="text-gray-400 text-xs">{ind.label}</Text><Text className="text-white font-bold text-xl">{ind.value}<Text className="text-sm font-normal text-gray-400">{ind.unit}</Text></Text></View>
                <View className="flex items-center gap-1">{ind.up ? <TrendingUp size={14} color={ind.color} /> : <TrendingDown size={14} color={ind.color} />}<Text className="font-bold text-sm" style={{ color: ind.color }}>{ind.trend}</Text></View>
              </View>
            ))}
            <View className="gap-3">{[{ icon: Globe, label: "Rang Afrique", value: "#4", color: "#F59E0B" }, { icon: Users, label: "IDH", value: "0.55", color: "#10B981" }, { icon: DollarSign, label: "PIB/hab.", value: "1 540$", color: "#6366F1" }].map(({ icon: Icon, label, value, color }) => (
          ...
          <>
            <View className="flex items-center gap-2 px-3 py-2 rounded-xl" style={{ backgroundColor: "rgba(255,255,255,.06)", borderWidth: 1, borderColor: "rgba(255,255,255,.08)", borderStyle: "solid" }}><Search size={14} color="#9CA3AF" /><TextInput value={search} onChangeText={value => setSearch(value)} placeholder="Rechercher des données..." className="flex-1 bg-transparent text-white text-sm outline-none" /></View>
            <View className="flex gap-2 overflow-x-auto pb-1" style={{  }}>{CATEGORIES.map(cat => <Pressable key={cat} onPress={() => setCatFilter(cat)} className="px-3 py-1.5 rounded-full text-xs font-medium" style={{ backgroundColor: catFilter === cat ? "#6366F1" : "rgba(255,255,255,.06)" }}>{cat}</Pressable>)}</View>
            {filtered.map((d, i) => (
              <View key={d.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * .07 }} className="flex items-center gap-3 p-4 rounded-2xl" style={{ backgroundColor: "rgba(255,255,255,.04)", borderWidth: 1, borderColor: "rgba(255,255,255,.08)", borderStyle: "solid" }}>
                <View className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ backgroundColor: `${d.color}20` }}><Database size={18} color={d.color} /></View>
                <View className="flex-1"><Text className="text-white font-semibold text-sm">{d.name}</Text><Text className="text-gray-400 text-xs">{d.source}· {d.updated}</Text><View className="flex items-center gap-2 mt-0.5"><Text className="text-xs px-2 py-0.5 rounded-full" style={{ backgroundColor: `${d.color}20`, color: d.color }}>{d.category}</Text><Text className="text-gray-500 text-xs">{d.downloads.toLocaleString()}DL</Text></View></View>
