    ...
    >
      ...
      <View className="flex-1 min-w-0">
        ...
        <View className="flex items-center gap-3 mt-1.5">
          <Text className="flex items-center gap-1 text-[10px] text-white/40">
            <Users size={9} /> {groupe.memberCount.toLocaleString()}
          </Text>
          {groupe.city && (
            <Text className="flex items-center gap-1 text-[10px] text-white/30">
              <MapPin size={9} /> {groupe.city}
            </Text>
          )}
        </View>
      </View>
...
function GroupeDetail({
  ...
  const handleJoin = async () => {
    try {
      ...
      UIService.openToast(result.joined ? "Groupe rejoint !" : "Groupe quitté", "success");
    } catch {
      UIService.openToast("Erreur lors de l'action", "error");
    }
  };

  const handlePost = async () => {
    // Fonction désactivée car `createGroupPost` n'existe pas encore.
    UIService.openToast("Publication dans les groupes : fonctionnalité à venir", "info");
    setShowCreate(false);
    setPostText("");
  };
  ...
  return (
    ...
    >
      ...
      <View className="flex-shrink-0 px-5 pt-6 pb-4">
        <View className="flex items-center gap-3 mb-4">
          ...
          <View className="flex-1 min-w-0">
            <View className="flex items-center gap-1.5">
              <Text className="text-base font-black text-white truncate">
                {groupe.name}
              </Text>
              {groupe.isPrivate && <Lock size={11} className="text-white/40" />}
            </View>
        ...
        <Text className="text-xs text-white/50 leading-relaxed mb-3">
          {groupe.description}
        </Text>
        <View className="flex flex-wrap gap-1.5 mb-4">
          {groupe.tags.map((t) => (
            <Text
              key={t}
            ...
            {groupe.isMember ? (
              <>
                ...
                <Pressable
                  ...
                  style={{  }}
                >
                  <Plus size={13} /> <Text>Publier</Text></Pressable>
                <Pressable
                  onPress={handleJoin}
                  className="px-3 py-2.5 rounded-xl text-xs font-semibold"
                  style={{ backgroundColor: "rgba(239,68,68,0.12)" }}
                >
                  <Text>Quitter</Text></Pressable>
              </>
            ) : (
              <Pressable
                onPress={handleJoin}
                ...
                style={{  }}
              >
                <Plus size={15} /> <Text>Rejoindre le groupe</Text></Pressable>
            )}
          </Authenticated>
          <Unauthenticated>
            <Pressable
              onPress={() =>
                UIService.openToast("Connectez-vous pour rejoindre le groupe", "info")
              }
              className="flex-1 flex items-center justify-center gap-1.5 py-3 rounded-xl text-sm font-bold text-white"
              style={{  }}
            >
      ...
      <View
        ...
        style={{  }}
      >
        {/* Message temporaire : posts désactivés */}
        <View className="flex flex-col items-center justify-center py-16 gap-3">
          ...
          <Text className="text-sm text-white/40 text-center">
            Les publications dans les groupes sont temporairement désactivées.
            <br />
          ...
          >
            ...
            >
              ...
              <TextInput
                ...
                className="w-full rounded-2xl px-4 py-3 text-sm text-white/90 outline-none"
                style={{ backgroundColor: "rgba(255,255,255,0.06)", borderWidth: 1, borderColor: "rgba(255,255,255,0.09)", borderStyle: "solid" }}
               multiline textAlignVertical="top"/>
              <Pressable
                onPress={handlePost}
                disabled={!postText.trim()}
                className="w-full mt-3 py-3 rounded-2xl text-sm font-bold text-white disabled:opacity-40"
                style={{  }}
              >
                <Text>Publier (désactivé)</Text></Pressable>
            </View>
          </View>
        )}
      </>
    ...
    >
      ...
      >
        ...
        <TextInput
          ...
          className="w-full rounded-2xl px-4 py-3 text-sm text-white/80 outline-none mb-3"
          style={{ backgroundColor: "rgba(255,255,255,0.06)", borderWidth: 1, borderColor: "rgba(255,255,255,0.09)", borderStyle: "solid" }}
         multiline textAlignVertical="top"/>
        <View className="mb-4">
          <Text className="text-xs text-white/50 mb-2">Catégorie</Text>
          <View className="flex flex-wrap gap-2">
            {CATS.map((c) => (
              <Pressable
                ...
                style={{ backgroundColor: categorie === c
                                      ? "rgba(139,92,246,0.25)"
                                      : "rgba(255,255,255,0.06)", borderColor: "rgba(139,92,246,0.4)", borderStyle: "solid" }}
              >
                {c}
              </Pressable>
            ))}
        ...
        <Pressable
          onPress={() => setPrive(!prive)}
          className="w-full flex items-center justify-between px-4 py-3 rounded-2xl mb-5"
        ...
        >
          <View className="flex items-center gap-2">
            ...
            <Text className="text-sm text-white/80">
              {prive ? "Groupe privé" : "Groupe public"}
            </Text>
          </View>
          <View
            className="w-10 h-5 rounded-full relative"
            style={{ backgroundColor: prive ? "#8B5CF6" : "rgba(255,255,255,0.15)" }}
          >
            <View
        ...
        <Pressable
          ...
          style={{  }}
        >
          <Text>Créer le groupe ✨</Text></Pressable>
      </View>
    </View>
  );
}
...
function GroupesPageInner({ onBack }: GroupesPageProps) {
  ...
  return (
    ...
    >
      ...
      <View className="flex-shrink-0 px-5 pt-6 pb-0">
        <View className="flex items-center gap-3 mb-5">
          ...
          <View>
            ...
            <Text className="text-[11px] text-white/40">
              {mesGroupes.length} groupes rejoints
            </Text>
          </View>
          <Authenticated>
            <Pressable
          ...
          {[
            ...
            return (
              ...
              >
                ...
                <Text className="text-[9px] text-white/40 text-center leading-tight">
                  {stat.label}
                </Text>
              </View>
            );
      ...
      <View
        className="flex-1 overflow-y-auto px-5 pb-8 flex flex-col gap-2.5"
        style={{  }}
      >
          ...
          <View className="flex flex-col items-center py-16 gap-3">
            ...
            <Authenticated>
              <Pressable
                ...
                style={{  }}
              >
                <Plus size={13} className="inline mr-1" /> <Text>Créer un groupe</Text></Pressable>
            </Authenticated>
          </View>
        ) : (
          filtered.map((g, i) => (
    ...
    <>
      <AuthLoading>
        <View
          ...
          style={{  }}
        >
          <View className="flex flex-col gap-3 px-5 w-full">
