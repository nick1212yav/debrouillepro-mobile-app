 ...
 * ============================================================================
 *
 * Le projet utilise actuellement une déclaration TypeScript de
 * lucide-react-native qui n'expose pas correctement les props JSX des icônes.
 *
 * On isole donc le cast à UNE SEULE frontière technique.
 *
 * Règles :
 * - aucune `any`
 * - aucun @ts-ignore
 * - aucune modification globale des typings
 * - les composants du fichier utilisent un contrat typé stable
 * - les icônes restent de vraies icônes Lucide natives
 * ========================================================================== */
...
type ContextIconComponent = ComponentType<ContextIconProps>;

function createContextIcon(icon: unknown): ContextIconComponent {
  return icon as ContextIconComponent;
}

const CheckIcon = createContextIcon(Check);
const ChevronDownIcon = createContextIcon(ChevronDown);
const Globe2Icon = createContextIcon(Globe2);
const LanguagesIcon = createContextIcon(Languages);
const LocateFixedIcon = createContextIcon(LocateFixed);
const MapPinIcon = createContextIcon(MapPin);
const NavigationIcon = createContextIcon(Navigation);
const XIcon = createContextIcon(X);
...
function getLanguageLabel(language?: string): string {
  ...
  const key = language.toLowerCase().split("-")[0] ?? language;
...
function getDeviceLanguage(): string {
  try {
    if (Platform.OS === "web") {
      ...
      if (webNavigator?.language) {
        return webNavigator.language.split("-")[0] ?? "fr";
      }
    }
  } catch {
    // Native platforms do not expose browser language.
  }
...
/**
 * La devise n'est jamais déduite artificiellement du locale.
 *
 * La source de vérité doit être :
 * - le contexte persistant fourni par le backend
 * - ou une préférence explicitement définie par l'utilisateur.
 */
...
function RotatingNavigation() {
  ...
  return (
    ...
    >
      <NavigationIcon size={15} color="#A5B4FC" />
    </Animated.View>
  );
}
...
export default function GlobalContextBar({
  ...
  /*
   * `onContextDetected` est volontairement conservé dans l'API publique du
   * composant pour permettre au parent de brancher ultérieurement son moteur
   * natif de localisation sans casser les consommateurs existants.
   *
   * La détection n'est pas simulée ici.
   */
  void onContextDetected;
  ...
  const languageLabel = getLanguageLabel(
    effectiveContext?.language ?? deviceLanguage,
  );

  const currencyLabel = effectiveContext?.currency ?? "Devise locale";

  const timezoneLabel =
    effectiveContext?.timezone ?? deviceTimezone ?? "Fuseau local";
  ...
  useEffect(() => {
    const animation = Animated.timing(chevronAnim, {
      toValue: open ? 1 : 0,
      duration: 220,
  ...
  }, [open, mounted, panelAnim, backdropAnim]);

  /* ==========================================================================
   * LOCATION
   *
   * Native-safe strategy:
   * - aucune navigator.geolocation
   * - aucune dépendance à window
   * - aucune coordonnée fabriquée
   * - la localisation native est fournie par le parent
   * ======================================================================== */

  const detectLocation = useCallback(() => {
    /*
     * Important :
     * cette fonction ne prétend pas avoir détecté une position.
     *
     * Le module de localisation natif doit fournir le contexte réel au parent,
     * qui le transmet ensuite via `context`.
     */
  ...
  const statusText = useMemo(() => {
    ...
    if (detectionState === "unavailable") {
      return "Localisation à configurer";
    }
      ...
      >
        ...
        <View style={styles.triggerIconWrap}>
          {isDetecting ? (
            ...
            <MapPinIcon size={15} color="#A5B4FC" />
          )}
        </View>
        ...
        <View style={styles.triggerText}>
          ...
          <View style={styles.triggerStatusRow}>
            <Globe2Icon size={9} color="rgba(255,255,255,0.4)" />
        ...
        >
          <ChevronDownIcon size={15} color="rgba(255,255,255,0.55)" />
        </Animated.View>
      </Pressable>
      ...
      {mounted ? (
        ...
        >
            ...
            <View style={styles.panelHeaderTop}>
              <View style={styles.panelHeaderContent}>
                <View style={styles.panelHeaderTitleRow}>
                  ...
                  >
                    <Globe2Icon size={13} color="#FFFFFF" />
                  </LinearGradient>
              ...
              >
                <XIcon size={13} color="rgba(255,255,255,0.6)" />
              </Pressable>
            </View>
          </View>
          ...
          <View style={styles.rowsWrap}>
            <ContextRow
              icon={<MapPinIcon size={14} color="rgba(255,255,255,0.7)" />}
              label="Localisation"
              value={cityLabel}
              detail={
                effectiveContext?.country ??
            ...
            <ContextRow
              icon={<LanguagesIcon size={14} color="rgba(255,255,255,0.7)" />}
              label="Langue"
              value={languageLabel}
            ...
            <ContextRow
              icon={<Globe2Icon size={14} color="rgba(255,255,255,0.7)" />}
              label="Fuseau horaire"
              value={timezoneLabel}
            />

            <ContextRow
              icon={<NavigationIcon size={14} color="rgba(255,255,255,0.7)" />}
              label="Position"
              value={
          ...
          {!coordinates ? (
            <View style={styles.detectWrap}>
              <Pressable
                ...
                accessibilityRole="button"
                accessibilityLabel="Configurer ma position"
                style={({ pressed }) => [
                  styles.detectBtn,
              ...
              >
                ...
                >
                  <LocateFixedIcon size={14} color="#FFFFFF" />
          ...
          {detectionState === "denied" ? (
            <View style={styles.deniedFooter}>
              <Text style={styles.deniedText}>
                La localisation n'est pas autorisée. Vous pouvez modifier cette
                préférence depuis les paramètres de l'application.
              </Text>
            </View>
          ) : null}
          ...
          {detectionState === "unavailable" ? (
            <View style={styles.infoFooter}>
              <Text style={styles.infoText}>
                La position doit être fournie par le système de localisation
                natif de l'application.
              </Text>
            </View>
          ) : null}
        </Animated.View>
