  ...
  return (
    <View className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((star) => {
        ...
        return (
          <Pressable key={star} whileTap={onChange ? { scale: 1.3 } : undefined} onPress={onChange ? () => onChange(star) : undefined} className={onChange ? "cursor-pointer" : "cursor-default"} style={{ lineHeight: 0 }}>
            <Star
              size={size}
              style={{ fill: filled ? color : "transparent" }}
            />
          </Pressable>
        );
      })}
...
export function RatingBadge({ avg, count, size = "sm", color = "#F59E0B" }: RatingBadgeProps) {
  ...
  return (
    <View className="flex items-center gap-1">
      ...
      />
      <Text className={`font-bold ${size === "sm" ? "text-[11px]" : "text-sm"}`} style={{ color }}>
        {avg.toFixed(1)}
      </Text>
