...
export const upsert = mutation({
  args: {
    ...
    language: v.optional(
      v.union(v.literal("fr"), v.literal("en"), v.literal("ar")),
    ),
  ...
  handler: async (ctx, args) => {
    ...
    const patch: Partial<
      Omit<Doc<"userPreferences">, "_id" | "_creationTime">
    > = {};
    if (args.theme !== undefined) patch.theme = args.theme;
    if (args.language !== undefined) patch.language = args.language;
    if (args.notifMessages !== undefined)
      patch.notifMessages = args.notifMessages;
    if (args.notifPublications !== undefined)
      patch.notifPublications = args.notifPublications;
    ...
    if (args.feedCategories !== undefined)
      patch.feedCategories = args.feedCategories;
    if (args.favoriteModules !== undefined)
      patch.favoriteModules = args.favoriteModules;
    ...
    if (args.colorBlindMode !== undefined)
      patch.colorBlindMode = args.colorBlindMode;
    if (args.highContrast !== undefined) patch.highContrast = args.highContrast;
    if (args.notifPrefsJson !== undefined)
      patch.notifPrefsJson = args.notifPrefsJson;
...
// ─────────────────────────────────────────────────────────────────────────────
// PRÉFÉRENCES DE NOTIFICATION (JSON typé côté client)
// ─────────────────────────────────────────────────────────────────────────────
//
// Le modèle des préférences de notification (17 types + DND) évolue côté UI.
// On le persiste dans un champ JSON unique plutôt que d'ajouter 20 colonnes
// au schéma. Le parse défensif est fait côté client (use-notifications.ts).
//
// Si un jour le modèle se stabilise et qu'on veut typer au schéma,
// ce champ deviendra obsolète au profit de colonnes explicites.

// Retourne le JSON des préférences de notification de l'utilisateur connecté.
// `undefined` si non authentifié ou si l'utilisateur n'a jamais enregistré
// ses préférences. Le client applique alors ses DEFAULT_PREFS locaux.
export const getMyNotifPrefs = query({
  args: {},
  handler: async (ctx): Promise<string | undefined> => {
    ...
    return prefs?.notifPrefsJson;
  },
});

// Enregistre les préférences de notification de l'utilisateur connecté.
// Crée le document `userPreferences` s'il n'existe pas encore, avec les
// valeurs par défaut du schéma + le JSON fourni.
export const saveNotifPrefs = mutation({
  args: {
    prefsJson: v.string(),
  },
  handler: async (ctx, args): Promise<void> => {
    ...
    // Borne : évite un payload abusif. 4 Ko suffisent très largement
    // pour 17 booléens + 2 strings DND + 1 bool.
    if (args.prefsJson.length > 4096) {
      throw new ConvexError({
        code: "INVALID_ARGUMENT",
        message: "prefsJson trop volumineux",
      });
    }
    ...
    if (existing) {
      await ctx.db.patch(existing._id, { notifPrefsJson: args.prefsJson });
      return;
    }

    // Création complète : DEFAULTS + notifPrefsJson.
    // On réutilise l'objet DEFAULTS du module — aucune valeur inventée,
    // aucune duplication du schéma.
    await ctx.db.insert("userPreferences", {
      userId: user._id,
      ...DEFAULTS,
      notifPrefsJson: args.prefsJson,
    });
  },
});
